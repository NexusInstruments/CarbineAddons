-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ClassResources
-- Copyright (c) NCsoft. All rights reserved
-----------------------------------------------------------------------------------------------

require "Window"

local CRB_ClassResources = {}

local knSaveVersion = 1

function CRB_ClassResources:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_ClassResources:Init()
    Apollo.RegisterAddon(self, nil, nil, {"ActionBarFrame"})
end

function CRB_ClassResources:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ClassResources.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)

	Apollo.RegisterEventHandler("ActionBarLoaded", "OnRequiredFlagsChanged", self)
end

function CRB_ClassResources:OnSave(eType)
...
function CRB_ClassResources:OnRestore(eType, tSavedData)
...
function CRB_ClassResources:OnDocumentReady()
...
function CRB_ClassResources:OnRequiredFlagsChanged()
...
function CRB_ClassResources:OnCharacterCreated()
...
function CRB_ClassResources:OnCreateEsper()
...
function CRB_ClassResources:OnEsperUpdateTimer()
	local unitPlayer = GameLib.GetPlayerUnit()
...
function CRB_ClassResources:OnEsperEnteredCombat(unitPlayer, bInCombat)
...
function CRB_ClassResources:OnEsperOutOfCombatFade()
...
function CRB_ClassResources:OnCreateSlinger()
...
function CRB_ClassResources:OnSlingerUpdateTimer()
	local unitPlayer = GameLib.GetPlayerUnit()
...
function CRB_ClassResources:OnSlingerEnteredCombat(unitPlayer, bInCombat)
	if unitPlayer ~= GameLib.GetPlayerUnit() or not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_ClassResources:OnCreateMedic()
...
function CRB_ClassResources:OnMedicUpdateTimer()
...
function CRB_ClassResources:OnMedicEnteredCombat(unitPlayer, bInCombat)
	if unitPlayer ~= GameLib.GetPlayerUnit() or not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_ClassResources:OnCreateStalker()
...
function CRB_ClassResources:OnStalkerUpdateTimer()
	local unitPlayer = GameLib.GetPlayerUnit()
	local bInCombat = unitPlayer:IsInCombat()
...
function CRB_ClassResources:OnCreateWarrior()
...
function CRB_ClassResources:OnWarriorUpdateTimer()
	local unitPlayer = GameLib.GetPlayerUnit()
	local bInCombat = unitPlayer:IsInCombat()
...
function CRB_ClassResources:OnWarriorResource_ChargeBarOverdriveTick()
	self.timerOverdriveTick:Stop()
	self.tWindowMap["ChargeBarOverdriven"]:SetProgress(0, 1 / 8)
end

function CRB_ClassResources:OnWarriorResource_ChargeBarOverdriveDone()
	self.timerOverdriveDone:Stop()
	self.bOverDriveActive = false
end
...
function CRB_ClassResources:OnCreateEngineer()
...
function CRB_ClassResources:OnEngineerUpdateTimer()
	local unitPlayer = GameLib.GetPlayerUnit()
	local bInCombat = unitPlayer:IsInCombat()
	local nResourceCurrent = unitPlayer:GetResource(1)
...
function CRB_ClassResources:OnStanceBtn(wndHandler, wndControl)
...
function CRB_ClassResources:HelperShowPetBar(bShowIt)
	self.tWindowMap["PetBarContainer"]:Show(bShowIt)
	self.tWindowMap["PetBtn"]:SetCheck(not bShowIt)
end

function CRB_ClassResources:OnPetBtn(wndHandler, wndControl)
	self.bShowPet = not self.tWindowMap["PetBarContainer"]:IsShown()
	
	self:HelperShowPetBar(self.bShowPet)
end

function CRB_ClassResources:OnShowActionBarShortcut(eWhichBar, bIsVisible, nNumShortcuts)
...
function CRB_ClassResources:OnEngineerPetBtnMouseEnter(wndHandler, wndControl)
	local strHover = ""
...
function CRB_ClassResources:OnEngineerPetBtnMouseExit(wndHandler, wndControl)
	self.tWindowMap["PetText"]:SetText(self.tWindowMap["PetText"]:GetData() or "")
	wndHandler:SetBGColor("UI_AlphaPercent50")
end

function CRB_ClassResources:OnPetStanceChanged(petId)
	-- Pet_GetStance(0) -- First arg is for the pet ID, 0 means all engineer pets
...
function CRB_ClassResources:OnPetSpawned(petId)
	-- Pet_GetStance(0) -- First arg is for the pet ID, 0 means all engineer pets
	if self.ktEngineerStanceToShortString[Pet_GetStance(0)] then
		self.tWindowMap["PetText"]:SetText(self.ktEngineerStanceToShortString[Pet_GetStance(0)])
		self.tWindowMap["PetText"]:SetData(self.ktEngineerStanceToShortString[Pet_GetStance(0)])
...
function CRB_ClassResources:OnGeneratePetCommandTooltip(wndControl, wndHandler, eType, arg1, arg2)
	local xml = nil
...
local CRB_ClassResourcesInst = CRB_ClassResources:new()
CRB_ClassResourcesInst:Init()
