-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_PathFrame
...
-----------------------------------------------------------------------------------------------
-- CRB_PathFrame Module Definition
-----------------------------------------------------------------------------------------------
local CRB_PathFrame = {} 
...
function CRB_PathFrame:Init()
    Apollo.RegisterAddon(self, nil, nil, {"ActionBarFrame", "Abilities"})
end 

-----------------------------------------------------------------------------------------------
-- CRB_PathFrame OnLoad
-----------------------------------------------------------------------------------------------
function CRB_PathFrame:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_PathFrame.xml")
	
	self.nSelectedPathId = nil
	self.bHasPathAbilities = false
end

function CRB_PathFrame:OnSave(eType)
...
function CRB_PathFrame:OnRestore(eType, tSavedData)
...
function CRB_PathFrame:GetAsyncLoadStatus()
...
function CRB_PathFrame:OnAsyncLoad()
	...
	--Load Forms
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_PathFrameForm", "FixedHudStratum", self)
...
-----------------------------------------------------------------------------------------------
-- CRB_PathFrame Functions
-----------------------------------------------------------------------------------------------
function CRB_PathFrame:DrawPathAbilityList()
	if not self.unitPlayer then
		return
	end
...
function CRB_PathFrame:HelperSetPathAbility(nAbilityId)
...
-----------------------------------------------------------------------------------------------
-- CRB_PathFrameForm Functions
-----------------------------------------------------------------------------------------------
function CRB_PathFrame:OnGenerateTooltip(wndControl, wndHandler, tType, arg1, arg2)
	if Tooltip ~= nil and Tooltip.GetSpellTooltipForm ~= nil then
		Tooltip.GetSpellTooltipForm(self, wndControl, arg1)
	end
end

function CRB_PathFrame:OnPathOptionToggle(wndHandler, wndControl, eMouseButton)
...
function CRB_PathFrame:OnPathBtn(wndControl, wndHandler)
...
function CRB_PathFrame:OnCloseBtn()
	self.wndMenu:Show(false)
end

function CRB_PathFrame:OnChangeWorld()
	self.wndMenu:Show(false)
end

function CRB_PathFrame:OnUnitEnteredCombat(unit, bIsInCombat)
...
function CRB_PathFrame:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
-----------------------------------------------------------------------------------------------
-- CRB_PathFrame Instance
-----------------------------------------------------------------------------------------------
local CRB_PathFrameInst = CRB_PathFrame:new()
