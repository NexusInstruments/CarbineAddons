-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_CraftingGrid
...
function CRB_CraftingGrid:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
	return o
...
function CRB_CraftingGrid:Init()
    Apollo.RegisterAddon(self)
end

function CRB_CraftingGrid:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CraftingGrid.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_CraftingGrid:OnDocumentReady()
...
function CRB_CraftingGrid:OnCraftingGrid_TimerCraftingStationCheck()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:FindChild("BGCraftingStationBlocker"):Show(not CraftingLib.IsAtCraftingStation())
	end
end

function CRB_CraftingGrid:OnGenericEvent_StartCraftingGrid(idSchematic)
...
function CRB_CraftingGrid:Initialize()
	if self.wndMain and self.wndMain:IsValid() then
		return
	end

    self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_CraftingGridForm", nil, self)
...
function CRB_CraftingGrid:RedrawAll()
...
function CRB_CraftingGrid:BuildVariantItem(wndParent, itemRecipe, tSchem, bHitThisSubSchematic)
...
function CRB_CraftingGrid:RedrawAllDetailed(idSchematic, tSchematicInfo, tCurrentCraft, bCurrentCraftStarted)
	-- Grid Glows
...
function CRB_CraftingGrid:RedrawCash()
...
function CRB_CraftingGrid:OnPreviewStartCraftBtn(wndHandler, wndControl) -- PreviewStartCraftBtn, data is idSchematic
...
function CRB_CraftingGrid:OnCraftingUpdateCurrent()
...
function CRB_CraftingGrid:OnCraftBtn(wndHandler, wndControl) -- CraftBtn
...
function CRB_CraftingGrid:OnCraftingGrid_CraftBtnTimer()
	self.timerBtn:Stop()
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_CraftingGrid:OnCraftingSchematicLearned(idTradeskill, idSchematic)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_CraftingGrid:OnCraftingDiscoveryHotCold(eHotCold, eDirection)
...
function CRB_CraftingGrid:OnAdditiveClick(wndHandler, wndControl) -- AdditiveItem, data is an itemData
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_CraftingGrid:HelperBuildMarker()
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_CraftingGrid:OnCatalystItemToggle(wndHandler, wndControl) -- CatalystItem, data is itemCatalyst or nil
	self.wndMain:FindChild("CatalystGlobalList"):SetData(wndHandler:IsChecked() and wndHandler:GetData() or nil)
end

function CRB_CraftingGrid:ExitAndReset(wndHandler, wndControl)
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
	end
...
function CRB_CraftingGrid:OnCloseBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_CraftingGrid:OnP2PTradeExitAndReset()
	local tCurrentCraft = CraftingLib.GetCurrentCraft()
...
function CRB_CraftingGrid:OnAdditiveMouseEnter(wndHandler, wndControl) -- AdditiveItem's AdditiveMouseCatcher, data is an itemData
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_CraftingGrid:OnAdditiveListMouseExit(wndHandler, wndControl) -- AdditiveItem or Code
	if wndHandler == wndControl and self.wndAdditiveEstimate and self.wndAdditiveEstimate:IsValid() then
		self.wndAdditiveEstimate:Destroy()
	end
end

function CRB_CraftingGrid:OnAdditiveHeaderBtnToggle(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_CraftingGrid:OnMaterialsLockedItemBtn(wndHandler, wndControl) -- wndHandler is "MaterialsLockedItemBtn" and its data is achSource
...
function CRB_CraftingGrid:BuildNewGrid(idSchematic)
	local tSchematicInfo = CraftingLib.GetSchematicInfo(idSchematic)
...
function CRB_CraftingGrid:OnVariantItemIconClick(wndHandler, wndControl)
...
function CRB_CraftingGrid:OnHitzoneClick(wndHandler, wndControl)
...
function CRB_CraftingGrid:OnHitZoneMouseEnter(wndHandler, wndControl)
	local eAxisId = ktHitZoneToAxisId[wndHandler:GetName()] or 0
...
function CRB_CraftingGrid:OnHitZoneMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl then
		for idx, wndCurr in pairs(self.wndMain:FindChild("AdditiveList"):GetChildren()) do
			if wndCurr:FindChild("AdditiveHeaderBtn") then
...
function CRB_CraftingGrid:BuildNewDiscoverable(tSchem) -- discoveryAngle, discoveryDistanceMin, discoveryDistanceMax
...
function CRB_CraftingGrid:BuildNewGridCircle(tSchem) -- vectorX, vectorY, radius
...
function CRB_CraftingGrid:HelperDetermineAdditiveDirection(tSchem)
	if tSchem.bIsUndiscovered then
		return 0
	end
...
function CRB_CraftingGrid:HelperVerifyAxis(tFilters, fVectorX, fVectorY)
	local bDisplay = true
...
function CRB_CraftingGrid:HelperStringMoneyConvert(nInCopper)
	local strResult = ""
...
function CRB_CraftingGrid:HelperBuildItemTooltip(wndArg, itemCurr)
	Tooltip.GetItemTooltipForm(self, wndArg, itemCurr, { bPrimary = true, bSelling = false })
end

function CRB_CraftingGrid:LoadByName(strForm, wndParent, strCustomName)
...
local CRB_CraftingGridInst = CRB_CraftingGrid:new()
CRB_CraftingGridInst:Init()
