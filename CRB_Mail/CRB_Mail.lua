...
function CRB_Mail:Init()
	Apollo.RegisterAddon(self, false, "", {"Util"})
end

function CRB_Mail:OnSave(eType)
...
function CRB_Mail:OnRestore(eType, tSavedData)
...
function CRB_Mail:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MailForms.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Mail:OnDocumentReady()
	...
	self.wndMain		= Apollo.LoadForm(self.xmlDoc, "CRB_MailForm", nil, self) --Our main form.
...
function CRB_Mail:OnInterfaceMenuListHasLoaded()
	Event_FireGenericEvent("InterfaceMenuList_NewAddOn", Apollo.GetString("InterfaceMenu_Mail"), {"ToggleMailWindow", "CRB_Mail", "Icon_Windows32_UI_CRB_InterfaceMenu_Mail"})
	self:CalculateMailAlert()
end

function CRB_Mail:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementAdd", {wnd = self.wndMain, strName = Apollo.GetString("InterfaceMenu_Mail")})
end

function CRB_Mail:OnPlayerCurrencyChanged()
...
function CRB_Mail:OpenMailWindow()
...
function CRB_Mail:CloseMailWindow()
	self:CalculateMailAlert()
	self.wndMain:Close()
...
function CRB_Mail:OnMainMailWindowClosed(wndHandler, wndControl)
...
function CRB_Mail:ToggleWindow()
...
function CRB_Mail:OnMailBoxActivate()
	self:OpenMailWindow()
end

function CRB_Mail:OnMailBoxDeactivate()
	if self.luaComposeMail ~= nil then
		self.luaComposeMail:UpdateControls()
	end
end

function CRB_Mail:DestroyList()
...
function CRB_Mail.SortMailItems(a, b)
...
function CRB_Mail.SortMailWindows(wndA, wndB)
...
function CRB_Mail:CascadeWindow(wndMailItem)
...
function CRB_Mail:PopulateList()
	...
	self.wndMailList:ArrangeChildrenVert(0, CRB_Mail.SortMailWindows)
	self:CalculateMailAlert()
	
	self:RefreshActionsButton()
end

function CRB_Mail:AddMailToUpdateTime(strId, wndMail)
	self.arMailToUpdate[strId] = wndMail
	self.timerMailUpdateDelay:Start()
end

function CRB_Mail:AddMailToOpenTimer(mail)
	if mail == nil then
		return
	end
...
function CRB_Mail:UpdateMailItemsTimer()
	...
	if next(self.arMailToUpdate) ~= nil then
		...
		table.sort(tSorted, CRB_Mail.SortMailItems)
...
function CRB_Mail:CalculateMailAlert()
...
function CRB_Mail:OnAvailableMail(arItems, bNewMail)
...
function CRB_Mail:OnUnavailableMail(arMailIds)
...
function CRB_Mail:OnRefreshMail(strMailId)
	if not self.wndMain:IsVisible() then
		return
	end
...
function CRB_Mail:OnMailRead(strMailId)
	if self.tMailItemWnds[strMailId] ~= nil then
		self:AddMailToUpdateTime(strMailId, self.tMailItemWnds[strMailId])
	end
end

function CRB_Mail:OnMailResult(eResult)
...
function CRB_Mail:OnMailAddAttachment(nValue)
...
function CRB_Mail:OnCloseErrorBtn(wndHandler, wndControl)
...
--------------------/CRB_Mail Form Controls/-----------------------------
function CRB_Mail:RefreshActionsButton()
...
function CRB_Mail:OnDeselectAll()
	for idx, wndMail in pairs(self.tMailItemWnds) do
		wndMail:FindChild("SelectMarker"):SetCheck(false)
	end
...
--------------------/CRB_Mail List Item Controls/-----------------------------
function CRB_Mail:OnItemCheck()
	Sound.Play(Sound.PlayUI19SelectStoreItemVirtual)
	self:RefreshActionsButton()
end

function CRB_Mail:OnItemUncheck()
	Sound.Play(Sound.PlayUI19SelectStoreItemVirtual)
	self.wndToggleAllBtn:SetCheck(false)
...
function CRB_Mail:UpdateListItem(wndMailItem, msgMail)
	if msgMail == nil then
		return
	end
...
function CRB_Mail:OnMailItemClick(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Mail:OpenReceivedMessage(msgMail)
	local strId = msgMail:GetIdStr()
...
function CRB_Mail:OnTooltipAttachment( wndHandler, wndControl, eToolTipType, x, y )
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Mail:OnOpenMailBtn(wndHandler, wndControl, eMouseButton)
	self:OpenSelectedMail(false)
end

function CRB_Mail:OnConfirmOpenBtn(wndHandler, wndControl, eMouseButton)
	self:OpenSelectedMail(true)
	self.wndConfirmOpenBlocker:Close()
end

function CRB_Mail:OpenSelectedMail(bConfirmed)
	local nCount = 0
	local tSelected = {}
	for strId, wndMail in pairs(self.tMailItemWnds) do
...
function CRB_Mail:OnCancelOpenBtn(wndHandler, wndControl, eMouseButton)
	self.wndConfirmOpenBlocker:Close()
end


function CRB_Mail:OnDeleteMailBtn(wndHandler, wndControl, eMouseButton)
	self:DeleteSelectedMail(false)
end

function CRB_Mail:OnConfirmDeleteBtn(wndHandler, wndControl, eMouseButton)
	self:DeleteSelectedMail(true)
	self.wndConfirmDeleteBlocker:Close()
end

function CRB_Mail:DeleteSelectedMail(bConfirmed)
	local tSelectedMail = {}
	for strId, wndMail in pairs(self.tMailItemWnds) do
		if wndMail:FindChild("SelectMarker"):IsChecked() then
...
function CRB_Mail:OnCancelDeleteBtn(wndHandler, wndControl, eMouseButton)
	self.wndConfirmDeleteBlocker:Close()
end


function CRB_Mail:OnTakeMailBtn(wndHandler, wndControl, eMouseButton)
	self:TakeSelectedMail(false)
end

function CRB_Mail:OnConfirmTakeBtn(wndHandler, wndControl, eMouseButton)
	self:TakeSelectedMail(true)
	self.wndConfirmTakeBlocker:Close()
end

function CRB_Mail:TakeSelectedMail(bConfirmed)
	local tSelectedMail = {}
	for strId, wndMail in pairs(self.tMailItemWnds) do
		if wndMail:FindChild("SelectMarker"):IsChecked() then
			tSelectedMail[#tSelectedMail + 1] = wndMail:GetData()
...
function CRB_Mail:OnCancelTakeBtn(wndHandler, wndControl, eMouseButton)
	self.wndConfirmTakeBlocker:Close()
end


function CRB_Mail:OnMarkReadMailBtn(wndHandler, wndControl, eMouseButton)
	self.wndActionPopout:Close()
...
function CRB_Mail:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor ~= GameLib.CodeEnumTutorialAnchor.CRB_Mail then return end
...
function MailReceived:OnAcceptCODBtn(wndHandler, wndControl)
	...
	self.msgMail:PayCoD()
	CRB_Mail.strPendingCOD = self.msgMail:GetIdStr()
	-- Timer UpdateControl handles changes to message if it occures
