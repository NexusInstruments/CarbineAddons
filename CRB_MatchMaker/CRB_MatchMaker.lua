-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_MatchMaker
...
-----------------------------------------------------------------------------------------------
-- CRB_MatchMaker Module Definition
-----------------------------------------------------------------------------------------------
local CRB_MatchMaker = {}
...
function CRB_MatchMaker:Init()
    Apollo.RegisterAddon(self)
end

-----------------------------------------------------------------------------------------------
-- CRB_MatchMaker OnLoad
-----------------------------------------------------------------------------------------------
function CRB_MatchMaker:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MatchMaker.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_MatchMaker:OnDocumentReady()
...
function CRB_MatchMaker:OnInterfaceMenuListHasLoaded()
...
function CRB_MatchMaker:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementAdd", {wnd = self.wndMain, strName = Apollo.GetString("InterfaceMenu_GroupFinder")})
end

-----------------------------------------------------------------------------------------------
-- CRB_MatchMaker Functions
-----------------------------------------------------------------------------------------------

function CRB_MatchMaker:OnLevelUpUnlock_GroupFinder_General()
	self:OnCRB_MatchMakerOn()
	self.wndMain:ToFront()
end

function CRB_MatchMaker:OnLevelUpUnlock_GroupFinder_Dungeons()
	self.eSelectedTab = MatchingGame.MatchType.Dungeon
	self:OnLevelUpUnlock_GroupFinder_General()
end

function CRB_MatchMaker:OnLevelUpUnlock_GroupFinder_Adventures()
	self.eSelectedTab = MatchingGame.MatchType.Adventure
	self:OnLevelUpUnlock_GroupFinder_General()
end

function CRB_MatchMaker:OnLevelUpUnlock_GroupFinder_Arenas()
	self.eSelectedTab = MatchingGame.MatchType.Arena
	self:OnLevelUpUnlock_GroupFinder_General()
end

function CRB_MatchMaker:OnLevelUpUnlock_GroupFinder_Warplots()
	self.eSelectedTab = MatchingGame.MatchType.Warplot
	self:OnLevelUpUnlock_GroupFinder_General()
end

function CRB_MatchMaker:OnLevelUpUnlock_PvP_Battleground()
	self.eSelectedTab = MatchingGame.MatchType.Battleground
	self:OnLevelUpUnlock_GroupFinder_General()
end

function CRB_MatchMaker:OnToggleMatchMaker()
...
function CRB_MatchMaker:ReloadMaps()
	if self.wndMain:IsShown() then
...
function CRB_MatchMaker:ReloadTeams()
...
function CRB_MatchMaker:OnCRB_MatchMakerOn()
...
function CRB_MatchMaker:OnRandomEntryMouseEnter(wndHandler, wndControl)
...
function CRB_MatchMaker:OnRandomEntryMouseExit(wndHandler, wndControl)
	local wnd = wndControl and wndControl:IsValid() and wndControl:FindChild("Rewards") or nil
...
function CRB_MatchMaker:HelperConfigureListAndRealmSelect()
...
function CRB_MatchMaker:HelperConfigureListAndRating()
	local nLeft, nTop, nRight, nBottom = self.wndListParent:GetAnchorOffsets()
	local nLeft2, nTop2, nRight2, nBottom2 = self.wndMyRating:GetAnchorOffsets()
...
function CRB_MatchMaker:HelperConfigureListAndRole() --dungeons & adventures
	local nLeft, nTop, nRight, nBottom = self.wndListParent:GetAnchorOffsets()
...
function CRB_MatchMaker:HelperConfigureListAndFindOthers() --shiphands
	local nLeft, nTop, nRight, nBottom = self.wndListParent:GetAnchorOffsets()
...
function CRB_MatchMaker:HelperConfigureListAndTeams(tTeam, bForWarplots)
	local nLeft, nTop, nRight , nBottom  = self.wndListParent:GetAnchorOffsets()
...
function CRB_MatchMaker:OnClose()
	Event_FireGenericEvent("LFGWindowHasBeenClosed")
end

function CRB_MatchMaker:OnEnteredCombat(unitPlayer, bInCombat)
...
function CRB_MatchMaker:OnPlayerChanged()
...
function CRB_MatchMaker:OnArenaTeamAddedOrDisbanded()
	if self.wndMain:IsShown() and self.eSelectedTab == MatchingGame.MatchType.Arena then
		self:HelperConfigureListAndTeams(self.tArenaTeams, false)
	end
end

function CRB_MatchMaker:OnWarpartyAddedOrDisbanded()
...
function CRB_MatchMaker:OnGuildResult(guildUpdated, strName, nRank, eResult)
...
function CRB_MatchMaker:RefreshStatus()
...
function CRB_MatchMaker:OnGameReady(bInProgress)
...
function CRB_MatchMaker:OnJoinQueue()
	self:RefreshStatus()
	self:HelperFindMatchesQueued()
...
function CRB_MatchMaker:OnLeaveQueue()
	self.wndConfirmRole:Show(false)
	self:RefreshStatus()
	self.matchesQueued = {}
end

function CRB_MatchMaker:OnVoteKickBegin(tPlayerInfo)
...
function CRB_MatchMaker:OnVoteKickEnd()
	self.wndVoteKick:Show(false)
	self:RefreshStatus()
end

function CRB_MatchMaker:OnVoteSurrenderBegin()
...
function CRB_MatchMaker:OnVoteSurrenderEnd()
	self.wndVoteSurrender:Show(false)
	self:RefreshStatus()
end

function CRB_MatchMaker:OnDuelStateChanged(eNewState, unitOpponent)
...
function CRB_MatchMaker:OnDuelAccepted(fCountdownTime)
...
function CRB_MatchMaker:OnDuelLeftArea(fTimeRemaining)
...
function CRB_MatchMaker:OnDuelCancelWarning()
	self.wndDuelWarning:Show(false)
end

function CRB_MatchMaker:OnRoleCheckStarted()
...
function CRB_MatchMaker:OnRoleCheckHidden()
	self.wndConfirmRole:Show(false)
	self:RefreshStatus()
end

function CRB_MatchMaker:OnAcceptRole()
	local tSelectedRoles = MatchingGame.GetSelectedRoles()
	if tSelectedRoles == nil then
		return
	end
...
function CRB_MatchMaker:OnCancelRole()
	MatchingGame.DeclineRoleCheck()
end

function CRB_MatchMaker:OnModeBtnCheck()
...
function CRB_MatchMaker:OnTeamRosterBtn(wndHandler, wndControl)
...
function CRB_MatchMaker:OnEntryInfoCheck(wndHandler, wndControl)
	-- destroy previous
...
function CRB_MatchMaker:OnEntryInfoUncheck()
...
function CRB_MatchMaker:OnMoreInfoPanelClosed(wndHandler, wndControl)
...
-----------------------------------------------------------------------------------------------
-- CRB_MatchMakerForm Functions
-----------------------------------------------------------------------------------------------

function CRB_MatchMaker:OnJoin()
	if next(self.matchesSelected) == nil then
		return
	end
	self.matchesQueued = {}
...
function CRB_MatchMaker:OnJoinAsGroup()
	if next(self.matchesSelected) == nil then
		return
	end
	self.matchesQueued = {}
...
function CRB_MatchMaker:OnLeaveQueueBtn(wndHandler, wndControl)
	if MatchingGame.CanLeaveQueueAsGroup() then
...
function CRB_MatchMaker:OnLeaveMatchBtn(wndHandler, wndControl)
	if MatchingGame.IsInMatchingGame() then
		MatchingGame.LeaveMatchingGame()
	end
end

function CRB_MatchMaker:OnTeleportIntoMatchBtn(wndHandler, wndControl)
	if not MatchingGame.IsInMatchingInstance() then
		MatchingGame.TransferIntoMatchingGame()
	end
end

function CRB_MatchMaker:OnFindReplacementsBtn(wndHandler, wndControl)
	MatchingGame.LookForReplacements()
end

function CRB_MatchMaker:OnCancelReplacementsBtn(wndHandler, wndControl)
	MatchingGame.StopLookingForReplacements()
end

function CRB_MatchMaker:OnVoteDisbandBtn( wndHandler, wndControl, eMouseButton )
	MatchingGame.InitiateVoteToSurrender()
end

function CRB_MatchMaker:OnMatchingGameSelect(wndHandler, wndControl)
...
function CRB_MatchMaker:OnMatchingGameUnSelect(wndHandler, wndControl, eMouseButton)
	local matchGame = wndControl:GetData()
...
--[[function CRB_MatchMaker:OnMatchingGameOption(wnd, wndCtrl)
	local game = wnd:GetData()
	self.matchSelectedDesc = game
	self:RefreshStatus()
end--]]

function CRB_MatchMaker:OnPendingGameResponded(wndHandler, wndControl, bResponse)
...
function CRB_MatchMaker:OnVoteKickYes(wndHandler, wndControl)
	MatchingGame.CastVoteKick(true)
	self.wndVoteKick:Show(false)
end

function CRB_MatchMaker:OnVoteKickNo(wndHandler, wndControl)
	MatchingGame.CastVoteKick(false)
	self.wndVoteKick:Show(false)
end

function CRB_MatchMaker:OnVoteSurrenderYes(wndHandler, wndControl)
	MatchingGame.CastVoteSurrender(true)
	self.wndVoteSurrender:Show(false)
end

function CRB_MatchMaker:OnVoteSurrenderNo(wndHandler, wndControl)
	MatchingGame.CastVoteSurrender(false)
	self.wndVoteSurrender:Show(false)
end

function CRB_MatchMaker:OnAcceptDuel(wndHandler, wndControl)
	GameLib.AcceptDuel()
	self.wndDuelRequest:Show(false)
end

function CRB_MatchMaker:OnDeclineDuel(wndHandler, wndControl)
	GameLib.DeclineDuel()
	self.wndDuelRequest:Show(false)
end

function CRB_MatchMaker:OnSelectBattlegrounds(wndHandler, wndControl)
	if self.eSelectedTab == MatchingGame.MatchType.Battleground then
		return
	end
...
function CRB_MatchMaker:OnSelectArenas(wndHandler, wndControl)
	if self.eSelectedTab == MatchingGame.MatchType.Arena then
		return
	end
...
function CRB_MatchMaker:OnSelectDungeons(wndHandler, wndControl)
...
function CRB_MatchMaker:OnSelectAdventures(wndHandler, wndControl)
...
function CRB_MatchMaker:OnSelectShiphands(wndHandler, wndControl)
...
function CRB_MatchMaker:OnSelectWarplots(wndHandler, wndControl)
...
function CRB_MatchMaker:OnSelectRatedBattlegrounds(wndHandler, wndControl)
...
function CRB_MatchMaker:OnSelectOpenArenas(wndHandler, wndControl)
...
function CRB_MatchMaker:OnOtherFilterChecked( wndHandler, wndControl, eMouseButton )
	Apollo.SetConsoleVariable(kstrConsoleDoNotFindOthers, true)
end

function CRB_MatchMaker:OnOtherFilterUnchecked( wndHandler, wndControl, eMouseButton )
	Apollo.SetConsoleVariable(kstrConsoleDoNotFindOthers, false)
end

function CRB_MatchMaker:OnAllowMercChecked( wndHandler, wndControl, eMouseButton )
	Apollo.SetConsoleVariable(kstrConsoleAllowMercFilter, true)
end

function CRB_MatchMaker:OnAllowMercUnchecked( wndHandler, wndControl, eMouseButton )
	Apollo.SetConsoleVariable(kstrConsoleAllowMercFilter, false)
end

function CRB_MatchMaker:OnAllowWarpartyChecked( wndHandler, wndControl, eMouseButton )
	Apollo.SetConsoleVariable(kstrConsoleAllowWarpartyFilter, true)
end

function CRB_MatchMaker:OnAllowWarpartyUnchecked( wndHandler, wndControl, eMouseButton )
	Apollo.SetConsoleVariable(kstrConsoleAllowWarpartyFilter, false)
end

function CRB_MatchMaker:OnTankChecked(wndHandler, wndControl)
...
function CRB_MatchMaker:OnTankUnchecked(wndHandler, wndControl)
	if wndHandler == self.tRoleCheckRoleButtons[MatchingGame.Roles.Tank] or wndHandler == self.tGroupFinderRoleButtons[MatchingGame.Roles.Tank] then
...
function CRB_MatchMaker:OnHealerChecked(wndHandler, wndControl)
...
function CRB_MatchMaker:OnHealerUnchecked(wndHandler, wndControl)
	if wndHandler == self.tRoleCheckRoleButtons[MatchingGame.Roles.Healer] or wndHandler == self.tGroupFinderRoleButtons[MatchingGame.Roles.Healer] then
...
function CRB_MatchMaker:OnDPSChecked(wndHandler, wndControl)
...
function CRB_MatchMaker:OnDPSUnchecked(wndHandler, wndControl)
	if wndHandler == self.tRoleCheckRoleButtons[MatchingGame.Roles.DPS] or wndHandler == self.tGroupFinderRoleButtons[MatchingGame.Roles.DPS] then
...
function CRB_MatchMaker:OnMatchEntered()
	self:RefreshStatus()
	self.wndMain:Show(false)
end

function CRB_MatchMaker:GetTimeString(fTimeInSeconds)
...
--[[
function CRB_MatchMaker:StartMatchTimer()
	Apollo.CreateTimer("MatchTimer", 1.0, true)
	Apollo.StartTimer("MatchTimer")
end

function CRB_MatchMaker:OnMatchTimer()
	self.fTimeRemaining = self.fTimeRemaining - 1
end
--]]

function CRB_MatchMaker:OnMatchExited()
	self:RefreshStatus()
	self.wndMain:Show(false)
end

function CRB_MatchMaker:StartQueueTimer()
	Apollo.CreateTimer("QueueTimer", 1.0, true)
	Apollo.StartTimer("QueueTimer")
end

function CRB_MatchMaker:OnQueueTimer()
...
function CRB_MatchMaker:OnTogglePvp()
	GameLib.TogglePvpFlags()
end

function CRB_MatchMaker:SetFlagToggleButton()
...
function CRB_MatchMaker:OnUnitPvpFlagsChanged(unitChanged)
...
function CRB_MatchMaker:OnCooldownTimer()
...
function CRB_MatchMaker:StartDuelCountdownTimer()
	Apollo.CreateTimer("DuelCountdownTimer", 1.0, false)
	Apollo.StartTimer("DuelCountdownTimer")
end

function CRB_MatchMaker:OnDuelCountdownTimer()
...
function CRB_MatchMaker:StartDuelWarningTimer()
	Apollo.CreateTimer("DuelWarningTimer", 1.0, false)
	Apollo.StartTimer("DuelWarningTimer")
end

function CRB_MatchMaker:OnDuelWarningTimer()
...
function CRB_MatchMaker:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
function CRB_MatchMaker:DisplayPendingInfo()
...
function CRB_MatchMaker:OnCheckRating(strCommand, strRatingType)
...
function CRB_MatchMaker:HelperFindMatchesQueued()
...
-----------------------------------------------------------------------------------------------
-- CRB_MatchMaker Instance
-----------------------------------------------------------------------------------------------
local CRB_MatchMakerInst = CRB_MatchMaker:new()
CRB_MatchMakerInst:Init()
