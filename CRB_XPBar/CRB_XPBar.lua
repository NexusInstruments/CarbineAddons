-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_XPBar
...
function CRB_XPBar:Init()
    Apollo.RegisterAddon(self)
end

function CRB_XPBar:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_XPBar.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_XPBar:OnDocumentReady()
...
function CRB_XPBar:RedrawCooldown()
	Apollo.StopTimer("BaseBarCorner_RedrawCooldown")
	self.bOnRedrawCooldown = false
	self:RedrawAllPastCooldown()
end

function CRB_XPBar:RedrawAll()
...
function CRB_XPBar:RedrawAllPastCooldown()
...
function CRB_XPBar:RedrawPathXP()
...
function CRB_XPBar:ConfigurePathXPTooltip(unitPlayer)
	if not PlayerPathLib then
		return ""
	end
...
function CRB_XPBar:ConfigureEPTooltip(unitPlayer)
...
function CRB_XPBar:ConfigureXPTooltip(unitPlayer)
	local nCurrentXP = GetXp() - GetXpToCurrentLevel() 		-- current amount of xp into the current level
	local nNeededXP = GetXpToNextLevel() 					-- total amount needed to move through current level
	local nRestedXP = GetRestXp() 							-- amount of rested xp
	local nRestedXPPool = GetRestXpKillCreaturePool() 		-- amount of rested xp remaining from creature kills
...
function CRB_XPBar:OnEnteredCombat(unitArg, bInCombat)
...
function CRB_XPBar:OnClearCombatFlag()
	self.bInCombat = false
	self:RedrawAll()
end

function CRB_XPBar:OnXpChanged()
	if GetXp() == 0 then
		return
	end
...
function CRB_XPBar:OnPathClicked()
	Event_FireGenericEvent("PlayerPathShow")
end

function CRB_XPBar:OnXpClicked()
	Event_FireGenericEvent("ToggleCharacterWindow")
end

function CRB_XPBar:OnUpdateInventory()
	local unitPlayer = GameLib.GetPlayerUnit()
	if not unitPlayer then
		return
	end
...
function CRB_XPBar:OnToggleFromDatachronIcon()
...
---------------------------------------------------------------------------------------------------
function CRB_XPBar:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
local BaseBarCornerInst = CRB_XPBar:new()
