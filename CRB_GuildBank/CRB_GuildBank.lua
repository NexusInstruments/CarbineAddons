---------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_GuildBank
...
function CRB_GuildBank:Init()
    Apollo.RegisterAddon(self)
end

function CRB_GuildBank:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_GuildBank.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_GuildBank:OnDocumentReady()
...
function CRB_GuildBank:GuildInitialize()
	...
	self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_GuildBankForm", nil, self)
...
function CRB_GuildBank:Reinitialize(guildToInit)
...
function CRB_GuildBank:Initialize(guildOwner)
...
function CRB_GuildBank:OnCloseBank()
...
function CRB_GuildBank:OnCloseBtnSignal(wndHandler, wndControl, eMouseButton)
	self:OnCloseBank()
...
function CRB_GuildBank:OnBankTabBtnVault(wndHandler, wndControl)
...
function CRB_GuildBank:OnBankTabMouseEnter(wndHandler, wndControl)
...
function CRB_GuildBank:OnBankTabCheck(wndHandler, wndControl)
...
function CRB_GuildBank:OnBankTabUncheck(wndHandler, wndControl)
...
function CRB_GuildBank:OnGuildBankTab(guildOwner, nTab)
...
function CRB_GuildBank:HelperDrawBankItem(itemDrawing, nTab, nInventorySlot)
...
function CRB_GuildBank:OnGuildBankItem(guildOwner, nTab, nInventorySlot, itemUpdated, bRemoved)
...
function CRB_GuildBank:OnBankItemMouseButtonDown(wndHandler, wndControl, eMouseButton, bDoubleClick)
...
function CRB_GuildBank:OnBankItemBeginDragDrop(wndHandler, wndControl, nTransferStackCount) -- BankItemIcon
...
function CRB_GuildBank:OnBankItemQueryDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType)
	if wndHandler ~= wndControl then
...
function CRB_GuildBank:OnBankItemDragDropClear() -- Any UI
	self:OnBankItemDragDropCancel()
end

function CRB_GuildBank:OnBankItemDragDropEnd() -- Any UI
	self:OnBankItemDragDropCancel()
end

function CRB_GuildBank:OnBankItemDragDropCancel() -- Also called from UI
	self.tCurrentDragData = nil
end

function CRB_GuildBank:OnBankItemEndDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nBagSlot) -- Bank Icon
...
function CRB_GuildBank:CreateSplitWindow(item, wndParent)
...
function CRB_GuildBank:OnSplitStackCloseClick()
...
function CRB_GuildBank:OnSplitStackConfirm(wndHandler, wndCtrl)
...
function CRB_GuildBank:OnBankTabBtnCash()
...
function CRB_GuildBank:OnGuildCashDeposit(wndHandler, wndControl)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnGuildCashWithdraw(wndHandler, wndControl)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnGuildCashInteractEditCashWindow(wndHandler, wndControl) -- GuildCashInteractEditCashWindow
...
function CRB_GuildBank:OnPlayerCurrencyChanged()
...
function CRB_GuildBank:OnGuildInfluenceAndMoney(guildOwner, nInfluence, monCash)
...
function CRB_GuildBank:OnGuildCashTransSuccessText()
...
function CRB_GuildBank:OnGuildBankWithdraw(guildOwner)
...
function CRB_GuildBank:OnBankTabBtnPermissions(wndHandler, wndControl)
	self:DrawTabPermissions()
end

function CRB_GuildBank:OnPermissionsResetBtn(wndHandler, wndControl)
	self:DrawTabPermissions()
end

function CRB_GuildBank:OnGuildRankChange() -- C++ Event
...
function CRB_GuildBank:OnPermissionsMoneyCashWindow(wndHandler, wndControl) -- PermissionsMoneyCashWindow
...
function CRB_GuildBank:OnPermissionsRepairCashWindow(wndHandler, wndControl) -- PermissionsRepairCashWindow
	self.tWndRefs.wndMain:FindChild("PermissionsSaveBtn"):Enable(true)
	if not wndHandler:GetAmount() or wndHandler:GetAmount() == 0 then
...
function CRB_GuildBank:OnPermissionsCurrentBtnLeftRight(wndHandler, wndControl)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:DrawTabPermissions()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:BuildPermissionIndividualTab(wndParent, guildOwner, bCanEditRanks, tCurrRankData, nBankTab)
...
function CRB_GuildBank:OnPermissionGridBtnWithdrawPlusSub(wndHandler, wndControl) -- PermissionGridBtnPlusWithdraw or PermissionGridBtnSubWithdraw
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnPermissionGridBtnVisibleDeposit(wndHandler, wndControl) -- PermissionGridIconVisible or PermissionGridIconDeposit
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnPermissionsSaveBtn(wndHandler, wndControl)
...
function CRB_GuildBank:OnBankTabBtnLog(wndHandler, wndControl)
...
function CRB_GuildBank:OnGuildBankLog(guildOwner, arLogs)
...
function CRB_GuildBank:OnBankLogTabCheck(wndHandler, wndControl)
...
function CRB_GuildBank:OnBankTabBtnMgmt()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnBankTabBtnMgmtAssist()
...
function CRB_GuildBank:OnLeaderOptionsEditBoxChanged(wndHandler, wndControl)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnLeaderOptionsTabRenameBtn(wndHandler, wndControl)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnGuildBankTabRename(guildOwner)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
...
function CRB_GuildBank:OnLeaderOptionsTabBuyBtn(wndHandler, wndControl)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
	local guildOwner = self.tWndRefs.wndMain:GetData()
...
function CRB_GuildBank:OnGuildBankTabCount()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:GetData() then
		return
	end
	local guildOwner = self.tWndRefs.wndMain:GetData()
...
function CRB_GuildBank:HelperEmptyMainBankScrollbar(bShow)
...
function CRB_GuildBank:HelperUpdateHeaderText(strNewHeader)
...
function CRB_GuildBank:DoFlashAnimation()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_GuildBank:HelperBuildItemTooltip(wndArg, itemCurrent)
...
function CRB_GuildBank:HelperLoadTabPerks(guildOwner)
	local tGuildPerks = guildOwner:GetPerks()
...
function CRB_GuildBank:HelperConvertToTime(nDays)
	tTimeData =
	{
		["name"]	= "",
		["count"]	= nil,
...
function CRB_GuildBank:OnGuildChange()
	for idx, guildCurr in pairs(GuildLib.GetGuilds()) do
		if guildCurr:GetType() == GuildLib.GuildType_Guild then
			return
		end
...
local CRB_GuildBankInst = CRB_GuildBank:new()
CRB_GuildBankInst:Init()
