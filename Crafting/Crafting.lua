...
function Crafting:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementRegister", {strName = Apollo.GetString("CBCrafting_Title")})
end
...
function Crafting:OnGenericEvent_StartCircuitCraft(idSchematic)
	...
	local strTooltip = ""
	if not self.tSchematicInfo.bIsUniversal then
		strTooltip = Apollo.GetString("CBCrafting_SlotPowerCore")
	end
...
function Crafting:CheckMaterials()
	...
	wndBlockerNoStation:Show(not CraftingLib.IsAtCraftingStation() and not (self.tSchematicInfo.bIsUniversal and CraftingLib.IsAtMasterCraftsman()))
	...
	elseif not bCurrentCraftStarted then
		...
		if self.tSchematicInfo.monMaxCraftingCost then
			...
			if GameLib.GetPlayerCurrency():GetAmount() < self.tSchematicInfo.monMaxCraftingCost:GetAmount() then
				...
				wndStartCraftBtn:FindChild("CashWindow"):SetTextColor("UI_WindowTextRed")
				wndPreviewBlocker:FindChild("Label"):SetTextColor("UI_BtnTextGreenDisabled")
			else
				wndStartCraftBtn:FindChild("CashWindow"):SetTextColor("white")
				wndStartCraftBtn:Enable(true)
				wndPreviewBlocker:FindChild("Label"):SetTextColor("UI_BtnTextGreenNormal")
			end
		else
	...
	elseif GameLib.GetPlayerCurrency():GetAmount() < wndCraftCost:GetAmount() then
		...
		wndCraftButton:Enable(false)
		wndCraftButton:FindChild("Label"):SetTextColor("UI_BtnTextGreenDisabled")
	elseif not self.tSchematicInfo.bIsUniversal and not self.tSocketInfo.itemPowerCore or not tCurrentCraft.tResult.bIsValidCraft then
		...
		wndCraftButton:FindChild("Label"):SetTextColor("UI_BtnTextGreenNormal")
...
function Crafting:UpdateStats()
	...
	if not (self.tSocketInfo.itemPowerCore or CraftingLib.GetSchematicInfo(tCraftingInfo.nSchematicId).bIsUniversal) or not tCraftingInfo then
		return
	end
...
function Crafting:UpdateOverchargeInfo()
	...
	self.wndMain:FindChild("FailPercentText"):SetText(bOverFailCap and ">" .. tCraftInfo.tResult.nFailCap .. "%" or tCraftInfo.tResult.nFailChance .. "%")
	
	local wndWarningText = self.wndMain:FindChild("WarningWindowText")
	wndWarningText:Show(not tCraftInfo.tResult.bIsValidCraft)
	if not tCraftInfo.tResult.bIsValidCraft then
		local strText = ""
		if bOverFailCap then
			strText = string.format("<P Font=\"CRB_InterfaceSmall_O\" Align=\"Center\" TextColor=\"UI_TextMetalBodyHighlight\">%s</P>", Apollo.GetString("CBCrafting_TooMuchChargeError"))
		elseif GameLib.GetEmptyInventorySlots() > 0 then
			strPopupText = string.format("<P Font=\"CRB_InterfaceSmall_O\" Align=\"Center\" TextColor=\"Reddish\">%s</P>", Apollo.GetString("CBCrafting_NoBagSpace"))
		end
		
		wndWarningText:SetAML(strText)
	end
	
	local wndCraftBtn = self.wndMain:FindChild("CraftButton")
	wndCraftBtn:Enable(tCraftInfo.tResult.bIsValidCraft)
	if tCraftInfo.tResult.bIsValidCraft then
		wndCraftBtn:FindChild("Label"):SetTextColor("UI_BtnTextGreenNormal")
	else
...
function Crafting:OnTutorialItemBtn(wndHandler, wndControl)
	...
	local wndTutorialPopup = self.wndMain:FindChild("TutorialPopup")
	wndTutorialPopup:SetAnchorOffsets(nRight, nBottom, nRight + wndTutorialPopup:GetWidth(), nBottom + wndTutorialPopup:GetHeight())
	wndTutorialPopup:FindChild("TutorialPopupText"):SetText(Apollo.GetString(ktTutorialText[wndHandler:GetData()]))
	wndTutorialPopup:Show(not wndTutorialPopup:IsShown())
end

function Crafting:OnTutorialPopupCloseBtn()
	self.wndMain:FindChild("TutorialPopup"):Show(false)
end
