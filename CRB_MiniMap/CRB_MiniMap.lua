-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_MiniMap
...
function CRB_MiniMap:CreateOverlayObjectTypes()
...
function CRB_MiniMap:BuildCustomMarkerInfo()
...
function CRB_MiniMap:Init()
	Apollo.RegisterAddon(self)
end

function CRB_MiniMap:OnLoad()
...
function CRB_MiniMap:OnDocumentReady()
...
function CRB_MiniMap:OnCharacterCreated()
...
function CRB_MiniMap:OnOptionsUpdated()
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnSave(eType)
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnRestore(eType, tSavedData)
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnChangeZoneName(oVar, strNewZone)
...
function CRB_MiniMap:UpdateZoneName(strZoneName)
...
function CRB_MiniMap:OnUnitPvpFlagsChanged(unitChanged)
...
function CRB_MiniMap:UpdatePvpFlag()
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnMenuBtn()
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnMenuBtnToggle(wndHandler, wndControl)
...
function CRB_MiniMap:OnMinusBtn()
	self.wndMiniMap:ZoomOut()
	Sound.Play(Sound.PlayUI15ZoomOutPhysical)
end

---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnPlusBtn()
	self.wndMiniMap:ZoomIn()
	Sound.Play(Sound.PlayUI14ZoomInPhysical)
end

function CRB_MiniMap:OnMapToggleBtn()
	Event_FireGenericEvent("ToggleZoneMap")
end

function CRB_MiniMap:OnMapGhostMode(bMode) -- Turn on/off the ghost mode notice
	self.wndMegaMapBtnOverlay:Show(bMode)
end

function CRB_MiniMap:OnToggleGhostModeMap() -- Turn on/off the ghost mode button (for key input toggle on and off)
	local bShow = not self.wndMegaMapBtnOverlay:IsShown()
	self.wndMegaMapBtnOverlay:Show(bShow)
end

---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnRotateMapCheck()
...
function CRB_MiniMap:OnRotateMapUncheck()
...
function CRB_MiniMap:OnMapPing(idUnit, tPos )
...
function CRB_MiniMap:OnPingTimer()
...
function CRB_MiniMap:OnMouseMove(wndHandler, wndControl, nX, nY)

end

function CRB_MiniMap:OnMapClick(wndHandler, wndControl, eButton, nX, nY, bDouble)

end

function CRB_MiniMap:OnMouseButtonUp(eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY)

end

---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnFailChallenge(tChallengeData)
	self:OnRemoveChallengeIcon(tChallengeData:GetId())
end

function CRB_MiniMap:OnRemoveChallengeIcon(chalOwner)
...
function CRB_MiniMap:OnAddChallengeIcon(chalOwner, strDescription, tPosition)
...
function CRB_MiniMap:OnFlashChallengeIcon(chalOwner, strDescription, fDuration, tPosition)
	if self.tChallengeObjects[chalOwner] ~= nil then
		self.wndMiniMap:RemoveObject(self.tChallengeObjects[chalOwner])
	end
...
function CRB_MiniMap:OnStopChallengeFlashIcon()
...
function CRB_MiniMap:OnPlayerPathMissionActivate(pmActivated)
...
function CRB_MiniMap:OnPlayerPathMissionDeactivate(pmDeactivated)
	self.wndMiniMap:RemoveObjectsByUserData(GameLib.CodeEnumMapOverlayType.PathObjective, pmDeactivated)
end

---------------------------------------------------------------------------------------------------

function CRB_MiniMap:ReloadPublicEvents()
...
function CRB_MiniMap:OnPublicEventUpdate(peUpdated)
...
function CRB_MiniMap:OnPublicEventEnd(peEnding)
...
function CRB_MiniMap:OnPublicEventStart(peStarting)
...
function CRB_MiniMap:OnPublicEventObjectiveUpdate(peoUpdated)
...
function CRB_MiniMap:OnPublicEventObjectiveEnd(peoUpdated)
	self.wndMiniMap:RemoveObjectsByUserData(self.eObjectTypePublicEvent, peoUpdated)
end

---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnCityDirectionMarked(tLocInfo)
...
function CRB_MiniMap:OnZoneMap_TimeOutCityDirectionEvent()
	if not self.wndMiniMap or not self.wndMiniMap:IsValid() then
		return
	end
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnQuestStateChanged()
...
function CRB_MiniMap:OnOneSecTimer()
...
function CRB_MiniMap:OnUnitCreated(unitNew)
...
function CRB_MiniMap:GetDefaultUnitInfo()
	local tInfo =
	{
		strIcon = "",
		strIconEdge = "MiniMapObjectEdge",
...
function CRB_MiniMap:UpdateHarvestableNodes()
...
function CRB_MiniMap:GetOrderedMarkerInfos(tMarkerStrings)
	local tMarkerInfos = {}
...
function CRB_MiniMap:HandleUnitCreated(unitNew)
...
function CRB_MiniMap:OnHazardShowMinimapUnit(idHazard, unitHazard, bIsBeneficial)
...
function CRB_MiniMap:OnHazardRemoveMinimapUnit(idHazard, unitHazard)
	if unitHazard == nil then
		return
	end
...
function CRB_MiniMap:OnUnitChanged(unitUpdated, eType)
...
function CRB_MiniMap:OnUnitDestroyed(unitDestroyed)
...
function CRB_MiniMap:OnGroupJoin()
...
function CRB_MiniMap:OnGroupAdd(strName)
	for idx = 2, GroupLib.GetMemberCount() do
		local tInfo = GroupLib.GetGroupMember(idx)
...
function CRB_MiniMap:OnGroupRemove(strName, eReason)
...
function CRB_MiniMap:OnGroupLeft(eReason)
	for idx, tMember in pairs(self.tGroupMembers) do -- remove all of the group objects
		self.wndMiniMap:RemoveObject(tMember.mapObject)
	end
...
function CRB_MiniMap:OnGroupUpdatePosition(arMembers)
...
function CRB_MiniMap:DrawGroupMembers()
	for idx = 2, GroupLib.GetMemberCount() do
...
function CRB_MiniMap:DrawGroupMember(tMember)
...
---------------------------------------------------------------------------------------------------
function CRB_MiniMap:OnGenerateTooltip(wndHandler, wndControl, eType, nX, nY)	
	local strTooltip = ""
...
function CRB_MiniMap:OnFriendshipAccountFriendsRecieved(tFriendAccountList)
	for idx, tFriend in pairs(tFriendAccountList) do
		self:OnRefreshRadar(FriendshipLib.GetUnitById(tFriend.nId))
	end
end

function CRB_MiniMap:OnFriendshipAdd(nFriendId)
	self:OnRefreshRadar(FriendshipLib.GetUnitById(nFriendId))
end

function CRB_MiniMap:OnFriendshipRemove(nFriendId)
	self:OnRefreshRadar(FriendshipLib.GetUnitById(nFriendId))
end
...
function CRB_MiniMap:OnFriendshipAccountFriendRemoved(nId)
	self:OnRefreshRadar()
end

function CRB_MiniMap:OnReputationChanged(tFaction)
	self:OnRefreshRadar()
end

function CRB_MiniMap:OnRefreshRadar(newUnit)
...
function CRB_MiniMap:OnMiniMapMouseEnter(wndHandler, wndControl)
...
function CRB_MiniMap:OnMiniMapMouseExit(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_MiniMap:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor ~= GameLib.CodeEnumTutorialAnchor.CRB_MiniMap then
		return
	end
...
function CRB_MiniMap:OnFilterOptionCheck(wndHandler, wndControl, eMouseButton)
...
function CRB_MiniMap:OnFilterOptionUncheck(wndHandler, wndControl, eMouseButton)
	local eCategory = wndControl:GetData()
	if eCategory == nil then
		return
	end
...
function CRB_MiniMap:RehideAllToggledIcons()
...
function CRB_MiniMap:GetToggledIconState(eSearchType)
...
---------------------------------------------------------------------------------------------------
-- CRB_MiniMap instance
---------------------------------------------------------------------------------------------------
local CRB_MiniMapInst = CRB_MiniMap:new()
CRB_MiniMapInst:Init()
