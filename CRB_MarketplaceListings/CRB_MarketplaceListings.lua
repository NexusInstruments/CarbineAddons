-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_MarketplaceListings
...
function CRB_MarketplaceListings:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_MarketplaceListings:Init()
    Apollo.RegisterAddon(self)
end

function CRB_MarketplaceListings:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MarketplaceListings.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_MarketplaceListings:OnDocumentReady()
...
function CRB_MarketplaceListings:OnInterfaceMenuListHasLoaded()
...
function CRB_MarketplaceListings:OnToggle()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
	else
		self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_MarketplaceListingsForm", nil, self)
...
function CRB_MarketplaceListings:OnDestroy()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
		Apollo.StopTimer("MarketplaceUpdateTimer")
...
function CRB_MarketplaceListings:RequestData()
...
function CRB_MarketplaceListings:RedrawData()
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_MarketplaceListings:OnOwnedItemAuctions(tAuctions)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_MarketplaceListings:OnOwnedCommodityOrders(tOrders)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_MarketplaceListings:OnCREDDExchangeInfoResults(arMarketStats, arOrders)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_MarketplaceListings:SharedDrawMain()
...
function CRB_MarketplaceListings:OnHeaderItemToggle(wndHandler, wndControl)
	self:SharedDrawMain()
end
...
function CRB_MarketplaceListings:BuildAuctionOrder(nIdx, aucCurrent, wndParent)
...
function CRB_MarketplaceListings:BuildCommodityOrder(nIdx, aucCurrent, wndParent)
	local tItem = aucCurrent:GetItem()
...
function CRB_MarketplaceListings:BuildCreddOrder(nIdx, aucCurrent, wndParent)
...
function CRB_MarketplaceListings:OnCancelBtn(wndHandler, wndControl)
...
function CRB_MarketplaceListings:OnAuctionCancelConfirmBtn(wndHandler, wndControl)
	local aucCurrent = wndHandler:GetData()
...
function CRB_MarketplaceListings:OnCommodityCancelConfirmBtn(wndHandler, wndControl)
	local aucCurrent = wndHandler:GetData()
...
function CRB_MarketplaceListings:OnCreddCancelConfirmBtn(wndHandler, wndControl)
	local aucCurrent = wndHandler:GetData()
	if not aucCurrent or not aucCurrent:IsPosted() then
		return
	end
...
function CRB_MarketplaceListings:OnCommodityItemSmallMouseEnter(wndHandler, wndControl)
...
function CRB_MarketplaceListings:OnCommodityItemSmallMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:FindChild("CommodityCancelBtn") then
...
function CRB_MarketplaceListings:OnCommodityAuctionRemoved(eAuctionEventType, oRemoved)
	-- TODO
...
function CRB_MarketplaceListings:OnCommodityAuctionUpdated(oUpdated)
	if self.tOrders ~= nil then
		local bFound = false
		for nIdx, tCurrOrder in ipairs(self.tOrders) do
...
function CRB_MarketplaceListings:OnPostCommodityOrderResult(eAuctionResult, oAdded)
...
function CRB_MarketplaceListings:OnItemAuctionRemoved(aucRemoved)
	if self.tAuctions ~= nil then
...
function CRB_MarketplaceListings:OnItemCancelResult(eAuctionResult, aucRemoved)
...
function CRB_MarketplaceListings:OnItemAuctionUpdated(aucUpdated)
	if self.tAuctions ~= nil then
		local bFound = false
		for nIdx, aucCurrent in ipairs(self.tAuctions) do
...
function CRB_MarketplaceListings:OnItemAuctionResult(eAuctionResult, aucAdded)
	if eAuctionResult ~= MarketplaceLib.AuctionPostResult.Ok then
		return
	end
...
function CRB_MarketplaceListings:OnItemListingClose(wndHandler, wndControl)
...
function CRB_MarketplaceListings:HelperFormatTimeString(oExpirationTime)
	local strResult = ""
...
function CRB_MarketplaceListings:FactoryProduce(wndParent, strFormName, tObject) -- Using AuctionObjects
...
local CRB_MarketplaceListingsInst = CRB_MarketplaceListings:new()
CRB_MarketplaceListingsInst:Init()
