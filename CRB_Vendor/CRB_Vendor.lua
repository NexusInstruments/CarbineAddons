...
{
	[Item.CodeEnumItemUpdateReason.CRB_Vendor] 		= Apollo.GetString("Vendor_Bought"),
...
function CRB_Vendor:Init()
	Apollo.RegisterAddon(self, false, "", {"Util"})
end

function CRB_Vendor:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Vendor.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Vendor:OnDocumentReady()
...
function CRB_Vendor:OnUpdateInventory()
...
function CRB_Vendor:OnVendorItemsUpdated()
	self:Redraw()
end

function CRB_Vendor:OnBuybackItemsUpdated()
	self:RefreshBuyBackTab()
	self:Redraw()
end

function CRB_Vendor:OnPlayerCurrencyChanged()
...
function CRB_Vendor:RedrawFully()
...
function CRB_Vendor:HelperTabManagement(tVendorList, bChanged)
...
function CRB_Vendor:HelperResetToBuyTab()
	self.wndItemContainer:DestroyChildren()
	self:DisableBuyButton()
	self.wndVendor:FindChild(kstrTabSell):SetCheck(false)
	self.wndVendor:FindChild(kstrTabBuyback):SetCheck(false)
...
function CRB_Vendor:OnBlockBuyTimer()
	self.wndInteractBlocker:Show(false, false)
end

function CRB_Vendor:DrawHeaderAndItems(tVendorList, bChanged)
...
function CRB_Vendor:SizeHeader(wndHeader)
...
function CRB_Vendor:DrawListItems(wndParent, tItems)
...
function CRB_Vendor:DrawListItem(wndCurr, tCurrItem)
...
function CRB_Vendor:EnableBuyButton(tData)
...
function CRB_Vendor:DisableBuyButton(bDontClear)
	self:HideRestockingFee()
...
function CRB_Vendor:DisableAmountValue()
...
function CRB_Vendor:EnableAmountValue(tItemData)
	local wndAmountValue = self.wndVendor:FindChild("AmountValue")
	local wndAmountFrame= self.wndVendor:FindChild("AmountFrame")
	local wndAmountBlocker= self.wndVendor:FindChild("AmountBlocker")
...
function CRB_Vendor:OnSpinnerChanged(wndHandler, wndControl)
...
function CRB_Vendor:OnVendorListItemCheck(wndHandler, wndControl) -- TODO REFACTOR
...
function CRB_Vendor:OnVendorListItemUncheck(wndHandler, wndControl) -- TODO REFACTOR
	if not wndHandler or not wndHandler:GetData() then
		return
	end
...
function CRB_Vendor:OnVendorListItemMouseDown(wndHandler, wndControl, eMouseButton, nPosX, nPosY, bDoubleClick)
...
function CRB_Vendor:SelectNextItemInLine(tItem)
...
function CRB_Vendor:FocusOnVendorListItem(tVendorItem)
...
function CRB_Vendor:OnItemAdded(itemBought, nCount, eReason)
...
function CRB_Vendor:OnItemRemoved(itemSold, nCount, eReason)
	if self.wndVendor and self.wndVendor:IsShown() and ktVendorRespondEvent[eReason] then
...
function CRB_Vendor:OnGenericError(eError, strMessage)
...
function CRB_Vendor:OnItemDurabilityUpdate(itemCurr, nOldValue)
...
function CRB_Vendor:ShowAlertMessageContainer(strMessage, bFailed)
...
function CRB_Vendor:HideRestockingFee()
...
function CRB_Vendor:ProcessingRestockingFee(tItemData)
...
function CRB_Vendor:OnAlertMessageTimer()
	Apollo.StopTimer("AlertMessageTimer")
	self.wndVendor:FindChild("AlertMessageContainer"):Show(false)
end

function CRB_Vendor:RefreshBuyBackTab()
...
function CRB_Vendor:RefreshRepairTab()
...
function CRB_Vendor:UpdateVendorItems() -- TODO: Old code
	if not self.wndVendor:GetData() then -- Get Data should be the CRB_Vendor Unit
		return
	end
...
---------------------------------------------------------------------------------------------------
function CRB_Vendor:UpdateSellItems()
	local unitPlayer = GameLib.GetPlayerUnit()
	
	if not self.wndVendor:GetData() or not unitPlayer then -- Get Data should be the CRB_Vendor Unit
		return
	end
...
---------------------------------------------------------------------------------------------------
function CRB_Vendor:UpdateBuybackItems()
	if not self.wndVendor:GetData() then -- Get Data should be the CRB_Vendor Unit
		return
	end
...
---------------------------------------------------------------------------------------------------
function CRB_Vendor:UpdateRepairableItems()
	if not self.wndVendor:GetData() then -- Get Data should be the CRB_Vendor Unit
		return
	end
...
function CRB_Vendor:OnWindowClosed()
	Event_CancelVending()
end

function CRB_Vendor:OnCloseBtn()
	self.wndVendor:Close()
end

function CRB_Vendor:OnCloseVendorWindow()
	self.wndVendor:Close()
end
...
function CRB_Vendor:OnGuildChange() -- Catch All method to validate Guild Repair
...
function CRB_Vendor:OnGuildRepairBtn(wndHandler, wndControl)
...
function CRB_Vendor:OnBuy(wndHandler, wndControl)
...
function CRB_Vendor:FinalizeBuy(tItemData)
	local nAmount = 0
	local wndAmountValue = self.wndVendor:FindChild("AmountValue")
	if wndAmountValue then
...
function CRB_Vendor:RepairAllHelper()
...
function CRB_Vendor:OnTabBtn(wndHandler, wndControl)
...
function CRB_Vendor:SetBuyButtonText()
	local strCaption = ""
	if self.wndVendor:FindChild(kstrTabBuy):IsChecked() then
...
function CRB_Vendor:ArrangeGroups(tItemList, tGroups)
	local tNewList = {
		tOther =
		{
...
function CRB_Vendor:ItemToVendorSellItem(itemCurr, nGroup)
...
function CRB_Vendor:OnVendorListItemGenerateTooltip(wndControl, wndHandler) -- wndHandler is VendorListItemIcon
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Vendor:OnItemPriceTooltip(wndHandler, wndControl, eToolTipType, monA, monB)
...
-- Deep table equality
function CRB_Vendor:TableEquals(tData1, tData2)
...
function CRB_Vendor:HelperRecipeAlreadyKnown(tCurrItem)
...
function CRB_Vendor:HelperIsTooExpensive(tCurrItem)
...
function CRB_Vendor:HelperPrereqFailed(tCurrItem)
	return tCurrItem.itemData and tCurrItem.itemData:IsEquippable() and not tCurrItem.itemData:CanEquip()
end

function CRB_Vendor:HelperPrereqBuyFailed(tCurrItem)
...
function CRB_Vendor:FactoryCacheProduce(wndParent, strFormName, strKey)
...
function CRB_Vendor:OnHeaderCheck(wndHandler, wndControl, eMouseButton)
...
function CRB_Vendor:OnHeaderUncheck(wndHandler, wndControl, eMouseButton)
	local wndParent = wndControl:GetParent()
...
function CRB_Vendor:HelperStringMoneyConvert(nInCopper)
	local strResult = ""
...
-- CRB_Vendor instance
---------------------------------------------------------------------------------------------------
local CRB_VendorInst = CRB_Vendor:new()
CRB_VendorInst:Init()
