-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_GuildRegistration
...
function CRB_GuildRegistration:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_GuildRegistration:Init()
    Apollo.RegisterAddon(self)
end

function CRB_GuildRegistration:OnLoad() -- TODO: Only load when needed
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_GuildRegistration.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_GuildRegistration:OnDocumentReady()
	...
	Apollo.RegisterEventHandler("GuildRegistrarOpen", 		"OnCRB_GuildRegistrationOn", self)
	Apollo.RegisterEventHandler("GuildRegistrarClose",		"OnCancel", self)

    -- load our forms
    self.wndMain 			= Apollo.LoadForm(self.xmlDoc, "CRB_GuildRegistrationForm", nil, self)
...
function CRB_GuildRegistration:ResetOptions()
	for idx = 1, 3 do
...
function CRB_GuildRegistration:OnNameChanging(wndHandler, wndControl)
	self.tCreate.strName = self.wndGuildName:GetText()
	self:UpdateOptions()
end

function CRB_GuildRegistration:OnOptionChanging(wndHandler, wndControl)
...
function CRB_GuildRegistration:UpdateOptions()
...
function CRB_GuildRegistration:HelperCheckForEmptyString(strText) -- make sure there's a valid string
	local strFirstChar
...
function CRB_GuildRegistration:HelperClearFocus(wndHandler, wndControl)
	for idx = 1, 3 do
...
function CRB_GuildRegistration:SetDefaultHolomark()
...
-----------------------------------------------------------------------------------------------
-- CRB_GuildRegistrationForm Functions
-----------------------------------------------------------------------------------------------
function CRB_GuildRegistration:FakeRadio(wndNewOption, wndSavedOption)
...
function CRB_GuildRegistration:UseDefaultTitleBtn(wndHandler, wndControl) -- reset an option to its default
	local nRank = wndControl:GetData()
...
function CRB_GuildRegistration:OnRegisterBtn(wndHandler, wndControl)
...
function CRB_GuildRegistration:OnCancel(wndHandler, wndControl)
...
function CRB_GuildRegistration:OnGuildResultInterceptResponse( guildCurr, eGuildType, eResult, wndRegistration, strAlertMessage )
...
function CRB_GuildRegistration:OnSuccessfulMessageTimer()
	self:OnCancel()
end


function CRB_GuildRegistration:OnErrorMessageTimer()
	self.wndAlert:Show(false)
	self.wndRegisterBtn:Enable(true) -- safe to assume since it was clicked once
end

function CRB_GuildRegistration:OnPlayerCurrencyChanged()
end

-----------------------------------------------------------------------------------------------
-- Holomark Functions
-----------------------------------------------------------------------------------------------
function CRB_GuildRegistration:InitializeHolomarkParts()
...
function CRB_GuildRegistration:OnHolomarkPartCheck1( wndHandler, wndControl, eMouseButton )
...
function CRB_GuildRegistration:OnHolomarkPartUncheck1( wndHandler, wndControl, eMouseButton )
	self.wndMain:FindChild("HolomarkBackgroundPartWindow"):Show(false)
end

function CRB_GuildRegistration:OnHolomarkPartCheck2( wndHandler, wndControl, eMouseButton )
...
function CRB_GuildRegistration:OnHolomarkPartUncheck2( wndHandler, wndControl, eMouseButton )
	self.wndMain:FindChild("HolomarkForegroundPartWindow"):Show(false)
end

function CRB_GuildRegistration:FillHolomarkPartList( strPartType )
	local wndList = nil
	local tPartList = nil
...
function CRB_GuildRegistration:AddHolomarkPartItem(wndList, index, tPart)
...
function CRB_GuildRegistration:OnHolomarkPartSelectionClosed( wndHandler, wndControl )
...
function CRB_GuildRegistration:OnHolomarkPartItemSelected(wndHandler, wndControl)
...
-- Forced Rename Code
-----------------------------------------------------------------------------------------------
function CRB_GuildRegistration:OnGuildRegistration_CheckForcedRename()
...
function CRB_GuildRegistration:OnRenameSocialCancel()
...
function CRB_GuildRegistration:OnRenameEditBoxChanged(wndHandler, wndControl)
...
function CRB_GuildRegistration:OnRenameSocialConfirm(wndHandler, wndControl)
...
local CRB_GuildRegistrationInst = CRB_GuildRegistration:new()
CRB_GuildRegistrationInst:Init()
