-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_HUDAlerts
...
function CRB_HUDAlerts:Init()
	Apollo.RegisterAddon(self)
end

function CRB_HUDAlerts:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_HUDAlerts.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_HUDAlerts:OnDocumentReady()
...
function CRB_HUDAlerts:OnHUDAlerts_DelayedInitialize() -- TODO HACK: This needs to dodge QuestLib not loading and the DatachronCallCleared GenericEvent
...
function CRB_HUDAlerts:OnActivateCCStateStun()
	self.wndAlertContainer:Show(false)
end

function CRB_HUDAlerts:OnRemoveCCStateStun()
	self.wndAlertContainer:Show(true)
end

function CRB_HUDAlerts:OnHoverGlowMouseEnter(wndHandler, wndControl)
...
function CRB_HUDAlerts:OnHoverGlowMouseExit(wndHandler, wndControl)
	if wndHandler:FindChild("AlertPopout") then
...
function CRB_HUDAlerts:ShowAndInitializeAlert(wndAlert)
	self.wndAlertContainer:ArrangeChildrenHorz(0)
	wndAlert:Show(true)
...
function CRB_HUDAlerts:OnUpdateInventory()
...
function CRB_HUDAlerts:OnCanVacuumChanged(bCanVacuum)
...
function CRB_HUDAlerts:OnDatachronCallIncoming()
...
function CRB_HUDAlerts:OnDatachronCallMissed() -- Also from Initialize
	local strKeybind = GameLib.GetKeyBinding("Communicator")
	self.wndCallAlert:FindChild("AlertItemKeybindText"):SetText(strKeybind)
	self.wndCallAlert:FindChild("AlertItemKeybind"):Show(strKeybind ~= Apollo.GetString("HUDAlert_Unbound"))
...
function CRB_HUDAlerts:OnHUDAlerts_DatachronCallExpired()
	self.wndCallAlert:Show(false)
	self.wndQuestCompleteAlert:Show(false)
	self.wndAlertContainer:ArrangeChildrenHorz(0)
end

function CRB_HUDAlerts:OnDatachronCallCleared()
	self.wndCallAlert:Show(false)
	self.wndAlertContainer:ArrangeChildrenHorz(0)
end

function CRB_HUDAlerts:OnChallengeUnlocked(tArgChallenge)
...
function CRB_HUDAlerts:ShowChallengeHUDAlert(tChallenge)
	self:ShowAndInitializeAlert(self.wndChallengeAlert)
	Apollo.CreateTimer("HUDAlerts_ChallengeTimerExpired", 60.0, false)
end

function CRB_HUDAlerts:OnHUDAlerts_ChallengeTimerExpired()
...
function CRB_HUDAlerts:DrawQuestCompleteAlert(tQuestsTracked)
	local strKeybind = GameLib.GetKeyBinding("Communicator")
...
function CRB_HUDAlerts:RebuildQuestList()
...
function CRB_HUDAlerts:OnDatacubeUpdated(id, bIsVolume)
	if not id then
		return
	end
...
function CRB_HUDAlerts:DrawDatacubePlayback(tDatacube) -- Also from eveents
	if not tDatacube then
		return
	end
...
function CRB_HUDAlerts:ShowDurabilityAlert() -- Also from events
...
function CRB_HUDAlerts:OnHUDAlerts_HideDurabilityTimer()
	Apollo.StopTimer("HUDAlerts_HideDurabilityTimer")
	self.bHideDurabilityTimer = false
	self:ShowDurabilityAlert()
end
...
function CRB_HUDAlerts:OnChallengeAlertBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_HUDAlerts:OnDurabilityAlertBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_HUDAlerts:OnDatacubeAlertBtn(wndHandler, wndControl, eMouseButton) -- wndHandler is "DatacubeAlertBtn" and its data is a Datacube
	if eMouseButton == GameLib.CodeEnumInputMouse.Right then
...
function CRB_HUDAlerts:OnHUDAlerts_PlayDatacubeTrack()
	Apollo.StopTimer("HUDAlerts_PlayDatacubeTrack")
...
function CRB_HUDAlerts:OnStopPlayingDatacube() -- Lots of events route here, including the natural playing end and manual end from collections
	Apollo.StopTimer("HUDAlerts_MaxPlayDatacubeTrackTime")
	self.wndDatacubeAlert:Show(false)
	self.wndAlertContainer:ArrangeChildrenHorz(0)
end

function CRB_HUDAlerts:OnCallAlertBtn(wndHandler, wndControl, eMouseButton)
	if eMouseButton == GameLib.CodeEnumInputMouse.Right then
...
function CRB_HUDAlerts:OnQuestCompleteTurnIn(wndHandler, wndControl) -- wndHandler is "QuestCompleteItem" or "QuestCompleteBtn" and its data is tCurrQuest
...
---------------------------------------------------------------------------------------------------
function CRB_HUDAlerts:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
local CRB_HUDAlertsInst = CRB_HUDAlerts:new()
CRB_HUDAlertsInst:Init()
