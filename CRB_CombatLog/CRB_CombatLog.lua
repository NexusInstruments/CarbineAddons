-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_CombatLog
...
function CRB_CombatLog:Init()
	Apollo.RegisterAddon(self, true, Apollo.GetString("CombatLogOptions_CombatLogBtn"))
end

function CRB_CombatLog:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CombatLog.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_CombatLog:OnDocumentReady()
...
function CRB_CombatLog:PostOnChannel(strResult)
...
function CRB_CombatLog:OnCombatLogDamage(tEventArgs)
...
function CRB_CombatLog:OnCombatLogFallingDamage(tEventArgs)
...
function CRB_CombatLog:OnCombatLogDeflect(tEventArgs)
...
function CRB_CombatLog:OnCombatLogImmunity(tEventArgs)
	local tCastInfo = self:HelperCasterTargetSpell(tEventArgs, true, true, true)
	tCastInfo.strSpellName = string.format("<T Font=\"%s\">%s</T>", kstrFontBold, tCastInfo.strSpellName)
...
function CRB_CombatLog:OnCombatLogDispel(tEventArgs)
	local tCastInfo = self:HelperCasterTargetSpell(tEventArgs, true, true, true)
	tCastInfo.strSpellName = string.format("<T Font=\"%s\">%s</T>", kstrFontBold, tCastInfo.strSpellName)
	local strResult = String_GetWeaselString(Apollo.GetString("CombatLog_BaseSkillUse"), tCastInfo.strCaster, tCastInfo.strSpellName, tCastInfo.strTarget)
...
function CRB_CombatLog:OnCombatLogHeal(tEventArgs)
...
function CRB_CombatLog:OnCombatLogModifyInterruptArmor(tEventArgs)
...
function CRB_CombatLog:OnCombatLogAbsorption(tEventArgs)
	local tCastInfo = self:HelperCasterTargetSpell(tEventArgs, true, true, true)
...
function CRB_CombatLog:OnCombatLogVitalModifier(tEventArgs)
...
function CRB_CombatLog:OnCombatLogCCState(tEventArgs)
...
-- Special
-----------------------------------------------------------------------------------------------
function CRB_CombatLog:OnCombatLogLifeSteal(tEventArgs)
...
function CRB_CombatLog:OnCombatLogTransference(tEventArgs)
	if not self.unitPlayer then
		self.unitPlayer = GameLib.GetControlledUnit()
	end
...
function CRB_CombatLog:OnCombatLogInterrupted(tEventArgs)
...
function CRB_CombatLog:OnCombatLogKillStreak(tEventArgs)
...
function CRB_CombatLog:OnCombatLogKillPVP(tEventArgs)
	local tCastInfo = self:HelperCasterTargetSpell(tEventArgs, true, false)
...
function CRB_CombatLog:OnCombatLogCCStateBreak(tEventArgs)
...
function CRB_CombatLog:OnCombatLogDelayDeath(tEventArgs)
...
function CRB_CombatLog:OnCombatLogDeath(tEventArgs)
	self:PostOnChannel(string.format("<P TextColor=\"%s\">%s</P>", kstrStateColor, Apollo.GetString("CombatLog_Death")))
end

function CRB_CombatLog:OnCombatLogStealth(tEventArgs)
	local strResult = ""
...
function CRB_CombatLog:OnCombatLogMount(tEventArgs)
	local strResult = ""
...
function CRB_CombatLog:OnCombatLogPet(tEventArgs)
	local strResult = ""
...
function CRB_CombatLog:OnCombatLogExperience(tEventArgs)
...
function CRB_CombatLog:OnCombatLogEndGameCurrencies(tEventArgs)
...
function CRB_CombatLog:OnCombatLogElderPointsLimitReached(tEventArgs)
	local strResult = String_GetWeaselString(Apollo.GetString("CombatLog_ElderPointLimitReached"))
	self:PostOnChannel(string.format("<P TextColor=\"%s\">%s</P>", kstrColorCombatLogXP, strResult))
end


function CRB_CombatLog:OnCombatLogDurabilityLoss(tEventArgs)
	if not self.unitPlayer then
		self.unitPlayer = GameLib.GetControlledUnit()
	end
...
function CRB_CombatLog:OnPathExperienceGained(nAmount, strText)
	if nAmount > 0 then
...
function CRB_CombatLog:OnFactionFloater(unitTarget, pstrMessage, nAmount, strFactionName, nFactionId) -- Reputation Floater
	if nAmount > 0 then
...
function CRB_CombatLog:OnPetStatusUpdated()
	self.tPetUnits = GameLib.GetPlayerPets()
end

function CRB_CombatLog:OnChangeWorld()
	self.unitPlayer = GameLib.GetControlledUnit()
end

function CRB_CombatLog:OnPlayerEquippedItemChanged(nEquippedSlot, itemNew, itemOld)
	local strResult = ""
...
function CRB_CombatLog:HelperCasterTargetSpell(tEventArgs, bTarget, bSpell, bColor)
	local tInfo =
	{
		strCaster = nil,
		strTarget = nil,
...
function CRB_CombatLog:HelperGetNameElseUnknown(nArg)
...
function CRB_CombatLog:HelperDamageColor(nArg)
...
function CRB_CombatLog:HelperPickColor(tEventArgs)
...
function CRB_CombatLog:OnSave(eType)
...
function CRB_CombatLog:OnRestore(eType, tSavedData)
...
function CRB_CombatLog:OnConfigure()
...
function CRB_CombatLog:InitOptions()
	self.wndOptions = Apollo.LoadForm(self.xmlDoc, "CRB_CombatLogForm", nil, self)
...
function CRB_CombatLog:OnMappedOptionsCheckbox(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
	Apollo.SetConsoleVariable(tData.consoleVar, not wndControl:IsChecked())
end

function CRB_CombatLog:OnCancel(wndHandler, wndControl, eMouseButton)
	self.wndOptions:Show(false)
end

function CRB_CombatLog:OnOK(wndHandler, wndControl, eMouseButton)
	self.wndOptions:Show(false)
end

local CRB_CombatLogInstance = CRB_CombatLog:new()
CRB_CombatLog:Init()
