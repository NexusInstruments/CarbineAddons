-------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ZoneMap
...
function CRB_ZoneMap:CreateOverlayObjectTypes()
...
function CRB_ZoneMap:CreatePOIIcons()
	self.tPOITypes =
	{
...
function CRB_ZoneMap:GetAllUnitTypes()
	local tUnitTypes =
	{
...
function CRB_ZoneMap:BuildCustomMarkerInfo()
...
function CRB_ZoneMap:BuildShownTypesArrays()
...
function CRB_ZoneMap:Init()
	Apollo.RegisterAddon(self)
end

function CRB_ZoneMap:OnSave(eType)
...
function CRB_ZoneMap:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function CRB_ZoneMap:OnLoad()
	Apollo.RegisterEventHandler("UnitCreated", 							"OnPreloadUnitCreated", self)

	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ZoneMapForms.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)

	Apollo.RegisterEventHandler("InterfaceMenuListHasLoaded", "OnInterfaceMenuListHasLoaded", self)
end

function CRB_ZoneMap:OnPreloadUnitCreated(unitNew)
	self.arPreloadUnits[unitNew:GetId()] = unitNew
end

function CRB_ZoneMap:OnDocumentReady()
	...
	self.wndZoneMap 			= self.wndMain:FindChild("CRB_ZoneMap")
...
function CRB_ZoneMap:OnInterfaceMenuListHasLoaded()
...
function CRB_ZoneMap:OnOptionsUpdated()
...
function CRB_ZoneMap:CreateUnitsFromPreload()
...
function CRB_ZoneMap:ToggleWindow()
...
function CRB_ZoneMap:OnToggleGhostModeMap() -- for keyboard input turning ghost mode map on/off
	local bShow = not self.wndZoneMap:IsShowingGhostWindow()
	self.wndZoneMap:SetGhostWindow(bShow)
end

function CRB_ZoneMap:UpdateCurrentZone()
...
function CRB_ZoneMap:OnWindowMove(wndHandler, wndControl)
...
function CRB_ZoneMap:OnZoneMapChange(wndHandler, wndControl, strContinent, idZone) -- Map changed from clicking in continent view
	self.wndMain:FindChild("ZoneComplexToggle"):SetData(idZone) -- No need to set Continent since it has to be the current one
end

function CRB_ZoneMap:OnZoneComplexListBtn(wndHandler, wndControl) -- ZoneComplexListBtn
...
function CRB_ZoneMap:OnContinentNormalBtn(wndHandler, wndControl)
...
function CRB_ZoneMap:OnContinentCustomBtn(wndHandler, wndControl)
...
function CRB_ZoneMap:OnShowLoc(tLocInfo) -- Also from City Directions
...
function CRB_ZoneMap:OnWorldLocTimer()
	self.wndMain:ToFront()
end
...
function CRB_ZoneMap:OnCloseBtn()
	self.wndMain:Show(false)
	self.wndMapControlPanel:Show(false)
	self.wndMain:FindChild("ZoneComplexList"):Show(false)
	self.eDisplayMode = self.wndZoneMap:GetDisplayMode()
...
function CRB_ZoneMap:OnWindowClosed()
...
function CRB_ZoneMap:OnWorldChanged()
...
function CRB_ZoneMap:ToggleGhostModeOn()
	self.wndZoneMap:SetGhostWindow(true)
	Event_FireGenericEvent("MapGhostMode", true)
	self:ToggleWindow()
end

function CRB_ZoneMap:OnGhostSlider(wndHandler, wndControl)
...
function CRB_ZoneMap:OnGhostModeOptionOK()
	self.wndGhostOptionPanel:Show(false)
end

function CRB_ZoneMap:OnGhostModeOptionTest()
	self.wndZoneMap:SetGhostWindow(true)
	Event_FireGenericEvent("MapGhostMode", true)
	self:ToggleWindow()
end

function CRB_ZoneMap:OnToggleControlsOn(wndHandler, wndControl)
...
function CRB_ZoneMap:OnToggleControlsOff(wndHandler, wndControl)
...
function CRB_ZoneMap:OnResizeOptionsPane()
...
function CRB_ZoneMap:SetControls() -- runs off timer, sets the controls to reflect the map's display
...
function CRB_ZoneMap:OnZoneChanged()
	Event_FireGenericEvent("GenericEvent_ZoneMap_ZoneChanged", self.idCurrentZone)
	self:ReloadNemesisRegions()
end

function CRB_ZoneMap:HelperCheckAndBuildSubzones(tZoneInfo, eZoomLevel) -- This repeatedly calls on a timer
...
function CRB_ZoneMap:HelperBuildZoneDropdown(idContinent) -- This only calls on button events, like picking from a dropdown menu
...
function CRB_ZoneMap:OnZoomChange()
	local eZoomLevel = self.wndZoneMap:GetDisplayMode()
...
function CRB_ZoneMap:OnMouseScroll(eDisplayMode)
	self:SetControls()
	self:OnZoomChange()
end

function CRB_ZoneMap:OnZoomIn()
...
function CRB_ZoneMap:OnZoomOut()
	local eZoomLevel = self.wndZoneMap:GetDisplayMode() -- 1: Panning, 2: Scaled, 3: Continent
	if eZoomLevel == ZoneMapWindow.CodeEnumDisplayMode.SuperPanning then -- Super Panning
		self.wndZoneMap:SetDisplayMode(ZoneMapWindow.CodeEnumDisplayMode.Panning)
	elseif eZoomLevel == ZoneMapWindow.CodeEnumDisplayMode.Panning then -- Panning
...
function CRB_ZoneMap:OnReturnBtn(wndHandler, wndControl)
...
function CRB_ZoneMap:OnQuestStateChanged()
...
function CRB_ZoneMap:AddQuestIndicators()
...
function CRB_ZoneMap:GetMissionTooltip(pmCurrent)
...
function CRB_ZoneMap:GetPublicEventTooltip(peEvent)
...
function CRB_ZoneMap:GetChallengeTooltip(oChallenge)
...
function CRB_ZoneMap:GetHexGroupTooltip(oHexGroup)
...
function CRB_ZoneMap:GetNemesisRegionTooltip(oNemesisRegion)
...
function CRB_ZoneMap:OnGenerateTooltip(wndHandler, wndControl, eType, nX, nY)
	local eZoomLevel = self.wndZoneMap:GetDisplayMode()
...
function CRB_ZoneMap:OnHazardShowMinimapUnit(idHazard, unitHazard, bIsBeneficial)
...
function CRB_ZoneMap:OnHazardRemoveMinimapUnit(idHazard, unitHazard)
	if unitHazard == nil then
		return
	end
...
function CRB_ZoneMap:OnUpdateHexGroup(hexGroup)
...
function CRB_ZoneMap:OnPlayerIndicatorUpdated()
	self:SetControls()
end

function CRB_ZoneMap:OnSubZoneChanged()
...
function CRB_ZoneMap:OnToggleLabels(wndHandler, wndControl)
...
function CRB_ZoneMap:OnTerrainHexCheck(wndHandler, wndControl)
	wndHandler:FindChild("Label"):SetTextColor(ApolloColor.new("UI_BtnTextGoldListNormal"))
	Apollo.SetConsoleVariable("draw.hexGrid", true)
end

function CRB_ZoneMap:OnTerrainHexUncheck(wndHandler, wndControl)
	wndHandler:FindChild("Label"):SetTextColor(ApolloColor.new("UI_TextMetalGoldHighlight"))
	Apollo.SetConsoleVariable("draw.hexGrid", false)
end

function CRB_ZoneMap:SetTypeVisibility(eToggledType, bVisible)
	local eZoomLevel = self.wndZoneMap:GetDisplayMode()
	if bVisible then
		for idx, eType in pairs(self.arAllowedTypesSuperPanning) do
...
function CRB_ZoneMap:OnMarkerBtnCheck(wndHandler, wndControl)
...
function CRB_ZoneMap:OnMarkerBtnUncheck(wndHandler, wndControl)
	local eType = wndHandler:GetData()
	if eType == ktMarkerCategories.QuestNPCs then
...
function CRB_ZoneMap:OnMissionsCheck(wndHandler, wndControl)
...
function CRB_ZoneMap:OnChallengesCheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function CRB_ZoneMap:OnPublicEventsCheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function CRB_ZoneMap:ReloadNemesisRegions()
...
function CRB_ZoneMap:OnMissionsUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function CRB_ZoneMap:OnChallengesUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function CRB_ZoneMap:OnPublicEventsUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function CRB_ZoneMap:OnNemesisRegionsUncheck(wndHandler, wndControl)
	if wndControl ~= nil then  -- nil if coming from map change
...
function CRB_ZoneMap:RehideAllToggledIcons()
...
function CRB_ZoneMap:OnUnitChanged(unitUpdated, eType)
...
function CRB_ZoneMap:OnUnitDestroyed(unitDead)
...
function CRB_ZoneMap:IsTypeCurrentlyHidden(objectType)
	local eZoomLevel = self.wndZoneMap:GetDisplayMode()
	if eZoomLevel == ZoneMapWindow.CodeEnumDisplayMode.SuperPanning then
...
function CRB_ZoneMap:GetDefaultUnitInfo()
	local tInfo =
	{
		strIcon 		= "",
		strIconEdge 	= "",
...
function CRB_ZoneMap:GetOrderedMarkerInfos(tMarkerStrings)
	local tMarkerInfos = {}
...
function CRB_ZoneMap:OnUnitCreated(unitMade)
...
function CRB_ZoneMap:OnUnitCreateDelayTimer()
...
function CRB_ZoneMap:OnUnitDelayedCreated(unitMade)
...
---------------------------------------------------------------------------------------------------
-- Challenges
function CRB_ZoneMap:OnFailChallenge(tChallengeData)
	self:OnRemoveChallengeIcon(tChallengeData:GetId())
	self:UpdateChallengeList()
end

function CRB_ZoneMap:OnRemoveChallengeIcon(idChallenge)
...
function CRB_ZoneMap:OnAddChallengeIcon(chalCurrent, strPointIcon)
...
function CRB_ZoneMap:OnFlashChallengeIcon(idChallenge, strDescription, fDuration, tPosition)
...
function CRB_ZoneMap:OnStopChallengeFlashIcon()
	Apollo.StopTimer("ChallengeFlashIconTimer")
...
function CRB_ZoneMap:UpdateChallengeList()
...
function CRB_ZoneMap:ChallengeEntryMouseEnter( wndHandler, wndControl, x, y )
...
function CRB_ZoneMap:ChallengeEntryMouseExit( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:ChallengeEntryMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
---------------------------------------------------------------------------------------------------
-- Missions
function CRB_ZoneMap:OnPlayerPathMissionActivate(pmActivated)
...
function CRB_ZoneMap:OnPlayerPathMissionDeactivate(pmEnded)
...
function CRB_ZoneMap:UpdateMissionList()
	if self.wndMain == nil then
		-- yes, it is possible for this to be nil here because we might not have gotten the OnLoad event yet
		return
	end
...
function CRB_ZoneMap:MissionEntryMouseEnter( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:MissionEntryMouseExit( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:MissionEntryMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
---------------------------------------------------------------------------------------------------
-- Public events
function CRB_ZoneMap:OnPublicEventUpdate(peUpdated)
...
function CRB_ZoneMap:OnPublicEventCleared(peCurrent)
...
function CRB_ZoneMap:OnPublicEventEnd(peEnding, eReason, tEventInfo)
...
function CRB_ZoneMap:OnPublicEventObjectiveUpdate(peoUpdated)
	if not self.tToggledIcons.bPublicEvents then
		return
	end
...
function CRB_ZoneMap:OnPublicEventObjectiveEnd(peoEnding)
...
function CRB_ZoneMap:OnMapPulseTimer()
...
function CRB_ZoneMap:UpdatePublicEventList()
	if self.wndMain == nil then
		-- yes, it is possible for this to be nil here because we might not have gotten the OnLoad event yet
		return
	end
...
function CRB_ZoneMap:PublicEventEntryMouseEnter( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:PublicEventEntryMouseExit( wndHandler, wndControl, x, y )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:PublicEventEntryMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:OnGroupJoin()
	for idx = 2, GroupLib.GetMemberCount() do
		local tInfo = GroupLib.GetGroupMember(idx)
		if tInfo.bIsOnline then
			self.tGroupMembers[idx] =
...
function CRB_ZoneMap:OnGroupAdd(strName)
	for idx = 2, GroupLib.GetMemberCount() do
		local tInfo = GroupLib.GetGroupMember(idx)
...
function CRB_ZoneMap:OnGroupRemove(strName, eReason)
...
function CRB_ZoneMap:OnGroupLeft(eReason)
	for idx, tMember in pairs(self.tGroupMembers) do -- remove all of the group objects
		self.wndZoneMap:RemoveObject(tMember.mapObject)
	end
...
function CRB_ZoneMap:OnGroupUpdatePosition(arMembers)
...
function CRB_ZoneMap:DrawGroupMembers()
	for idx = 2, GroupLib.GetMemberCount() do
...
function CRB_ZoneMap:DrawGroupMember(tMember)
...
-----------------------------------------------------------------------------------------
-- CRB_ZoneMap Mouse Move and Click
-----------------------------------------------------------------------------------------

function CRB_ZoneMap:OnZoneMapButtonDown(wndHandler, wndControl, eButton, nX, nY, bDoubleClick) -- TODO: Bulletproof this
...
function CRB_ZoneMap:OnZoneMapMouseMove(wndHandler, wndControl, nX, nY)
...
function CRB_ZoneMap:OnZoneMap_MapCoordinateDelay()
	Apollo.StopTimer("ZoneMap_MapCoordinateDelay")
	self.bMapCoordinateDelay = false
end
...
function CRB_ZoneMap:OnCityDirectionsList(tDirections)
...
function CRB_ZoneMap:OnCityDirectionsClosed(wndHandler, wndControl)
	Event_CancelCityDirections()
	self.wndCityDirections:Destroy()
end

function CRB_ZoneMap:OnCityDirectionBtn(wndHandler, wndControl)
	GameLib.MarkCityDirection(wndHandler:GetData())
	self.wndCityDirections:Destroy()
end

function CRB_ZoneMap:OnCityDirectionMarked(tLocInfo)
...
function CRB_ZoneMap:OnZoneMap_PollCityDirectionsMarker()
...
function CRB_ZoneMap:OnZoneMap_TimeOutCityDirectionMarker()
	self.tCityDirectionsLoc = nil
	Apollo.StopTimer("ZoneMap_PollCityDirectionsMarker")
	Apollo.StopTimer("ZoneMap_TimeOutCityDirectionMarker")
...
function CRB_ZoneMap:UpdateQuestList()
	if self.wndMain == nil then
		-- yes, it is possible for this to be nil here because we might not have gotten the OnLoad event yet
		return
	end
...
function CRB_ZoneMap:BuildQuestTitleString(queCurr)
	local strName = ""
...
---------------------------------------------------------------------------------------------------
function CRB_ZoneMap:BuildQuestTooltip(queCurr)
...
---------------------------------------------------------------------------------------------------
function CRB_ZoneMap:QuestEntryMouseEnter(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
---------------------------------------------------------------------------------------------------
function CRB_ZoneMap:QuestEntryMouseExit(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:QuestEntryMouseButtonDown(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_ZoneMap:OnZoneMap_OpenMapToQuest(queArg)
...
function CRB_ZoneMap:ToggleActiveRegion(objUserData)
...
---------------------------------------------------------------------------------------------------
function CRB_ZoneMap:OnMapTrackedUnitUpdate(idTrackedUnit, tPos)
...
function CRB_ZoneMap:OnMapTrackedUnitDisable(idTrackedUnit)
	self.wndZoneMap:RemoveObjectsByUserData(self.eObjectTypeMapTrackedUnit, idTrackedUnit)
end

---------------------------------------------------------------------------------------------------
function CRB_ZoneMap:OnLevelChanged(level)
	if level == nil then
		return
	end
...
function CRB_ZoneMap:HelperLevelupUnlockGotoMap(idWorldZone)
	if not self.wndMain:IsVisible() then
		self:ToggleWindow()
	end
...
function CRB_ZoneMap:OnLevelUpUnlock_WorldMapAdventure_Astrovoid()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Illium)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapAdventure_Galeras()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Whitevale)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapAdventure_Hycrest()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Thayd)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapAdventure_Malgrave()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Grimvault)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapAdventure_NorthernWilds()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Whitevale)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapAdventure_Whitevale()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Wilderrun)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_UltimateProtogames()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Malgrave)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_SwordMaiden()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Wilderrun)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_Skullcano()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Whitevale)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_KelVoreth()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Auroria)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_Stormtalon()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Galeras)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_ProtogamesAcademyExile()
...
function CRB_ZoneMap:OnLevelUpUnlock_WorldMapDungeon_ProtogamesAcademyDominion()
	local tCurrentZoneInto = GameLib.GetCurrentZoneMap()
	if tCurrentZoneInto == nil then
		return
	end
...
function CRB_ZoneMap:OnLevelUpUnlock_WorldMapCapital_Thayd()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Thayd)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapCapital_Illium()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Illium)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Algoroc()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Algoroc)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Auroria()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Auroria)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Celestion()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Celestion)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_CrimsonIsle()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.CrimsonIsle)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Deradune()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Deradune)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Ellevar()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Ellevar)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_EverstarGrove()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.EverstarGrove)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Farside()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Farside)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Galeras()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Galeras)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Grimvault()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Grimvault)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_LevianBay()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.LevianBay)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Malgrave()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Malgrave)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_NorthernWilds()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.NorthernWilds)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Whitevale()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Whitevale)
end

function CRB_ZoneMap:OnLevelUpUnlock_WorldMapNewZone_Wilderrun()
	self:HelperLevelupUnlockGotoMap(GameLib.MapZone.Wilderrun)
end

---------------------------------------------------------------------------------------------------
function CRB_ZoneMap:FactoryProduce(wndParent, strFormName, tObject)
...
local ZoneMap_Singleton = CRB_ZoneMap:new()
