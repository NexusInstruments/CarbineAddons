-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Keybind
...
function CRB_Keybind:Init()
	Apollo.RegisterAddon(self, true, Apollo.GetString("CRB_Keybindings"))
end

function CRB_Keybind:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_KeybindingForms.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Keybind:OnDocumentReady()
	...
	self.wndKeybindForm 	= Apollo.LoadForm(self.xmlDoc, "CRB_KeybindForm", nil, self)
...
function CRB_Keybind:InitializeVariables()
...
function CRB_Keybind:RefreshKeybindList()
    self.arKeyBindings = GameLib.GetKeyBindings()
    self:PopulateKeybindList()
end

function CRB_Keybind:AddCategory(tCategory)
...
function CRB_Keybind:IsSprintAction( tBinding )
	if tBinding.strAction == "SprintModifier" then return true end
	return false
end

function CRB_Keybind:IsModifierKeyName( strName )
...
function CRB_Keybind:ClearSprintIfModifier( iKeyBinding )
...
function CRB_Keybind:AddKeyBinding(iKeyBinding, tBinding)
    -- add new binding into the form
...
function CRB_Keybind:HelperFormatKeybind(strArg)
	return strArg == Apollo.GetString("Keybinding_Unbound") and "" or strArg
end

function CRB_Keybind:PopulateKeybindList()
...
-- message dialog
---------------------------------------------------------------------------------------------------
function CRB_Keybind:ShowMessage(strMessage)
...
function CRB_Keybind:OnOkFormOk(wndHandler, wndControl, eMouseButton)
...
-- Yes no dialog
---------------------------------------------------------------------------------------------------
function CRB_Keybind:ShowYesNoDialog(strMsg, fnContinue, oContinue, fnYes, oYes, fnNo, oNo)
...
function CRB_Keybind:ExitYesNoDialog(bYesNo)
    self.bShowingYesNoDialog = false
    if bYesNo == true then
...
function CRB_Keybind:OnEscapeYesNoDialog()
    self:ExitYesNoDialog(false)
end
function CRB_Keybind:OnYesNoFormYes()
    if self.bShowingYesNoDialog then
        self:ExitYesNoDialog(true)
        return
    end
end
function CRB_Keybind:OnYesNoFormNo()
    if self.bShowingYesNoDialog then
        self:ExitYesNoDialog(false)
        return
    end
...
function CRB_Keybind:OnToggleSetSelect() --this opens the set-selection dialog
	self:ShowUndoUnbindMessage(false)
...
function CRB_Keybind:OnSetSelected(wndHandler, wndControl) --this function covers the buttons used in picking a set(ShortcutSet radio group)
...
function CRB_Keybind:ContinueSetSelected(wndControl)
...
function CRB_Keybind:OnEscapeSetSelect()
    self:SetState(LuaEnumKeybindingState.Idle)
end

function CRB_Keybind:OnEscapeSelectModifier( wndHandler, wndControl )
	self.timerModifierSelectionDelay:Start()
end

function CRB_Keybind:OnExitModifierTimer()
	self:SetState(LuaEnumKeybindingState.Idle)
end
...
function CRB_Keybind:OnRestoreDefaults( wndHandler, wndControl, eMouseButton )
	self:ShowYesNoDialog(Apollo.GetString("CRB_This_will_replace_all_keybinds"), nil, nil, self.RestoreDefaults, nil, nil)
end

function CRB_Keybind:RestoreDefaults()
	self:ShowUndoUnbindMessage(false)
...
---------------------------------------------------------------------------------------------------
function CRB_Keybind:ShowUndoUnbindMessage(bShow, strOptionalMessage, bAppendMsg)
...
function CRB_Keybind:OnUndoUnbind( wndHandler, wndControl, eMouseButton )
...
function CRB_Keybind:OverwriteDuplicates(iKeybind, tInput)
...
function CRB_Keybind:DoesUnbindKeyBreakBreakoutGameplay(iKeyBind, iSlot)
	return
...
function CRB_Keybind:OnCancelBind()
    -- back to idle state
	self:SetState(LuaEnumKeybindingState.Idle)
	return true
end

function CRB_Keybind:GetModifierFlag(nModifierScancode)
...
function CRB_Keybind:GetModifierString(nModifierScancode)
	if nModifierScancode == GameLib.CodeEnumInputModifierScancode.LeftShift then
...
function CRB_Keybind:OnKeyDown(wndHandler, wndControl, strKeyName, nCode, eModifier)
...
function CRB_Keybind:OnWindowMouseWheel(wndHandler, wndControl, nX, nY, nDelta, eModifier)
	self:OnMouseWheel(nX, nY, nDelta, eModifier)
end

function CRB_Keybind:OnMouseWheel(nX, nY, nDelta, eModifier)
...
function CRB_Keybind:OnWindowMouseButtonUp( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
	self:OnMouseButtonUp( eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
end

function CRB_Keybind:OnMouseButtonUp( eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
...
function CRB_Keybind:AcceptCurrInput(iKeybind, tCurrInput)
...
function CRB_Keybind:OnClick( wndHandler, wndControl )
    if self.bShowingYesNoDialog then
...
function CRB_Keybind:OnBindButton(wndHandler, wndControl, eMouseButton)
...
function CRB_Keybind:OnUnbindButton()
    if self.eState == LuaEnumKeybindingState.AcceptingInput then
        -- set the currently selected input binding
        local tCurrInput = {}
		tCurrInput.eDevice = GameLib.CodeEnumInputDevice.None
...
function CRB_Keybind:OnSaveBtn() --the bottom "Save" button
    self:Save()
	self:OnClose()
end

function CRB_Keybind:OnCancelBtn() --the bottom "Cancel" button
...
function CRB_Keybind:OnModifierSelected(wndHandler, wndControl, eMouseButton)
...
function CRB_Keybind:FinalizeModifierSelected(tModifierSelectedState)
...
function CRB_Keybind:IsModifierInUse(newCode)
...
function CRB_Keybind:OnModifierDropdownCheck(wndHandler, wndControl, eMouseButton)
...
function CRB_Keybind:OnCategoryClick(wndHandler, wndControl)
...
function CRB_Keybind:OnToggleCategories(wndHandler, wndControl)
	if self.bCollapseAll then
		self.wndCollapseAllBtn:SetText(Apollo.GetString("CRB_Collapse_All"))
...
function CRB_Keybind:FactoryCacheProduce(wndParent, strFormName, strKey)
...
local CRB_KeybindInst = CRB_Keybind:new()
CRB_KeybindInst:Init()
