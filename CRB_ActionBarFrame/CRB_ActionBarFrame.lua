-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ActionBarFrame
...
local CRB_ActionBarFrame = {}

function CRB_ActionBarFrame:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_ActionBarFrame:Init()
    Apollo.RegisterAddon(self)
end

function CRB_ActionBarFrame:OnLoad()
	self.nSelectedMount = nil
	self.nSelectedPotion = nil
	
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ActionBarFrame.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_ActionBarFrame:OnDocumentReady()
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_ActionBarFrameForm", "FixedHudStratum", self)
...
function CRB_ActionBarFrame:OnSave(eType)
...
function CRB_ActionBarFrame:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function CRB_ActionBarFrame:OnPlayerEquippedItemChanged()
...
function CRB_ActionBarFrame:IsWeaponEquipped()
...
function CRB_ActionBarFrame:OnUnitEnteredCombat(unit)
...
function CRB_ActionBarFrame:InitializeBars()
	self:RedrawStances()
	self:RedrawMounts()
	self:RedrawPotions()
...
function CRB_ActionBarFrame:RedrawBarVisibility()
	local unitPlayer = GameLib.GetPlayerUnit()
...
-- Main Redraw
-----------------------------------------------------------------------------------------------
function CRB_ActionBarFrame:RedrawStances()
...
function CRB_ActionBarFrame:OnMountBtn(wndHandler, wndControl)
...
function CRB_ActionBarFrame:RedrawPotions()
	local unitPlayer = GameLib.GetPlayerUnit()
...
function CRB_ActionBarFrame:OnPotionBtn(wndHandler, wndControl)
...
function CRB_ActionBarFrame:OnShowActionBarShortcut(eWhichBar, bIsVisible, nNumShortcuts)
...
function CRB_ActionBarFrame:OnShowActionBarShortcutDocked(bVisible)
	self.wndArt:FindChild("BarFrameShortcut"):Show(bVisible, not bVisible)
	self:RedrawBarVisibility()
end

function CRB_ActionBarFrame:ShowVehicleBar(eWhichBar, bIsVisible, nNumShortcuts)
...
function CRB_ActionBarFrame:OnUpdateActionBarTooltipLocation()
	for idx = 0, 9 do
		self:HelperSetTooltipType(self.arBarButtons[idx])
	end
end

function CRB_ActionBarFrame:HelperSetTooltipType(wnd)
...
function CRB_ActionBarFrame:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
function CRB_ActionBarFrame:OnUpdateInventory()
	local unitPlayer = GameLib.GetPlayerUnit()
...
function CRB_ActionBarFrame:OnGenerateTooltip(wndControl, wndHandler, eType, arg1, arg2)
	local xml = nil
...
function CRB_ActionBarFrame:OnActionBarNonSpellShortcutAddFailed()
	--TODO: Print("You can not add that to your Limited Action Set bar.")
end

function CRB_ActionBarFrame:OnCharacterCreated()
...
local CRB_ActionBarFrameInst = CRB_ActionBarFrame:new()
CRB_ActionBarFrameInst:Init()
