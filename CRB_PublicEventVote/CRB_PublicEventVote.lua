-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_PublicEventVote
...
-----------------------------------------------------------------------------------------------
-- CRB_PublicEventVote Module Definition
-----------------------------------------------------------------------------------------------
local CRB_PublicEventVote = {}

local knSaveVersion = 1

function CRB_PublicEventVote:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_PublicEventVote:Init()
    Apollo.RegisterAddon(self)
end

function CRB_PublicEventVote:OnSave(eType)
...
function CRB_PublicEventVote:OnRestore(eType, tSavedData)
...
function CRB_PublicEventVote:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_PublicEventVote.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_PublicEventVote:OnDocumentReady()
...
function CRB_PublicEventVote:Initialize()
	...
	if not self.wndMain or not self.wndMain:IsValid() then
		self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_PublicEventVoteForm", nil, self)
...
function CRB_PublicEventVote:OnPublicEventInitiateVote() -- The close checking also routes here
...
function CRB_PublicEventVote:OnOneSecTimer()
...
function CRB_PublicEventVote:OnPublicEventVoteTallied(nChoice)
	if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:IsShown() then 
		return 
	end
...
function CRB_PublicEventVote:OnVoteOptionBtn(wndHandler, wndControl) -- VoteOptionBtn, data is tOptionData
...
function CRB_PublicEventVote:OnPublicEventVoteEnded(nWinner)
	if not self.wndMain or not self.wndMain:IsValid() then
		self:Initialize()
	end
...
function CRB_PublicEventVote:OnVoteFrameHideBtn(wndHandler, wndControl)
	self.wndMain:Destroy()
	self.wndMain = nil
	Sound.Play(Sound.PlayUIWindowPublicEventVoteClose)
end

local CRB_PublicEventVoteInst = CRB_PublicEventVote:new()
CRB_PublicEventVoteInst:Init()
