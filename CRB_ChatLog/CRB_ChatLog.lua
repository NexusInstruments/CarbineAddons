-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ChatLog
...
function CRB_ChatLog:OnRestore(eType, tSavedData)
...
function CRB_ChatLog:OnConfigure() -- From ESC -> Options
...
function CRB_ChatLog:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ChatLog.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
	self.tRecent = {}
end

function CRB_ChatLog:OnDocumentReady()
...
function CRB_ChatLog:OnWindowManagementUpdate(tSettings)
...
function CRB_ChatLog:OnWindowManagementReady()
	for key, wndChat in pairs(self.tChatWindows) do
...
function CRB_ChatLog:NewChatWindow(strTitle, tViewedChannels, bCombatLog, channelCurrent)
...
function CRB_ChatLog:OnLuaChatLogMessage(strArgMessage, tArgFlags)
...
function CRB_ChatLog:OnCombatLogItemDestroy(tEventArgs)
...
function CRB_ChatLog:OnCombatLogCrafting(tEventArgs)
...
function CRB_ChatLog:OnCombatLogDatacube(tEventArgs)
...
function CRB_ChatLog:OnCombatLogModifying(tEventArgs)
...
function CRB_ChatLog:OnChatMessage(channelCurrent, tMessage)
...
function CRB_ChatLog:HelperQueueMessage(tQueuedMessage)
...
function CRB_ChatLog:HelperCheckForEmptyString(strText)
	local strFirstChar
...
function CRB_ChatLog:OnChatFlag( channelUpdated )
	-- example
...
function CRB_ChatLog:OnChatZone( description )
	-- TODO
	--local zoneName = description
end


function CRB_ChatLog:OnChatResult(channelSender, eResult)
...
function CRB_ChatLog:OnChatTellFailed( channel, strCharacterTo )
...
function CRB_ChatLog:OnChatAccountTellFailed( channel, strCharacterTo )
...
function CRB_ChatLog:OnAccountSupportTicketResult( channelSource, bSuccess )
	if( bSuccess ) then
...
function CRB_ChatLog:OnChatJoin( channelJoined )
...
function CRB_ChatLog:OnChatLeave( channelLeft, bKicked, bBanned )
	if( bBanned ) then
...
function CRB_ChatLog:OnChatList( channelSource )
...
function CRB_ChatLog:OnChatAction( channelSource, eAction, strActor, strActedOn )
	local strMessage = ""
	local strChanName = ""
...
function CRB_ChatLog:OnChatJoinResult( strChanName, eResult )
	local strMessage = Apollo.GetString("CombatFloaterType_Error")
...
function CRB_ChatLog:OnChatLineTimer()
...
function CRB_ChatLog:ShowQueuedMessages(wndForm, nDeltaTimeMS)
...
function CRB_ChatLog:PrettyItUp(wndForm)
	local wndChatList = wndForm:FindChild("Chat")
...
function CRB_ChatLog:OnChatLineFadeTimer()
	for idx, wndChat in pairs(self.tChatWindows) do
		self:RunChatWindowFade(wndChat)
	end
end

function CRB_ChatLog:RunChatWindowFade(wndChat)
...
function CRB_ChatLog:OnMouseEnter(wndHandler, wndControl, x, y)
...
function CRB_ChatLog:ChatActionTaken(wndChat)
	local tChatData = wndChat:GetData()
...
function CRB_ChatLog:OnNodeClick(wndHandler, wndControl, strNode, tAttributes, eMouseButton)
			...
			if Apollo.IsShiftKeyDown() then
				...
				if self.tLinks[nIndex].uItem then
					...
					if bWindowExists == false then
						local wndChatItemToolTip = Apollo.LoadForm("CRB_ChatLog.xml", "TooltipWindow", nil, self)
...
function CRB_ChatLog:OnCloseItemTooltipWindow(wndHandler, wndControl)
...
function CRB_ChatLog:OnEvent_EngageAccountWhisper(strDisplayName, strCharacterName, strRealmName)
	local wndEdit = self:HelperGetCurrentEditbox()
...
function CRB_ChatLog:OnEvent_EngageWhisper(strPlayerName)
	local wndEdit = self:HelperGetCurrentEditbox()
...
function CRB_ChatLog:OnChatReply()
...
function CRB_ChatLog:OnGenericEvent_ChatLogWhisper(strTarget)
	local wndParent = nil
	for idx, wndCurr in pairs(self.tChatWindows) do
...
function CRB_ChatLog:OnEmoteCheck(wndHandler, wndControl)
...
function CRB_ChatLog:OnEmoteMenuClosed(wndHandler, wndControl)
	local wndParent = wndHandler:GetParent()
	wndParent:FindChild("CloseBtn"):Enable(true)
	wndHandler:Close()
end

function CRB_ChatLog:OnEmoteMenuClose(wndHandler, wndControl)
...
function CRB_ChatLog:OnEmoteMenuEntry(wndHandler, wndControl)
...
function CRB_ChatLog:HelperLoadSetRules(luaSubclass)
...
function CRB_ChatLog:OnSuggestedMenuResult(tInfo, nTextBoxId)
	local wndEdit = self:HelperGetCurrentEditbox()
...
function CRB_ChatLog:OnCloseChatWindow(wndHandler, wndControl)
...
function CRB_ChatLog:OnChatInputReturn(wndHandler, wndControl, strText)
...
function CRB_ChatLog:VerifyChannelVisibility(channelChecking, tInput, wndChat)
	local tChatData = wndChat:GetData()
...
function CRB_ChatLog:InsertIntoRecent(strInsertName, bAccountFriends)
...
function CRB_ChatLog:RemoveFromRecent(strCharacterName)
...
function CRB_ChatLog:OnWindowGainedFocus(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_ChatLog:OnInputChanged(wndHandler, wndControl, strText)
	local wndForm = wndControl:GetParent()
...
function CRB_ChatLog:FindAnInputWindow()
	for idx, wnd in pairs(self.tChatWindows) do
		wnd:FindChild("Input"):SetData(false)
	end
end

function CRB_ChatLog:OnAddWindow(wndHandler, wndControl)
...
function CRB_ChatLog:OnAddTabSubFormClose(wndHandler, wndControl)
	local wndForm = wndControl:GetParent() -- TODO refactor
	wndForm:FindChild("AddTabSubForm"):Close()
end

function CRB_ChatLog:OnAddNewTabChat(wndHandler, wndControl) -- this is when a tab; each is its own window
	wndControl:GetParent():Close()
...
function CRB_ChatLog:OnAddNewTabCombat(wndHandler, wndControl) -- this is when a tab; each is its own window
	wndControl:GetParent():Close()
	self.nChatIndex = self.nChatIndex + 1
...
function CRB_ChatLog:OnSaveLog(wndHandler, wndControl)
	wndControl:Enable(false)
end

function CRB_ChatLog:GetDisplayNameForChannel(strChannel)
	return strChannel
end

function CRB_ChatLog:AddChannelTypeToList(tData, wndList, channel)
...
function CRB_ChatLog:OnViewCheck(wndHandler, wndControl)
...
function CRB_ChatLog:OnChatTitleChanged(wndHandler, wndControl, strNewTitle)
	local tData = wndControl:GetParent():GetParent():GetParent():GetParent():GetData()
	tData.wndForm:SetText(strNewTitle)
end

function CRB_ChatLog:OnOptionsSubFormClosed( wndHandler, wndControl )
...
function CRB_ChatLog:OnSettings(wndHandler, wndControl)
	local wndForm = wndControl:GetParent()
	local tData = wndForm:GetData()
...
function CRB_ChatLog:OnSettingsCombat(wndHandler, wndControl)
	local wndForm = wndControl:GetParent()
	local tData = wndForm:GetData()
...
function CRB_ChatLog:DrawSettings(wndForm)
	local tData = wndForm:GetData()
...
function CRB_ChatLog:DrawSettingsCombat(wndForm)
	local tData = wndForm:GetData()
...
function CRB_ChatLog:OnBeginChat(wndHandler, wndControl)
	wndControl:GetParent():FindChild("Input"):SetFocus()
end

function CRB_ChatLog:OnItemSentToCrate(itemSentToCrate, nCount)
...
function CRB_ChatLog:OnHarvestItemsSentToOwner(arSentToOwner)
...
function CRB_ChatLog:OnGenericEvent_LootChannelMessage(strMessage)
	ChatSystemLib.PostOnChannel( ChatSystemLib.ChatChannel_Loot, strMessage, "" )
end

function CRB_ChatLog:OnGenericEvent_SystemChannelMessage(strMessage)
	ChatSystemLib.PostOnChannel( ChatSystemLib.ChatChannel_System, strMessage, "" )
end

function CRB_ChatLog:OnGenericEvent_LinkItemToChat(itemLinked)
...
function CRB_ChatLog:OnCombatLogLoot(tEventArgs)
	local strResult = ""
...
function CRB_ChatLog:OnPlayedtime(strCreationDate, strPlayedTime, strPlayedLevelTime, strPlayedSessionTime, dateCreation, nSecondsPlayed, nSecondsLevel, nSecondsSession)
...
function CRB_ChatLog:OnGenericEvent_QuestLink(queLinked)
...
function CRB_ChatLog:OnGenericEvent_ArchiveArticleLink(artLinked)
...
function CRB_ChatLog:OnSaveBtn()
end

function CRB_ChatLog:OnInputTypeCheck(wndHandler, wndControl)
	local wndParent = wndControl:GetParent()
...
function CRB_ChatLog:BuildInputTypeMenu(wndChat) -- setting this up externally so we can remove it from toggle at some point
	--local wndChannel = wndControl:GetParent()
...
function CRB_ChatLog:OnInputMenuEntry(wndHandler, wndControl)
...
function CRB_ChatLog:OnRoleplayBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_ChatLog:OnLeaveCustomChannelBtn(wndHandler, wndControl)
...
function CRB_ChatLog:OnAddJoinCustomChannel(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_ChatLog:OnAddJoinInputChanging(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_ChatLog:OnAddJoinCancelBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
	local wndForm = wndControl:GetParent()
...
function CRB_ChatLog:OnAddJoinFormClose(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_ChatLog:OnAddJoinConfirmBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
	local wndForm = wndControl:GetParent()
...
function CRB_ChatLog:OnFontSizeOption(wndHandler, wndControl)
...
function CRB_ChatLog:OnTimestamp(wndHandler, wndControl)
	self.bShowTimestamp = wndControl:GetData()
end

function CRB_ChatLog:OnProfanityFilter(wndHandler, wndControl)
	if wndHandler == wndControl then
...
function CRB_ChatLog:OnChannelLabel(wndHandler, wndControl)
	self.bShowChannel = wndControl:GetData()
end

function CRB_ChatLog:OnBGFade(wndHandler, wndControl)
	local wndParent = wndControl:GetParent()
...
function CRB_ChatLog:OnChatLineFade(wndHandler, wndControl, eMouseButton)
	self.bEnableChatLineFade = wndControl:GetData()
end

function CRB_ChatLog:OnBGDrawSlider(wndHandler, wndControl)
	self.nBGOpacity = self.wndChatOptions:FindChild("BGOpacitySlider"):GetValue()
...
function CRB_ChatLog:OnTogglePlayerChatBubbles(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_ChatLog:OnToggleNPCChatBubbles(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_ChatLog:OnSaveToLog(wndHandler, wndControl)
	if wndHandler == wndControl then
...
function CRB_ChatLog:HelperGenerateChatMessage(tQueuedMessage)
...
function CRB_ChatLog:HelperGetTimeStr()
...
function CRB_ChatLog:HelperReplaceLinks(strText, arEditLinks)
...
function CRB_ChatLog:HelperAddChannelToAll(channelAdded)
...
function CRB_ChatLog:HelperRemoveChannelFromAll(channelRemoved)
...
function CRB_ChatLog:HelperRemoveChannelFromInputWindow(channelRemoved) -- used when we've totally removed a channel
	for idx, wnd in pairs(self.tChatWindows) do
...
function CRB_ChatLog:HelperFindAViewedChannel()
...
function CRB_ChatLog:HelperGetCurrentEditbox()
	local wndEdit
...
function CRB_ChatLog:OnTradeSkillSigilResult(eResult)
...
local CRB_ChatLogInstance = CRB_ChatLog:new()
CRB_ChatLogInstance:Init()
