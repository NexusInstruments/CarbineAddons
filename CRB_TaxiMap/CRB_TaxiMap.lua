-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_TaxiMap
...
-- CRB_TaxiMap OnLoad
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_TaxiMap.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_TaxiMap:OnDocumentReady()
    ...
    -- load our forms
    self.wndMain 	= Apollo.LoadForm(self.xmlDoc, "CRB_TaxiMapForm", nil, self)
...
function CRB_TaxiMap:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementAdd", {wnd = self.wndMain, strName = Apollo.GetString("CRB_Taxi_Vendor"), nSaveVersion = 2})
end

-----------------------------------------------------------------------------------------------
-- CRB_TaxiMap Functions
-----------------------------------------------------------------------------------------------

function CRB_TaxiMap:OnInvokeTaxiWindow(unitTaxi, bSettlerTaxi)
...
function CRB_TaxiMap:OnInvokeShuttlePrompt(unitTaxi)
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnFlightPathUpdate()
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:PopulateTaxiMap()
...
-----------------------------------------------------------------------------------------------
-- CRB_TaxiMapForm Functions
-----------------------------------------------------------------------------------------------

function CRB_TaxiMap:OnWindowClosed()
	self.tTaxiObjects = {}
	self.tTaxiNodes = {}
	self.tTaxiRoutes = {}
	self.nTaxiUnderCursor = 0
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnPromptWindowClosed()
	Event_CancelTaxiVendor()
end

-----------------------------------------------------------------------------------------------
-- when the Cancel button is clicked
function CRB_TaxiMap:OnCancel(wndHandler, wndControl)
...
function CRB_TaxiMap:OnNo(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
function CRB_TaxiMap:OnYes(wndHandler, wndControl)
	if wndHandler:GetId() ~= wndControl:GetId() then
		return
	end
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnCloseVendorWindow()
	if self.wndMain:IsShown() then
		self.wndMain:Close()
	end
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnTaxiMapButtonDown(wndHandler, wndControl, eButton, nX, nY, bDoubleClick)
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnMouseMove(wndHandler, wndControl, nX, nY)
	self:OnGenerateTooltip(wndHandler, wndControl, Tooltip.TooltipGenerateType_Default, nX, nY)
end

-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnGenerateTooltip(wndHandler, wndControl, eType, nX, nY)
...
-----------------------------------------------------------------------------------------------
function CRB_TaxiMap:OnMessageDisplayTimer()
	self.wndMessage:Show(false)
end

-----------------------------------------------------------------------------------------------
-- CRB_TaxiMap Instance
-----------------------------------------------------------------------------------------------
local CRB_TaxiMapInst = CRB_TaxiMap:new()
CRB_TaxiMapInst:Init()
