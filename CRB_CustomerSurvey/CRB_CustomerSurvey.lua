-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_CustomerSurvey
...
local CRB_CustomerSurvey = {}
function CRB_CustomerSurvey:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_CustomerSurvey:Init()
    Apollo.RegisterAddon(self)
end

function CRB_CustomerSurvey:OnLoad()
   	Apollo.RegisterEventHandler("NewCustomerSurveyRequest", "OnSurveyRequest", self)
	Apollo.RegisterEventHandler("UnitEnteredCombat", "OnEnteredCombat", self)
end

function CRB_CustomerSurvey:Initialize()
	if self.wndMain and self.wndMain:IsValid() then
		return
	end

    self.wndMain = Apollo.LoadForm("CRB_CustomerSurvey.xml", "CRB_CustomerSurveyForm", nil, self)
...
function CRB_CustomerSurvey:OnSurveyRequest()
	self:Initialize()
	self:RedrawAll()
end

function CRB_CustomerSurvey:OnEnteredCombat(unit, bInCombat)
...
function CRB_CustomerSurvey:RedrawAll()
	self:UpdateAlert()
	self:UpdateSurvey()
end

function CRB_CustomerSurvey:UpdateAlert()
	if self.wndAlertContainer and self.wndAlertContainer:IsValid() then
		self.wndAlertContainer:Destroy()
	end

	self.wndAlertContainer = Apollo.LoadForm("CRB_CustomerSurvey.xml", "CustomerSurveyAlertContainer", "FixedHudStratum", self)
	for idx = 1, 3 do
		local tSurvey = CustomerSurveyLib.GetPending(idx)
		if tSurvey then
			local wndAlert = Apollo.LoadForm("CRB_CustomerSurvey.xml", "CustomerSurveyAlert", self.wndAlertContainer:FindChild("List"), self)
...
function CRB_CustomerSurvey:UpdateSurvey()
	local bDoShow = false
	...
	if self.csActiveSurvey then
		...
		if tQuestions then
			...
			for idx, strQuestion in pairs(tQuestions) do
				local wndCurr = Apollo.LoadForm("CRB_CustomerSurvey.xml", "QuestionEntry", self.wndMain:FindChild("QuestionsForm"), self)
			...
			if not self.wndCommentEntry or not self.wndCommentEntry:IsValid() then
				self.wndCommentEntry = Apollo.LoadForm("CRB_CustomerSurvey.xml", "CommentEntry", self.wndMain:FindChild("QuestionsForm"), self)
			end
...
function CRB_CustomerSurvey:SaveEntries()
	if self.csActiveSurvey then
		local tResults = {}
...
-----------------------------------------------------------------------------------------------
-- CRB_CustomerSurveyForm Functions
-----------------------------------------------------------------------------------------------

function CRB_CustomerSurvey:OnOK()
	self:SaveEntries()
	if self.csActiveSurvey then
...
function CRB_CustomerSurvey:OnClose()
	if self.csActiveSurvey then
...
function CRB_CustomerSurvey:OnCustomerSurveyAlertBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_CustomerSurvey:OnAnyInputFromUI(wndHandler, wndControl)
...
local CRB_CustomerSurveyInst = CRB_CustomerSurvey:new()
CRB_CustomerSurveyInst:Init()
