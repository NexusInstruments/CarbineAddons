-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_AbilityAMPs
...
function CRB_AbilityAMPs:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_AbilityAMPs:Init()
    Apollo.RegisterAddon(self)
end

function CRB_AbilityAMPs:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_AbilityAMPs.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_AbilityAMPs:OnDocumentReady()
...
function CRB_AbilityAMPs:Initialize(wndParent)
	self.arHighestUnlockedTier = {}
	self:OnStartRedrawAll(wndParent)
end

function CRB_AbilityAMPs:OnStartRedrawAll(wndParent)
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() then
		self.tWndRefs.wndMain:Destroy()
		self.tWndRefs = {}
	end

	self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_AbilityAMPsForm", wndParent, self)
...
function CRB_AbilityAMPs:OnPlayerCurrencyChanged()
...
function CRB_AbilityAMPs:OnShowResurrectDialog()
	self:OnPlayerCurrencyChanged()
end

function CRB_AbilityAMPs:OnPlayerResurrected()
	self.bResurrected = true
	self:OnPlayerCurrencyChanged()
end

function CRB_AbilityAMPs:OnClose(wndHandler, wndControl)
...
function CRB_AbilityAMPs:BuildFromEvent()
...
function CRB_AbilityAMPs:DestroyAndBuild()
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() then
...
function CRB_AbilityAMPs:RedrawAll() -- Do not pass in arguments, this can come from multiple sources
...
function CRB_AbilityAMPs:RedrawSelections(tEldanAugmentationData)
	-- AMP icon states
...
function CRB_AbilityAMPs:OnLabelShortcutBtn(wndHandler, wndControl)
...
function CRB_AbilityAMPs:OnResetConfirmYesBtn(wndHandler, wndControl)
	AbilityBook.RespecEldanAugmentations()
	self:DestroyAndBuild()
end

function CRB_AbilityAMPs:OnResetConfirmWindowClosed(wndHandler, wndControl)
...
function CRB_AbilityAMPs:OnAugmentationTooltip(wndHandler, wndControl, eToolTipType, x, y)
	...
	self.wndTooltip = wndHandler:LoadTooltipForm("CRB_AbilityAMPs.xml", "TooltipForm", self)
...
function CRB_AbilityAMPs:OnAmpFormBtn(wndHandler, wndControl, eMouseBtn) -- AmpFormButton
	local tAugment = wndHandler:GetData()
...
function CRB_AbilityAMPs:OnResetCategoryBtn(wndHandler, wndControl)
...
function CRB_AbilityAMPs:OnResetConfirm(wndHandler, wndControl)
...
function CRB_AbilityAMPs:OnResetCancel(wndHandler, wndControl)
	self.tWndRefs.wndMain:FindChild("ResetConfirmation"):Show(false)
end

function CRB_AbilityAMPs:HelperCreateMessage(strMessage, nDuration)
...
function CRB_AbilityAMPs:OnMessageDisplayTimer()
	self.timerMessageDisplay:Stop()
...
function CRB_AbilityAMPs:OnUpdateMessageMouseClick(wndHandler, wndControl)
	self:OnMessageDisplayTimer()
end

function CRB_AbilityAMPs:OnConfigureAMPs()
	Event_FireGenericEvent("CharacterUnlockedInlaidEldanAugmentation")
	self.tWndRefs.wndDialogReset:Destroy()
end

function CRB_AbilityAMPs:OnResetAMPsClose()
	self.tWndRefs.wndDialogReset:Destroy()
end	

function CRB_AbilityAMPs:HelperCheckCached()
...
local CRB_AbilityAMPsInst = CRB_AbilityAMPs:new()
CRB_AbilityAMPsInst:Init()
