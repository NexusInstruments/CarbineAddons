-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_SocialPanel
...
local CRB_SocialPanel = {}
local knMaxNumberOfCircles = 5

function CRB_SocialPanel:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_SocialPanel:Init()
    Apollo.RegisterAddon(self)
end

function CRB_SocialPanel:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_SocialPanel.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_SocialPanel:OnDocumentReady()
...
function CRB_SocialPanel:OnToggleSocialWindow(strArg) -- 1st Arg may be objects from code and such
	self:Initialize()
...
function CRB_SocialPanel:OnToggleGuild()
	self:OnToggleSocialWindow("GuildFrame")
end

function CRB_SocialPanel:OnInvokeNeighborsList()
...
function CRB_SocialPanel:OnInvokeFriendsList()
	self:OnToggleSocialWindow("ContactsFrame")
end

function CRB_SocialPanel:OnFriendshipInviteChange(tInvite)
	self:FullRedrawIfVisible()
	Apollo.StartTimer("RecalculateInvitesTimer")
end

function CRB_SocialPanel:FullRedrawIfVisible()
...
function CRB_SocialPanel:Initialize()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_SocialPanelForm", nil, self)
...
function CRB_SocialPanel:FullyDrawSplashScreen(bHide)
	self:Initialize()
...
function CRB_SocialPanel:HelperSortCirclesChannelOrder(guildLhs, guildRhs)
...
function CRB_SocialPanel:OnCloseBtn(wndHandler, wndControl)
...
---------------------------------------------------------------------------------------------------
-- CRB_SocialPanelForm Functions
---------------------------------------------------------------------------------------------------

function CRB_SocialPanel:OnSplashContactsCheck(wndHandler, wndControl, eMouseButton, nPosX, nPosY, bDoubleClick)
	Event_FireGenericEvent("GenericEvent_InitializeFriends", self.tWndRefs.wndContactsFrame)
end

function CRB_SocialPanel:OnSplashContactsUncheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_DestroyFriends")
end

function CRB_SocialPanel:OnSplashNeighborCheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_InitializeNeighbors", self.tWndRefs.wndNeighborsFrame)
end

function CRB_SocialPanel:OnSplashNeighborUncheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_DestroyNeighbors")
end

function CRB_SocialPanel:OnSplashGuildCheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_InitializeGuild", self.tWndRefs.wndGuildFrame)
end

function CRB_SocialPanel:OnSplashGuildUncheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_DestroyGuild")
end

function CRB_SocialPanel:OnCircleItemCheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_InitializeCircles", self.tWndRefs.wndCirclesFrame, wndHandler:GetData())
end

function CRB_SocialPanel:OnCircleItemUncheck( wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_DestroyCircles")
end

-- Special Circle Handlers
function CRB_SocialPanel:OnSplashCirclesAddBtn(wndHandler, wndControl)
	Event_FireGenericEvent("EventGeneric_OpenCircleRegistrationPanel", self.tWndRefs.wndMain)
end

function CRB_SocialPanel:OnSplashCirclesCheck( wndHandler, wndControl)
	self.tWndRefs.wndMain:FindChild("SplashCircleItemContainerFrame"):Show(true)
end

function CRB_SocialPanel:OnSplashCirclesUncheck( wndHandler, wndControl)
...
function CRB_SocialPanel:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
function CRB_SocialPanel:OnInterfaceMenuListHasLoaded()
...
function CRB_SocialPanel:CalcFriendInvites()
...
function CRB_SocialPanel:OnFriendshipRemove()
	self.timerRemovedMemberDelay:Start()
end

local CRB_SocialPanelInst = CRB_SocialPanel:new()
CRB_SocialPanelInst:Init()
