-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_QuestTracker
...
function CRB_QuestTracker:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_QuestTracker:Init()
    Apollo.RegisterAddon(self, false, nil, {"Tooltips"})
end

function CRB_QuestTracker:OnDependencyError(strDependency, strError)
	return true
end

function CRB_QuestTracker:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_QuestTracker.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
	
	Event_FireGenericEvent("ObjectiveTracker_RequestParent")
end

function CRB_QuestTracker:OnSave(eType)
...
function CRB_QuestTracker:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function CRB_QuestTracker:OnDocumentReady()
...
function CRB_QuestTracker:OnObjectiveTrackerLoaded(wndForm)
	...
	self.bLoaded = true
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_QuestTrackerForm", wndForm, self)
...
function CRB_QuestTracker:OnToggleQuestOptions()
	self:DrawContextMenu()
end

function CRB_QuestTracker:InitializeWindowMeasuring() -- Try not to run these OnLoad as they may be expensive
...
function CRB_QuestTracker:OnOptionsUpdated()
...
function CRB_QuestTracker:OnQuestInit()
	self:RequestRedrawAll()
end

function CRB_QuestTracker:OnSubZoneChanged()
	self:RequestRedrawAll()
end

function CRB_QuestTracker:OnChangeWorld()
	self:DestroyAndRedraw()
end

function CRB_QuestTracker:OnQuestTrackerOrderTimer()
...
function CRB_QuestTracker:OnQuestTrackerBlinkTimer()
...
function CRB_QuestTracker:RequestRedrawAll()
...
function CRB_QuestTracker:RequestRunObjectiveTimer()
	self.bRunObjectiveTimer = true
end

function CRB_QuestTracker:DestroyAndRedraw()
...
function CRB_QuestTracker:RedrawTimed()
...
function CRB_QuestTracker:RedrawAll()
	Apollo.StopTimer("QuestTrackerRedrawTimer")
	self.bRedrawQueued = false
...
function CRB_QuestTracker:CleanEpisodeGroup(strMarker)
...
function CRB_QuestTracker:BuildContainer(strEpisodeGroupMarker)
...
function CRB_QuestTracker:BuildEpisodeGroup(strEpisodeGroupMarker)
...
function CRB_QuestTracker:DrawFilteredTaskEpisode(idx, epiEpisode, wndParent, wndPinnedContainer)
...
function CRB_QuestTracker:DrawEpisode(idx, epiEpisode, wndParent, wndPinnedContainer)
...
function CRB_QuestTracker:DrawEpisodeQuests(epiEpisode, wndContainer, wndPinnedContainer)
	for nIdx, queQuest in pairs(epiEpisode:GetTrackedQuests(0, self.bQuestTrackerByDistance)) do
		if self.tHiddenImbu and self.tHiddenImbu[queQuest:GetId()] then
			-- Skip imbuement quests, these need to constantly re-grant themselves so we have to use a UI hack.
		else
...
function CRB_QuestTracker:DrawContract(queQuest, wndParent)
...
function CRB_QuestTracker:DrawQuest(nIdx, queQuest, wndParent)
...
function CRB_QuestTracker:DrawQuestSpell(queQuest, wndQuest)
...
function CRB_QuestTracker:DrawQuestObjective(wndQuest, wndObjective, queQuest, tObjective)
...
function CRB_QuestTracker:DrawPublicEpisodes(tPublicEvents)
...
function CRB_QuestTracker:DrawEvent(wndParent, peEvent, nAlphabetNumber)
...
function CRB_QuestTracker:DrawZombieEvent(wndParent, tZombieEvent, nAlphabetNumber)
...
function CRB_QuestTracker:DrawEventObjective(wndObjective, peEvent, idObjective, peoObjective)
...
function CRB_QuestTracker:OnShowCommMsg(idMsg, idCaller, queUpdated, strText)
...
function CRB_QuestTracker:ResizeAll()
	-- Sort
...
function CRB_QuestTracker:OnResizeEpisodeGroup(wndEpisodeGroup)
...
function CRB_QuestTracker:OnResizeEpisode(wndEpisode)
...
function CRB_QuestTracker:OnResizeQuest(wndQuest)
...
function CRB_QuestTracker:OnResizeQuestObjective(wndObj)
...
function CRB_QuestTracker:OnEpisodeMinimizedBtnChecked(wndHandler, wndControl, eMouseButton)
	self.tMinimized.tEpisode[wndHandler:GetData()] = true
	self:RedrawAll()
end

function CRB_QuestTracker:OnEpisodeMinimizedBtnUnChecked(wndHandler, wndControl, eMouseButton)
	self.tMinimized.tEpisode[wndHandler:GetData()] = false
	self:RedrawAll()
end

function CRB_QuestTracker:OnEpisodeGroupMinimizedBtnChecked(wndHandler, wndControl, eMouseButton)
	self.tMinimized.tEpisodeGroup[wndHandler:GetData()] = true
	self:RedrawAll()
end

function CRB_QuestTracker:OnEpisodeGroupMinimizedBtnUnChecked(wndHandler, wndControl, eMouseButton)
	self.tMinimized.tEpisodeGroup[wndHandler:GetData()] = false
	self:RedrawAll()
end

function CRB_QuestTracker:OnQuestOpenMapBtn(wndHandler, wndControl) -- wndHandler should be "QuestOpenMapBtn" and its data is tQuest
	Event_FireGenericEvent("ZoneMap_OpenMapToQuest", wndHandler:GetData())
	self:OnQuestTrackerRightClickClose()
end

function CRB_QuestTracker:OnQuestCallbackBtn(wndHandler, wndControl) -- wndHandler is "QuestCallbackBtn" and its data is tQuest
	CommunicatorLib.CallContact(wndHandler:GetData())
end

function CRB_QuestTracker:OnShowEventStatsBtn(wndHandler, wndControl) -- wndHandler is "ShowEventStatsBtn" and its parent's data is peEvent
...
function CRB_QuestTracker:OnEventCallbackBtn(wndHandler, wndControl) -- wndHandler is "QuestCallbackBtn" and its data is wndEvent
...
function CRB_QuestTracker:OnEpisodeHintArrow(wndHandler, wndControl) -- wndHandler is "ControlBackerBtn"
	wndHandler:GetData():ShowHintArrow()
end

function CRB_QuestTracker:OnQuestHintArrow(wndHandler, wndControl, eMouseButton) -- wndHandler is "ControlBackerBtn" and its data is wndQuest
...
function CRB_QuestTracker:OnQuestObjectiveHintArrow(wndHandler, wndControl, eMouseButton) -- "QuestObjectiveBtn" (can be from EventItem), data is { tQuest, tObjective.index }
...
function CRB_QuestTracker:OnQuestGearBtn(wndHandler, wndControl)
...
function CRB_QuestTracker:HelperQuestRightClick(queQuest)
	self:OnQuestTrackerRightClickClose()
...
function CRB_QuestTracker:OnQuestTrackerRightClickClose() -- From a variety of source
...
function CRB_QuestTracker:OnRightClickOpenLogBtn(wndHandler, wndControl, eMouseButton) -- wndHandler is "RightClickOpenLogBtn" and its data is tQuest
...
function CRB_QuestTracker:OnRightClickShareQuestBtn(wndHandler, wndControl)
	wndHandler:GetData():Share()
	self:OnQuestTrackerRightClickClose()
end

function CRB_QuestTracker:OnRightClickLinkToChatBtn(wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_QuestLink", wndHandler:GetData())
	self:OnQuestTrackerRightClickClose()
end

function CRB_QuestTracker:OnRightClickMaxMinBtn(wndHandler, wndControl)
...
function CRB_QuestTracker:OnRightClickPinUnpinBtn(wndHandler, wndControl)
	local queQuest = wndHandler:GetData()
	if self.tPinned.tQuests[queQuest:GetId()] then
...
function CRB_QuestTracker:OnRightClickHideBtn(wndHandler, wndControl)
	local queQuest = wndHandler:GetData()
...
function CRB_QuestTracker:OnQuestItemMouseEnter(wndHandler, wndControl)
...
function CRB_QuestTracker:OnQuestItemMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:GetData() and Quest.is(wndHandler:GetData()) then
...
function CRB_QuestTracker:OnQuestNumberBackerMouseEnter(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:FindChild("QuestNumberBackerGlow"):Show(true)
	end
end

function CRB_QuestTracker:OnQuestNumberBackerMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:FindChild("QuestNumberBackerGlow"):Show(false)
	end
end

function CRB_QuestTracker:OnControlBackerMouseEnter(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:FindChild("QuestGearBtn") then
		wndHandler:FindChild("QuestGearBtn"):Show(true)
	end
end

function CRB_QuestTracker:OnControlBackerMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:FindChild("QuestGearBtn") then
		wndHandler:FindChild("QuestGearBtn"):Show(false)
	end
end

function CRB_QuestTracker:OnEpisodeControlBackerMouseEnter(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:FindChild("EpisodeMinimizeBtn"):Show(true)
	end
end

function CRB_QuestTracker:OnEpisodeControlBackerMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl then
...
function CRB_QuestTracker:OnEpisodeGroupControlBackerMouseEnter(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:FindChild("EpisodeGroupMinimizeBtn"):Show(true)
	end
end

function CRB_QuestTracker:OnEpisodeGroupControlBackerMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl then
...
function CRB_QuestTracker:OnShowResurrectDialog()
	if self.unitPlayer then
		self.bPlayerIsDead = self.unitPlayer:IsDead()
	end
end

function CRB_QuestTracker:OnPlayerResurrected()
	self.bPlayerIsDead = false
end

function CRB_QuestTracker:OnToggleLongQuestText(bToggle)
	self.bShowLongQuestText = bToggle
end

function CRB_QuestTracker:OnQuestStateChanged(queQuest, eState)
...
function CRB_QuestTracker:OnContractStateChanged(contractActive, eState)
	self:OnQuestStateChanged(contractActive:GetQuest(), eState)
end

function CRB_QuestTracker:OnQuestObjectiveUpdated(queQuest, idObjective)
...
function CRB_QuestTracker:OnQuestTracker_EarliestProgBarTimer()
...
function CRB_QuestTracker:OnGenericEvent_QuestLog_TrackBtnClicked(queSelected)
...
function CRB_QuestTracker:OnDestroyQuestObject(queQuest)
	self.nFlashThisQuest = queQuest
	self:QueueQuestForDestroy(queQuest)
	self:RequestRedrawAll()
end
...
function CRB_QuestTracker:OnPublicEventStart(peEvent)
...
function CRB_QuestTracker:OnPublicEventEnd(peEvent, eReason, tStats)
...
function CRB_QuestTracker:OnPublicEventUpdate(peEvent)
	self:RequestRedrawAll()
end

function CRB_QuestTracker:OnPublicEventLiveStatsUpdate(peEvent)
	self:RequestRedrawAll()
end

function CRB_QuestTracker:OnPublicEventObjectiveUpdate(peoObjective)
	self:RequestRedrawAll()
end
...
function CRB_QuestTracker:HelperShowQuestCallbackBtn(wndQuest, queQuest, strNumberBackerArt, strCallbackBtnArt)
...
function CRB_QuestTracker:HelperBuildTimedQuestTitle(queQuest)
	local strTitle = queQuest:GetTitle()
...
function CRB_QuestTracker:BuildObjectiveTitleString(queQuest, tObjective, bIsTooltip)
	local strResult = ""
...
function CRB_QuestTracker:BuildEventObjectiveTitleString(queQuest, peoObjective, bIsTooltip)
	-- Use short form or reward text if possible
	local strResult = ""
...
function CRB_QuestTracker:OnGenerateTooltip(wndControl, wndHandler, eType, arg1, arg2)
	local xml = nil
...
function CRB_QuestTracker:UpdateGroup()
...
function CRB_QuestTracker:HelperPrefixTimeString(fTime, strAppend, strColorOverride)
...
function CRB_QuestTracker:QueueQuestForDestroy(queQuest)
	table.insert(self.tQuestsQueuedForDestroy, queQuest)
end

function CRB_QuestTracker:HelperFindAndDestroyQuests()
...
function CRB_QuestTracker:OnQuestDestroyed(queQuest)
	for index, tQuestInfo in pairs(self.tTimedQuests) do
...
function CRB_QuestTracker:OnEventDestroyed(peEvent)
	for index, tEventInfo in pairs(self.tTimedEvents) do
...
function CRB_QuestTracker:FactoryProduce(wndParent, strFormName, tObject)
...
-- Right Click
-----------------------------------------------------------------------------------------------
function CRB_QuestTracker:CloseContextMenu() -- From a variety of source
...
function CRB_QuestTracker:DrawContextMenu()
...
function CRB_QuestTracker:HelperDrawContextMenuSubOptions(wndIgnore)
	if self.wndContextMenu and self.wndContextMenu:IsValid() then
...
function CRB_QuestTracker:OnToggleShowContracts()
...
function CRB_QuestTracker:OnToggleShowQuests()
...
function CRB_QuestTracker:OnToggleFilterDistance()
...
function CRB_QuestTracker:OnMissionDistanceEditBoxChanged(wndHandler, wndControl)
...
function CRB_QuestTracker:OnRightClickOptionBtn(wndHandler, wndControl)
...
---------------------------------------------------------------------------------------------------
function CRB_QuestTracker:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor == GameLib.CodeEnumTutorialAnchor.CRB_QuestTracker or eAnchor == GameLib.CodeEnumTutorialAnchor.QuestCommunicatorReceived then
...
local CRB_QuestTrackerInst = CRB_QuestTracker:new()
CRB_QuestTrackerInst:Init()
