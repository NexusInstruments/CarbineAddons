-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ZoneCompletion
...
function CRB_ZoneCompletion:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_ZoneCompletion:OnSave(eType)
...
function CRB_ZoneCompletion:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function CRB_ZoneCompletion:Init()
    Apollo.RegisterAddon(self)
end

function CRB_ZoneCompletion:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ZoneCompletion.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_ZoneCompletion:OnDocumentReady()
...
function CRB_ZoneCompletion:OnClose(wndHandler, wndControl)
	Apollo.StopTimer("ZoneCompletion_UpdateWindowTimer")
...
function CRB_ZoneCompletion:OnMinimized(wndHandler, wndControl)
...
function CRB_ZoneCompletion:OnOpened(wndHandler, wndControl)
...
function CRB_ZoneCompletion:OnShow(wndParent)
	-- One time initialize
...
function CRB_ZoneCompletion:OnZoneChanged(nZoneId)
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_ZoneCompletion:OnZoomLevelChanged(bValidZoneZoom)
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_ZoneCompletion:OnSetMapCompletionShown(bToggle)
	if self.wndMinimizeToggle and self.wndMinimizeToggle:IsValid() then
		self.wndMinimizeToggle:FindChild("ZoneCompletionMinimizeToggleBtn"):SetCheck(bToggle)
	end
end

function CRB_ZoneCompletion:RedrawFromUI(wndHandler, wndControl)
	self:RedrawAll()
end

function CRB_ZoneCompletion:RedrawAll() -- Also from GenericEvent_ZoneMap_ZoneChanged and XML	
...
function CRB_ZoneCompletion:HelperBuildCountItem(tLocalZoneValues, nIndex)
...
function CRB_ZoneCompletion:HelperBuildPercentExploredItem(nPercent)
	local wndCount = Apollo.LoadForm(self.xmlDoc, "CountItem", self.wndMain:FindChild("CountsContainer"), self)
...
function CRB_ZoneCompletion:HelperBuildPathCountItem(tPathValues)
...
function CRB_ZoneCompletion:OnMinimizeToggle(wndHandler, wndControl)
...
local CRB_ZoneCompletionInst = CRB_ZoneCompletion:new()
CRB_ZoneCompletionInst:Init()
