-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_SprintMeter
...
function CRB_SprintMeter:Init()
	Apollo.RegisterAddon(self)
end

function CRB_SprintMeter:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_SprintMeter.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_SprintMeter:OnDocumentReady()
...
function CRB_SprintMeter:OnWindowManagementReady()
...
function CRB_SprintMeter:OnFrame()
	...
	if bAtMaxSprint and not self.bJustFilledSprint and self.tWindowMap["SprintBG"]:IsVisible() then
		...
		self.tWindowMap["SprintProgFlash"]:SetSprite("CRB_SprintMeter:sprVerticalBar_Flash")
	...
	if self.nLastDashValue ~= nDashCurr then
		...
		if nDashCurr >= nHalfDash and self.nLastDashValue < nHalfDash then
			self.tWindowMap["DashProgFlash1"]:SetSprite("CRB_SprintMeter:sprDashBot_Flash")
		end
	...
	if bAtMaxDash and not self.bJustFilledDash and self.tWindowMap["DashBG"]:IsVisible() then
		...
		self.tWindowMap["DashProgFlash2"]:SetSprite("CRB_SprintMeter:sprDashTop_Flash")
...
function CRB_SprintMeter:OnSprintMeterGracePeriod()
	Apollo.StopTimer("SprintMeterGracePeriod")
	self.bJustFilledSprint = false
	self.tWindowMap["SprintBG"]:Show(false)
end

function CRB_SprintMeter:OnDashMeterGracePeriod()
	Apollo.StopTimer("DashMeterGracePeriod")
	self.bJustFilledDash = false
	self.tWindowMap["DashBG"]:Show(false)
end

function CRB_SprintMeter:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor == GameLib.CodeEnumTutorialAnchor.CRB_SprintMeter then
		local tRect = {}
...
local CRB_SprintMeterInst = CRB_SprintMeter:new()
CRB_SprintMeterInst:Init()
