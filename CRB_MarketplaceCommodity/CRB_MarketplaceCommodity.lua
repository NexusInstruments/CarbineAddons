-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_MarketplaceCommodity
...
local CRB_MarketplaceCommodity = {}
...
function CRB_MarketplaceCommodity:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_MarketplaceCommodity:Init()
    Apollo.RegisterAddon(self)
end

function CRB_MarketplaceCommodity:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MarketplaceCommodity.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_MarketplaceCommodity:OnDocumentReady()
...
function CRB_MarketplaceCommodity:OnDestroy()
...
function CRB_MarketplaceCommodity:Initialize()
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_MarketplaceCommodityForm", nil, self)
...
function CRB_MarketplaceCommodity:InitializeCategories()
...
function CRB_MarketplaceCommodity:OnResizeCategories() -- Can come from XML
...
function CRB_MarketplaceCommodity:OnHeaderBtnToggle()
	-- Filters
...
function CRB_MarketplaceCommodity:InitializeSell()
...
function CRB_MarketplaceCommodity:InitializeBuy()
...
function CRB_MarketplaceCommodity:OnSearchEditBoxChanged(wndHandler, wndControl) -- SearchEditBox
	self.wndMain:FindChild("SearchClearBtn"):Show(string.len(wndHandler:GetText() or "") > 0)
end

function CRB_MarketplaceCommodity:OnSearchClearBtn(wndHandler, wndControl)
	self.wndMain:FindChild("SearchEditBox"):SetText("")
	self.wndMain:FindChild("SearchClearBtn"):Show(false)
	self:OnSearchCommitBtn()
end

function CRB_MarketplaceCommodity:OnSearchCommitBtn(wndHandler, wndControl) -- ALso SearchEditBox's WindowKeyReturn
...
function CRB_MarketplaceCommodity:OnRefreshBtn(wndHandler, wndControl) -- Also from lua and multiple XML buttons
	self.wndMain:FindChild("RefreshAnimation"):SetSprite("CRB_WindowAnimationSprites:sprWinAnim_BirthSmallTemp")
	self:OnHeaderBtnToggle()
end
...
function CRB_MarketplaceCommodity:BuildListItem(tCurrItem, strWindowName, strBtnText)
...
function CRB_MarketplaceCommodity:OnListInputNumberChanged(wndHandler, wndControl, strText)
...
function CRB_MarketplaceCommodity:OnListInputNumberUpBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
...
function CRB_MarketplaceCommodity:OnListInputNumberDownBtn(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
...
function CRB_MarketplaceCommodity:OnListInputNumberHelper(wndParent, nNewValue)
...
function CRB_MarketplaceCommodity:OnListInputPriceAmountChanged(wndHandler, wndControl) -- ListInputPrice, data is parent
...
function CRB_MarketplaceCommodity:OnListInputPriceMouseDown(wndHandler, wndControl)
	wndHandler:SetStyleEx("SkipZeroes", false)
	self:HelperValidateListInputForSubmit(wndHandler:GetData())
end

function CRB_MarketplaceCommodity:OnListInputPriceLoseFocus(wndHandler, wndControl)
	wndHandler:SetStyleEx("SkipZeroes", true)
	self:HelperValidateListInputForSubmit(wndHandler:GetData())
end

function CRB_MarketplaceCommodity:HelperValidateListInputForSubmit(wndParent)
	local nAvailable = 0
	local nQuantity = 0
	local nPrice = 0
...
function CRB_MarketplaceCommodity:OnFilterOptionsLevelUpBtn(wndHandler, wndControl)
...
function CRB_MarketplaceCommodity:OnFilterOptionsLevelDownBtn(wndHandler, wndControl)
	local wndEditBox = wndHandler:GetParent():FindChild("FilterOptionsLevelEditBox")
	local nOldValue = tonumber(wndEditBox:GetText())
	local nNewValue = nOldValue and nOldValue - 1
	wndEditBox:SetText(nNewValue)
...
function CRB_MarketplaceCommodity:OnFilterEditBoxChanged(wndHandler, wndControl)
	local wndEditBox = wndHandler:GetParent():FindChild("FilterOptionsLevelEditBox")
	self:HelperCheckValidLevelValues(wndEditBox)
...
function CRB_MarketplaceCommodity:OnFilterOptionsRarityItemToggle(wndHandler, wndControl) -- FilterOptionsRarityItemBtn
...
function CRB_MarketplaceCommodity:OnResetFilterBtn(wndHandler, wndControl)
	local wndFilter = self.wndMain:FindChild("FilterContainer")
...
function CRB_MarketplaceCommodity:OnFilterOptionsWindowClosed(wndHandler, wndControl)
...
function CRB_MarketplaceCommodity:HelperCheckValidLevelValues(wndChanged)
...
function CRB_MarketplaceCommodity:OnCategoryTopBtnToggle(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
...
function CRB_MarketplaceCommodity:OnCategoryMidBtnCheck(wndHandler, wndControl)
...
function CRB_MarketplaceCommodity:OnGenerateSimpleConfirmTooltip(wndHandler, wndControl, eType, nX, nY) -- wndHandler is ListSubmitBtn, data is { tCurrItem and window "SimpleListItem" }
	...
	local bBuyNow = self.wndMain:FindChild("HeaderBuyNowBtn"):IsChecked()
	local wndTooltip = wndHandler:LoadTooltipForm("CRB_MarketplaceCommodity.xml", "SimpleConfirmTooltip", self)
...
function CRB_MarketplaceCommodity:OnGenerateAdvancedConfirmTooltip(wndHandler, wndControl, eType, nX, nY)
	...
	local wndTooltip = wndHandler:LoadTooltipForm("CRB_MarketplaceCommodity.xml", "AdvancedConfirmTooltip", self)
...
function CRB_MarketplaceCommodity:OnGenerateTooltipFullStats(wndHandler, wndControl, eType, nX, nY) -- GOTCHA: wndHandler is ListSubtitle
	...
	local nLastCount = 0
	local wndFullStats = wndHandler:LoadTooltipForm("CRB_MarketplaceCommodity.xml", "FullStatsFrame", self)
	for nRowIdx = 1, 3 do
		local strBuy = ""
...
function CRB_MarketplaceCommodity:OnGenerateTooltipListIcon(wndHandler, wndControl, eType, nX, nY)
...
function CRB_MarketplaceCommodity:OnCommodityInfoResults(nItemId, tStats, tOrders)
...
function CRB_MarketplaceCommodity:OnPostCommodityOrderResult(eAuctionPostResult, orderSource, nActualCost)
...
function CRB_MarketplaceCommodity:OnPostCustomMessage(strMessage, bResultOK, nDuration)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_MarketplaceCommodity:OnCommodityAuctionRemoved(eAuctionEventType, orderRemoved)
	-- TODO
...
function CRB_MarketplaceCommodity:OnPostResultTimer()
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_MarketplaceCommodity:OnCommodityDataReceived(tOrders) -- From MarketplaceLib.RequestOwnedCommodityOrders()
	self:UpdateOrderLimit(#tOrders)
end

function CRB_MarketplaceCommodity:UpdateOrderLimit(nCount)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
	if nCount < 0 then
...
function CRB_MarketplaceCommodity:OnOpenMarketListingsBtn(wndHandler, wndControl)
...
function CRB_MarketplaceCommodity:OnListIconMouseUp(wndHandler, wndControl, eMouseButton)
...
function CRB_MarketplaceCommodity:OnPostResultNotificationClick(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:Show(false)
	end
end

function CRB_MarketplaceCommodity:LoadByName(strForm, wndParent, strCustomName)
...
function CRB_MarketplaceCommodity:UpdateDisplayedAverage(wndAuction, nCount)
...
local CRB_MarketplaceCommodityInst = CRB_MarketplaceCommodity:new()
CRB_MarketplaceCommodityInst:Init()
