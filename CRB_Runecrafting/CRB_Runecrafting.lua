-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Runecrafting
...
local CRB_Runecrafting = {}

function CRB_Runecrafting:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_Runecrafting:Init()
    Apollo.RegisterAddon(self)
end

function CRB_Runecrafting:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Runecrafting.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Runecrafting:OnDocumentReady()
...
function CRB_Runecrafting:OnInterfaceMenuListHasLoaded()
...
function CRB_Runecrafting:OnOpenCraftingResume()
	self:Initialize()
	self:RedrawAll()
	self.tWindowMap["RerollItemTitle"]:SetText(Apollo.GetString("EngravingStation_RuneSlotRerollTitleResume"))
end

function CRB_Runecrafting:Initialize()
	if self.wndMain and self.wndMain:IsValid() then
		return
	end

    self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_RunecraftingForm", nil, self)
...
function CRB_Runecrafting:OnRunecrafting_TimerStationCheck() -- Hackish: These are async from the rest of the UI (and definitely can't handle data being set)
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_Runecrafting:OnCloseFromUI(wndHandler, wndControl)
	if wndHandler == wndControl then
		self:OnClose()
	end
end

function CRB_Runecrafting:OnClose()
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_Runecrafting:OnRedrawAllFromUI()
	self:RedrawAll()
end

function CRB_Runecrafting:OnRuneCreationElementToggle(wndHandler, wndControl)
...
function CRB_Runecrafting:RedrawAll(itemToOpenTo)
...
function CRB_Runecrafting:RedrawRerollBlocker(tCurrentCraft)
...
function CRB_Runecrafting:RedrawEquipment(itemToOpenTo)
...
function CRB_Runecrafting:RedrawCreation()
	...
	for idx, tCurrSchematic in pairs(CraftingLib.GetSchematicList(CraftingLib.CodeEnumTradeskill.CRB_Runecrafting, eElementFilter, nil, false)) do
...
function CRB_Runecrafting:DrawCreateRuneContents(wndParent)
...
function CRB_Runecrafting:OnTypeTierItemCheck(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	self:DrawCreateRuneContents(wndParent)
	self:ResizeCreateRunes()
end

function CRB_Runecrafting:OnTypeTierItemUncheck(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	wndParent:FindChild("TypeTierItemContainer"):DestroyChildren()
	self:ResizeCreateRunes()
end

function CRB_Runecrafting:ResizeCreateRunes()
...
function CRB_Runecrafting:OnUpdateInventory()
	self:HelperRefreshBottomMaterials()
	self:RedrawAll()
end

function CRB_Runecrafting:HelperRefreshBottomMaterials()
...
function CRB_Runecrafting:OnRuneCreationChatBtn(wndHandler, wndControl)
...
function CRB_Runecrafting:OnNewRuneMoreInfoCheck(wndHandler, wndControl) -- NewRuneMoreInfoBtn, data is tSchematicInfo
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Runecrafting:OnNewRuneMoreInfoUncheck(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Runecrafting:ResetRuneCreationBottom()
	local wndParent = self.wndMain:FindChild("RuneCreationContainer"):FindChild("RuneCreationBottom")
	wndParent:FindChild("RuneCreationMaterialsList"):DestroyChildren()
...
function CRB_Runecrafting:OnRuneCreationCraftBtn(wndHandler, wndControl) -- RuneCreationCraftBtn
	local tSchematicInfo = wndHandler:GetData()
...
function CRB_Runecrafting:OnCloseSummary()
...
function CRB_Runecrafting:NewEquipmentItem(itemSource, wndParent, bEquipped)
...
function CRB_Runecrafting:OnAppendConfirmYes(wndHandler, wndControl)
	local itemSource = wndHandler:GetData()
	CraftingLib.AddRuneSlot(itemSource)
	self:OnCurrentConfirmPopupClose()
end

function CRB_Runecrafting:OnRerollConfirmYes(wndHandler, wndControl)
...
function CRB_Runecrafting:OnRerollElementPickedBtn(wndHandler, wndControl) -- From RerollElementLeftBtn or RerollElementRightBtn
	self:HelperPostRerollTransition(self.tWindowMap["RerollDoneCastBarBG"])
end

function CRB_Runecrafting:OnRerollElementAbandonBtn(wndHandler, wndControl)
	self:HelperPostRerollTransition(self.tWindowMap["RerollDoneCastBarBG"])
end

function CRB_Runecrafting:HelperPostRerollTransition(wndCastBar)
	self.tWindowMap["RerollBlockerHoloBG"]:Show(false)
	self.tWindowMap["RerollBlocker"]:Show(true, true)
	wndCastBar:Show(true)
end

function CRB_Runecrafting:OnUnlockConfirmYes(wndHandler, wndControl)
	local nSlotIndex = wndHandler:GetData()[1]
	local itemSource = wndHandler:GetData()[3] -- Data is passed along, origates from RuneSlotBtn and is { nSlotIndex, tCurrRuneSlot, itemSource }
...
function CRB_Runecrafting:OnClearConfirmYes(wndHandler, wndControl)
	local nSlotIndex = wndHandler:GetData()[1]
	local itemSource = wndHandler:GetData()[3] -- Data is passed along, origates from RuneSlotBtn and is { nSlotIndex, tCurrRuneSlot, itemSource }
...
function CRB_Runecrafting:OnRuneSlotBtn(wndHandler, wndControl) -- RuneSlotBtn of RuneSlotLockedItem
...
function CRB_Runecrafting:OnRuneSlotAppendBtn(wndHandler, wndControl) -- RuneSlotAppendBtn of RuneSlotAppendItem
	if not self.bAllowClicks then
		return
	end
...
function CRB_Runecrafting:OnRuneSlotLockedBtn(wndHandler, wndControl) -- RuneSlotBtn of RuneSlotLockedItem
	if not self.bAllowClicks then
		return
	end
...
function CRB_Runecrafting:OnRunePickerItemBtn(wndHandler, wndControl)
...
function CRB_Runecrafting:DrawRuneRerollConfirm(wndHandler, itemSource, nSlotIndex)
	if not self.bAllowClicks then
		return
	end
...
function CRB_Runecrafting:DrawAddRuneConfirm(wndHandler, itemSource, nSlotIndex, itemRune)
	self:OnCurrentConfirmPopupClose()
...
function CRB_Runecrafting:DrawRuneReplaceConfirm(wndHandler, itemRune)
...
function CRB_Runecrafting:OnCurrentConfirmPopupClose(wndHandler, wndControl)
...
function CRB_Runecrafting:OnRunecrafting_ConfirmPopupDelay()
	self.bAllowClicks = true
end

function CRB_Runecrafting:HelperRepositionConfirmPopup(wndOrigin)
...
function CRB_Runecrafting:OnRuneSlotQueryDragDrop(wndHandler, wndControl, x, y, wndSource, strType, nDragInventoryItemIdx, eResult)
...
function CRB_Runecrafting:OnRuneSlotDragDrop(wndHandler, wndControl, x, y, wndSource, strType, nDragInventoryItemIdx, bDragDropHasBeenReset)
...
function CRB_Runecrafting:OnAddConfirmYes(wndHandler, wndControl) -- Potentially from drag drop or from picker
	local nSlotIndex = wndHandler:GetData()[1]
...
function CRB_Runecrafting:OnReplaceConfirmYes( wndHandler, wndControl, eMouseButton )
	local nSlotIndex = wndHandler:GetData()[1]
	local itemRune = wndHandler:GetData()[2] -- GOTCHA: This is an item object, not the normal table of data
	local itemSource = wndHandler:GetData()[3]
...
function CRB_Runecrafting:HelperCheckElementMatch(tItemRuneInfo, tRuneSlot)
...
function CRB_Runecrafting:HelperPrereqFailed(tCurrItem)
	return tCurrItem and tCurrItem:IsEquippable() and not tCurrItem:CanEquip()
end

function CRB_Runecrafting:HelperBuildItemTooltip(wndArg, itemCurr)
...
function CRB_Runecrafting:LoadByName(strForm, wndParent, strCustomName)
...
local CRB_RunecraftingInst = CRB_Runecrafting:new()
CRB_RunecraftingInst:Init()
