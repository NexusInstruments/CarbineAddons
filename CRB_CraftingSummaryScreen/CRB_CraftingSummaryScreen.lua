-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_CraftingSummaryScreen
...
local CRB_CraftingSummaryScreen = {}
...
function CRB_CraftingSummaryScreen:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_CraftingSummaryScreen:Init()
    Apollo.RegisterAddon(self)
end

function CRB_CraftingSummaryScreen:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CraftingSummaryScreen.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_CraftingSummaryScreen:OnDocumentReady()
	...
	self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_CraftingSummaryScreenForm", wndParent, self) -- Needs to start initialized so it can listen to messages
...
function CRB_CraftingSummaryScreen:OnCraftingSummary_StationTimer() -- Hackish: These are async from the rest of the UI (and definitely can't handle data being set)
...
function CRB_CraftingSummaryScreen:OnGenericEvent_StartCraftCastBar(wndParent, itemOutput)
	...
	self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_CraftingSummaryScreenForm", wndParent, self)
	self.tWndRefs.wndMain:Show(false, true)

	self.itemTooltipOverride = itemOutput -- Technically this is the most accurate
end

function CRB_CraftingSummaryScreen:OnGenericEvent_ClearCraftSummary()
	self.strOnGoingMessage = ""
	self.itemTooltipOverride = nil
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() then
...
function CRB_CraftingSummaryScreen:OnCraftingInterrupted()
...
function CRB_CraftingSummaryScreen:OnCraftingSchematicComplete(idSchematic, bPass, nEarnedXp, arMaterialReturnedIds, idSchematicCrafted, idItemCrafted) -- Main starting method
	...
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then -- GOTCHA: This is possible if the parent UI was deleted
		self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_CraftingSummaryScreenForm", nil, self)
	end
...
function CRB_CraftingSummaryScreen:OnCraftingSummaryRecraftBtn(wndHandler, wndControl) -- Data is idSchematic
	self:OnClose()
...
function CRB_CraftingSummaryScreen:OnCraftingSummaryOkBtn(wndHandler, wndControl)
...
function CRB_CraftingSummaryScreen:OnCraftingSummaryCloseBtn(wndHandler, wndControl)
	if wndHandler == wndControl then
		self:OnClose()
	end
end

function CRB_CraftingSummaryScreen:OnClose()
...
function CRB_CraftingSummaryScreen:HelperWriteToCraftingSummaryDetails(strMessage)
...
function CRB_CraftingSummaryScreen:OnCraftingDiscoveryHotCold(eHotCold, eDirection)
...
function CRB_CraftingSummaryScreen:OnTradeskillAchievementUpdate(achUpdated, nValueCurr, nValueNeeded)
...
function CRB_CraftingSummaryScreen:OnCraftingSchematicLearned(idTradeskill, idSchematic)
...
function CRB_CraftingSummaryScreen:OnTradeSkillFloater(unitAttach, strMessage)
	self:HelperWriteToCraftingSummaryDetails(strMessage)
end

function CRB_CraftingSummaryScreen:OnGenericEvent_CraftSummaryMsg(strMessage) -- From Lua
	self:HelperWriteToCraftingSummaryDetails(strMessage)
end

function CRB_CraftingSummaryScreen:OnGenericEvent_BotchCraft()
	self.bBotchCraft = true -- Skip entire UI if botch craft (e.g. Abandon Button)
end

local CRB_CraftingSummaryScreenInst = CRB_CraftingSummaryScreen:new()
CRB_CraftingSummaryScreenInst:Init()
