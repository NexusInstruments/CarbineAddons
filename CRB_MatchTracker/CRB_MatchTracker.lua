-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_MatchTracker
...
function CRB_MatchTracker:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_MatchTracker:Init()
    Apollo.RegisterAddon(self)
end

function CRB_MatchTracker:OnSave(eType)
...
function CRB_MatchTracker:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Account then
		return
	end
...
-----------------------------------------------------------------------------------------------
-- CRB_MatchTracker OnLoad
-----------------------------------------------------------------------------------------------

function CRB_MatchTracker:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MatchTracker.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_MatchTracker:OnDocumentReady()
...
function CRB_MatchTracker:ResetTracker()
	...
	self.tWndRefs.wndMatchTracker 	= Apollo.LoadForm(self.xmlDoc, "CRB_MatchTracker", "FixedHudStratum", self)
...
function CRB_MatchTracker:OnMatchEntered()
	if MatchingGame:IsInPVPGame() then
		Apollo.StartTimer("OneSecMatchTimer")
	end
end

function CRB_MatchTracker:OnMatchExited()
	Apollo.StopTimer("OneSecMatchTimer")
...
function CRB_MatchTracker:OnMatchPvpInactivityAlert(nRemainingTimeMs)
...
function CRB_MatchTracker:OnShowTeamAlert(peEvent)
...
function CRB_MatchTracker:OnHideTeamAlert()
...
function CRB_MatchTracker:OnOneSecMatchTimer()
...
function CRB_MatchTracker:OnPVPMatchFinished(eWinner, eReason)
	if self.wndHoldLineFloater and self.wndHoldLineFloater:IsValid() then
		self.locSavedWindowLoc = self.wndHoldLineFloater:GetLocation()
		self.wndHoldLineFloater:Destroy()
		self.wndHoldLineFloater = nil
...
function CRB_MatchTracker:OnMatchLeaveBtn(wndHandler, wndControl)
...
-- CTF Events
-----------------------------------------------------------------------------------------------
function CRB_MatchTracker:SetupFlags(tMatchState)
...
function CRB_MatchTracker:OnCTFFlagCollected(nArg) -- This is picking up a neutral flag
...
function CRB_MatchTracker:OnCTFFlagStolenDroppedCollected(nArg) -- This is someone picking up/relaying a stolen flag
	self:HelperCTFMinusOneFlag(LuaEnumTeam.Stolen)		--Stolen flag is no longer stolen
	self:HelperCTFPlusOneFlag(nArg == LuaEnumTeam.Blue and LuaEnumTeam.Blue or LuaEnumTeam.Red) --One side gains a flag
end

function CRB_MatchTracker:OnCTFFlagRecovered(nArg) -- This is a stolen flag despawning
	self:HelperCTFMinusOneFlag(LuaEnumTeam.Stolen)
end

function CRB_MatchTracker:OnCTFFlagSpawned(nArg)
	self:HelperCTFPlusOneFlag(LuaEnumTeam.Neutral)
end

function CRB_MatchTracker:OnCTFFlagDespawned()
	self:HelperCTFMinusOneFlag(LuaEnumTeam.Neutral)
end

function CRB_MatchTracker:OnCTFFlagDropped(nArg)
...
function CRB_MatchTracker:HelperCTFMinusOneFlag(nArg)
	local strWindowName = ktRavelIdToCTFWindowName[nArg]
	if strWindowName then
...
function CRB_MatchTracker:ShowAppropriateMessages()
...
function CRB_MatchTracker:DrawDeathmatchScreen()
	local tMatchState = MatchingGame:GetPVPMatchState()
...
function CRB_MatchTracker:DrawCTFScreen(peMatch)
	if not peMatch then
		return
	end
...
-- Hold The Line
-----------------------------------------------------------------------------------------------
function CRB_MatchTracker:HideHoldLineScreen()
...
function CRB_MatchTracker:DrawHoldLineScreen(peMatch)
...
function CRB_MatchTracker:OnHTLTimeToBeat(nSeconds, bAttacking)
	if self.wndHoldLineFloater and self.wndHoldLineFloater:IsValid() then
...
function CRB_MatchTracker:OnHoldLineMouseCatcherClick(wndHandler, wndControl)
	if wndHandler:GetData() then
		wndHandler:GetData():ShowHintArrow()
	end
end

function CRB_MatchTracker:OnHTLRespawn()
	if self.nHTLHintArrow then
		self.nHTLHintArrow:ShowHintArrow()
	end
end

function CRB_MatchTracker:OnHTLCaptureMod(nWhole, nDec)
	self.nHTLCaptureMod = nWhole + nDec / 100
end

function CRB_MatchTracker:OnHTLPrepPhase(bPreping)
...
function CRB_MatchTracker:OnPublicEventEnd(peEvent, eReason, tStats)
...
function CRB_MatchTracker:OnViewEventStatsBtn(wndHandler, wndControl) -- ViewEventStatsBtn
	local peDisplay = nil
...
function CRB_MatchTracker:HelperClearMatchesExcept(nShow) -- hides all other window types
...
function CRB_MatchTracker:HelperTimeString(nTimeInSeconds)
...
-----------------------------------------------------------------------------------------------
-- CRB_MatchTracker Instance
-----------------------------------------------------------------------------------------------
local CRB_MatchTrackerInst = CRB_MatchTracker:new()
