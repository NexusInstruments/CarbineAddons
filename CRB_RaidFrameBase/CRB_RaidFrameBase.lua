-------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_RaidFrameBase
-- Copyright (c) NCsoft. All rights reserved
-----------------------------------------------------------------------------------------------

require "Window"

local CRB_RaidFrameBase = {}
...
function CRB_RaidFrameBase:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_RaidFrameBase:Init()
    Apollo.RegisterAddon(self)
end

function CRB_RaidFrameBase:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_RaidFrameBase.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_RaidFrameBase:OnSave(eType)
...
function CRB_RaidFrameBase:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function CRB_RaidFrameBase:OnDocumentReady()
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_RaidFrameBaseForm", "FixedHudStratum", self)
...
function CRB_RaidFrameBase:OnWindowManagementReady()
...
function CRB_RaidFrameBase:OnCharacterCreated()
...
function CRB_RaidFrameBase:OnFrame()
	if not GroupLib.InRaid() then
		return
	end
...
function CRB_RaidFrameBase:DrawHealth(tWndMember, nHealthCurr, nHealthMax, nShieldCurr, nShieldMax, nAbsorbCurr, nAbsorbMax, nManaCurr, nManaMax, bInstantProgress)
...
function CRB_RaidFrameBase:CategorizeMembers()
	local tCategories = {}
...
function CRB_RaidFrameBase:BuildMembers()
	local nMemberCount = GroupLib.GetMemberCount()
...
function CRB_RaidFrameBase:BuildMemeberCategory(wndCategoryItems, arMembers)
	local arColumns = {}
...
function CRB_RaidFrameBase:BuildMember(wndMemberContainer, tMember)
...
function CRB_RaidFrameBase:OnRaidMemberBtnClick(wndHandler, wndControl, eMouseButton)
...
function CRB_RaidFrameBase:UpdateRaidOptions()
...
function CRB_RaidFrameBase:OnEnteredCombat(unitArg, bInCombat)
...
function CRB_RaidFrameBase:OnUnitCreated(unitNew)
	if not GroupLib.InRaid() then return end
...
function CRB_RaidFrameBase:OnUnitDestroyed(unitOld)
	if not GroupLib.InRaid() then return end
...
function CRB_RaidFrameBase:OnGroup_Join()
	if not GroupLib.InRaid() then return end
	self:OnCharacterCreated()
end

function CRB_RaidFrameBase:OnGroup_Add(strName)
	if not GroupLib.InRaid() then return end
	self:BuildMembers()
end

function CRB_RaidFrameBase:OnGroup_Remove(strName)
	if not GroupLib.InRaid() then return end
	
	self:OnCharacterCreated()
end

function CRB_RaidFrameBase:OnGroup_Left()
...
function CRB_RaidFrameBase:OnGroup_MemberFlagsChanged(nMemberIdx, bFromPromotion, tChangedFlags)
	if not GroupLib.InRaid() then return end
...
function CRB_RaidFrameBase:OnGroup_FlagsChanged()
	if not GroupLib.InRaid() then
...
function CRB_RaidFrameBase:OnGroup_SetMark(idMark, unitMarked)
	if not GroupLib.InRaid() then return end
...
function CRB_RaidFrameBase:OnRaidLockFrameBtnToggle(wndHandler, wndControl) -- RaidLockFrameBtn
	self.tSettings.bIsLocked = wndHandler:IsChecked()
	self:HelperUpdateSettings()
end

function CRB_RaidFrameBase:OnRaidCategoryBtnToggle(wndHandler, wndControl) -- RaidCategoryBtn
	self:BuildMembers()
end

function CRB_RaidFrameBase:OnRaidLeaderOptionsToggle(wndHandler, wndControl) -- RaidLeaderOptionsBtn
...
function CRB_RaidFrameBase:OnRaidMasterLootToggle(wndHandler, wndControl) -- RaidMasterLootBtn
...
function CRB_RaidFrameBase:OnRaidConfigureToggle(wndHandler, wndControl) -- RaidConfigureBtn
...
function CRB_RaidFrameBase:OnLeaveBtn(wndHandler, wndControl)
	self:OnUncheckLeaderOptions()
	self:OnUncheckMasterLoot()
	GroupLib.LeaveGroup()
end

function CRB_RaidFrameBase:OnRaidLeaveShowPrompt(wndHandler, wndControl)
...
function CRB_RaidFrameBase:OnRaidLeaveYes(wndHandler, wndControl)
	wndHandler:GetParent():Destroy()
	self:OnLeaveBtn()
end

function CRB_RaidFrameBase:OnRaidLeaveNo(wndHandler, wndControl)
	wndHandler:GetParent():Destroy()
end

function CRB_RaidFrameBase:OnUncheckLeaderOptions()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndRaidLeaderOptionsBtn:SetCheck(false)
	end
end

function CRB_RaidFrameBase:OnUncheckMasterLoot()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndRaidMasterLootBtn:SetCheck(false)
	end
end

function CRB_RaidFrameBase:OnRaidWrongInstance()
...
function CRB_RaidFrameBase:OnGroupBagBtn()
	Event_FireGenericEvent("GenericEvent_ToggleGroupBag")
end

function CRB_RaidFrameBase:OnGroup_LootRulesChanged()
	if not GroupLib.InRaid() then return end
	
	self:UpdateLootRules()
end

function CRB_RaidFrameBase:OnMasterLootUpdate()
	if not GroupLib.InRaid() then return end
...
function CRB_RaidFrameBase:UpdateLootRules()
...
-- Ready Check
-----------------------------------------------------------------------------------------------
function CRB_RaidFrameBase:OnReadyCheckChanged(wndHandler, wndControl, strText)
...
function CRB_RaidFrameBase:OnStartReadyCheckBtn(wndHandler, wndControl) -- StartReadyCheckBtn
...
function CRB_RaidFrameBase:OnReadyCheckTimeout()
	local bReadyCheckOnCooldown = GroupLib.IsReadyCheckOnCooldown()
	self.wndStartReadyCheckBtn:Enable(not bReadyCheckOnCooldown)
	self.wndReadyCheckMessageEditBox:SetPrompt(bReadyCheckOnCooldown and Apollo.GetString("RaidFrame_ReadyCheckOnCooldown") or Apollo.GetString("RaidFrame_AreYouReady"))
end
...
function CRB_RaidFrameBase:HelperUpdateSettings()
	local wndRaidOptions = self.wndMain:FindChild("RaidOptions:SelfConfigRaidCustomizeOptions")
...
function CRB_RaidFrameBase:OnConfigSetAsDPSToggle(wndHandler, wndControl)
	GroupLib.SetRoleDPS(1, wndHandler:IsChecked()) -- Will fire event Group_MemberFlagsChanged
end

function CRB_RaidFrameBase:OnConfigSetAsTankToggle(wndHandler, wndControl)
	GroupLib.SetRoleTank(1, wndHandler:IsChecked()) -- Will fire event Group_MemberFlagsChanged
end

function CRB_RaidFrameBase:OnConfigSetAsHealerToggle(wndHandler, wndControl)
	GroupLib.SetRoleHealer(1, wndHandler:IsChecked()) -- Will fire event Group_MemberFlagsChanged
end

function CRB_RaidFrameBase:OnRaidCustomizeCheck(wndHandler, wndControl, eMouseButton)
	self.tSettings[wndControl:GetData()] = wndControl:IsChecked()
	self:BuildMembers()
end

function CRB_RaidFrameBase:OnRaidCustomizeNumColAdd(wndHandler, wndControl)
...
function CRB_RaidFrameBase:OnRaidCustomizeNumColSub(wndHandler, wndControl)
	self.tSettings.nNumColumns = self.tSettings.nNumColumns - 1
...
function CRB_RaidFrameBase:OnRaidCustomizeRowSizeAdd(wndHandler, wndControl)
...
function CRB_RaidFrameBase:OnRaidCustomizeRowSizeSub(wndHandler, wndControl)
	self.tSettings.nRowSize = self.tSettings.nRowSize - 1
...
local CRB_RaidFrameBaseInst = CRB_RaidFrameBase:new()
CRB_RaidFrameBaseInst:Init()
