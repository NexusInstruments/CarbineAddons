-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Trading
...
function CRB_Trading:Init()
	Apollo.RegisterAddon(self)
end

function CRB_Trading:OnSave(eType)
...
function CRB_Trading:OnRestore(eType, tSavedData)
...
function CRB_Trading:OnLoad()
    self.xmlDoc = XmlDoc.CreateFromFile("CRB_TradingForms.xml")
    self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Trading:OnDocumentReady()
...
-------------------------------------------------------------------------------------
-- Handlers for CRB_Trading
-------------------------------------------------------------------------------------

function CRB_Trading:UpdateTrade()
...
function CRB_Trading:OnItemMouseButtonDown(wndHandler, wndControl, eButton, nX, nY, bDouble)
...
function CRB_Trading:OnP2PTradeChange()
	self:OnUpdate()
	self:UpdateTrade()
end
...
function CRB_Trading:OnP2PTradeInvite(unitInviter)
	self.wndTradeInvite:Invoke()
...
function CRB_Trading:OnP2PTradeResult(eResult)
...
function CRB_Trading:OnP2PCancelTrade(eResult)
	self:HelperResetItems()
	self:HelperResetTrade()
...
function CRB_Trading:OnAcceptTradeBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Trading:OnP2PTradeWithTarget(unitTarget, strType, itemData)
...
function CRB_Trading:OnP2PTradeQueryDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	if wndHandler ~= wndControl then
...
function CRB_Trading:OnP2PTradeDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_Trading:OnCommitCancel()
	if P2PTrading.AmICommitted() then
...
function CRB_Trading:OnCashAmountChanged(wndHandler, wndControl)
...
function CRB_Trading:OnP2PTradeCommit()
	self.wndTradeForm:SetFocus()
end

function CRB_Trading:OnCancelBtn()
	self:HelperResetItems()
	P2PTrading.CancelTrade()
end

function CRB_Trading:OnCraftingCloseWindow()
	self:HelperResetItems()
	self.wndTradeInvite:Show(false)
	P2PTrading.CancelTrade()
...
function CRB_Trading:OnReportTradeInviteSpamBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Trading:OnDeclineTradeBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
	self.wndTradeInvite:Show(false)
...
function CRB_Trading:OnP2PDeclineNotice(unitDecliner)
...
function CRB_Trading:OnDeclineAcknowledgeBtn(wndHandler, wndControl)
...
function CRB_Trading:OnErrorAcknowledgeBtn( wndHandler, wndControl, eMouseButton )
	self.wndTradeForm:Show(false)
...
function CRB_Trading:HelperResetTrade()
...
function CRB_Trading:HelperResetItems()
	for idx = 1, knMaxTradeItems do
...
function CRB_Trading:HelperBuildItemTooltip(wndArg, tItemData)
...
local CRB_TradingInstance = CRB_Trading:new()
CRB_Trading:Init()
