-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Warparty
...
function CRB_Warparty:Init()
    Apollo.RegisterAddon(self)
end

function CRB_Warparty:OnSave(eType)
...
function CRB_Warparty:OnRestore(eType, tSavedData)
...
function CRB_Warparty:OnLoad()
    self.xmlDoc = XmlDoc.CreateFromFile("CRB_Warparty.xml")
    self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Warparty:OnDocumentReady()
    ...
    self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_WarpartyForm", nil, self)
...
function CRB_Warparty:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementAdd", {wnd = self.wndMain, strName = Apollo.GetString("Warparty_Warparty")})
end

function CRB_Warparty:OnClose()
	Apollo.StopTimer("RetryLoadingGuilds")
	self.wndMain:Close()
end

-----------------------------------------------------------------------------------------------
-- CRB_Warparty Functions
-----------------------------------------------------------------------------------------------
function CRB_Warparty:OnShowWarpartyInfo(tPos)
...
function CRB_Warparty:ResetRosterMemberButtons()
...
-- The buttons
function CRB_Warparty:OnRosterAddMemberClick(wndHandler, wndControl)
...
function CRB_Warparty:OnRosterRemoveMemberClick(wndHandler, wndControl)
	self:ResetRosterMemberButtons()
...
function CRB_Warparty:OnWarpartyPublicClick(wndHandler, wndControl)
...
-- The Pop Up Bubbles
function CRB_Warparty:OnRosterAddMemberCloseBtn() -- The Window Close Event can also route here
	self.wndMain:FindChild("AddMemberEditBox"):SetText("")
	self.wndMain:FindChild("AddMemberContainer"):Show(false)
	self.wndMain:FindChild("RosterOptionBtnAdd"):SetCheck(false)
end

function CRB_Warparty:OnRosterRemoveMemberCloseBtn() -- The Window Close Event can also route here
	self.wndMain:FindChild("RemoveMemberContainer"):Show(false)
	self.wndMain:FindChild("RosterOptionBtnRemove"):SetCheck(false)
end

function CRB_Warparty:OnWarpartyPublicCloseBtn()
	self.wndMain:FindChild("PublicContainer"):Show(false)
	self:ResetRosterMemberButtons()
end

function CRB_Warparty:OnWarpartyPublicConfirmBtn(wndHandler, wndControl)
	local guildOwner = self.wndMain:FindChild("RosterOptionBtnPublic"):GetData()
	local tFlags = guildOwner and guildOwner:GetFlags()
	local bNewFlag = self.wndMain:FindChild("RosterOptionBtnPublic"):IsChecked()
	if tFlags and tFlags.bMercenary ~= bNewFlag then
...
function CRB_Warparty:OnRosterRemoveMemberYesClick(wndHandler, wndControl)
...
function CRB_Warparty:OnAddMemberEditBoxReturn(wndHandler, wndControl, strText)
...
function CRB_Warparty:OnAddMemberConfirmBtn(wndHandler, wndControl)
...
function CRB_Warparty:OnConfirmDisbandBtn(wndHandler, wndControl)
	if wndHandler and wndHandler:GetData() then
		wndHandler:GetData():Disband()
	end
end

function CRB_Warparty:OnDisbandContainerCloseBtn(wndHandler, wndControl)
	self.wndMain:FindChild("RosterDisbandBtn"):FindChild("DisbandContainer"):Show(false)
	self.wndMain:FindChild("RosterDisbandBtn"):SetCheck(false)
end

function CRB_Warparty:OnConfirmLeaveBtn(wndHandler, wndControl)
	if wndHandler and wndHandler:GetData() then
		wndHandler:GetData():Leave()
	end
end

function CRB_Warparty:OnLeaveContainerCloseBtn(wndHandler, wndControl)
	self.wndMain:FindChild("RosterLeaveBtn"):FindChild("LeaveContainer"):Show(false)
	self.wndMain:FindChild("RosterLeaveBtn"):SetCheck(false)
end

function CRB_Warparty:OnRosterLeaveBtn(wndHandler, wndControl)
	wndControl:FindChild("LeaveContainer"):Show(true)
end

function CRB_Warparty:OnRosterDisbandBtn(wndHandler, wndControl)
	wndControl:FindChild("DisbandContainer"):Show(true)
end

function CRB_Warparty:OnInviteToGroupClick(wndHandler, wndControl)
	local tMemberInfo = self.wndMain:FindChild("RosterGrid"):GetData()
...
function CRB_Warparty:OnGuildRoster(guildOwner, tRoster) -- Event from CPP
...
function CRB_Warparty:BuildRosterList(guildOwner, tRoster)
...
function CRB_Warparty:OnRosterGridItemClick(wndControl, wndHandler, iRow, iCol)
...
function CRB_Warparty:OnGuildPvp(guildCurrent)
	if guildCurrent == self.wndMain:GetData() then
		self:UpdatePvpRating()
	end
end

function CRB_Warparty:UpdatePvpRating()
...
-----------------------------------------------------------------------------------------------
-- CRB_Warparty Invite Window
-----------------------------------------------------------------------------------------------

function CRB_Warparty:OnGuildInvite( strGuildName, strInvitorName, eGuildType )
...
function CRB_Warparty:OnWarpartyInviteAccept(wndHandler, wndControl)
	GuildLib.Accept()
...
function CRB_Warparty:OnReportWarpartyInviteSpamBtn()
	Event_FireGenericEvent("GenericEvent_ReportPlayerWarpartyInvite")
	self.wndWarpartyInvite:Destroy()
end

function CRB_Warparty:OnDecline()
	if self.wndWarpartyInvite then
		self.wndWarpartyInvite:Destroy()
	end
	GuildLib.Decline()
...
function CRB_Warparty:OnGuildFlags(guildCurrent)
...
function CRB_Warparty:OnGuildMemberChange(guildCurrent)
	if self.bOkayToReload and (guildCurrent:GetType() == GuildLib.GuildType_WarParty) and self.wndMain:IsShown() then
		self.wndMain:Show(false)
		self:OnShowWarpartyInfo()
		self.bOkayToReload = false
...
function CRB_Warparty:OnReloadCooldownTimer()
	Apollo.StopTimer("ReloadCooldownTimer")
	self.bOkayToReload = true
end

function CRB_Warparty:OnGuildResult( guildSender, strName, nRank, eResult)
...
function CRB_Warparty:HelperConvertToTime(nDays)
	if nDays == 0 then 
...
function CRB_Warparty:OnRanksButtonSignal()
	local wndRankPopout = self.wndMain:FindChild("RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
...
function CRB_Warparty:OnAddRankBtnSignal(wndControl, wndHandler)
	local wndRankPopout = self.wndMain:FindChild("RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
...
function CRB_Warparty:OnRemoveRankBtnSignal(wndControl, wndHandler)
...
function CRB_Warparty:OnRankSettingsSaveBtn(wndControl, wndHandler)
	local wndRankPopout = self.wndMain:FindChild("RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
	local wndSettings = wndRankPopout:FindChild("RankSettingsEntry")
...
function CRB_Warparty:OnRankSettingsDeleteBtn(wndControl, wndHandler)
	local wndRankPopout = self.wndMain:FindChild("RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
...
function CRB_Warparty:OnRankSettingsNameChanging(wndControl, wndHandler, strText)
	self:HelperValidateAndRefreshRankSettingsWindow(self.wndRankPopout:FindChild("RankSettingsEntry"))
end

function CRB_Warparty:OnRankSettingsPermissionBtn(wndControl, wndHandler)
	self:HelperValidateAndRefreshRankSettingsWindow(self.wndRankPopout:FindChild("RankSettingsEntry"))
end

function CRB_Warparty:HelperValidateAndRefreshRankSettingsWindow(wndSettings)
...
function CRB_Warparty:OnRankSettingsCloseBtn(wndControl, wndHandler)
	local wndRankPopout = self.wndMain:FindChild("RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
	local wndSettings = wndRankPopout:FindChild("RankSettingsEntry")
...
function CRB_Warparty:OnGuildRankChange(guildCurr)
...
function CRB_Warparty:OnRankPopoutCloseBtn(wndControl, wndHandler)
	local wndParent = wndControl:GetParent()
	wndParent:Show(false)
end

---------------------------------------------------------------------------------------------------
-- CRB_WarpartyForm Functions
---------------------------------------------------------------------------------------------------

function CRB_Warparty:OnRosterPromoteMemberCloseBtn( wndHandler, wndControl, eMouseButton )
	self.wndMain:FindChild("PromoteMemberContainer"):Show(false)
	self.wndMain:FindChild("RosterOptionBtnPromote"):SetCheck(false)
end

function CRB_Warparty:OnRosterDemoteMemberClick( wndHandler, wndControl, eMouseButton )
	local guildCurr = self.wndMain:GetData()
	local tMember = self.wndMain:FindChild("RosterGrid"):GetData()
	guildCurr:Demote(tMember.strName)
end

function CRB_Warparty:OnRosterPromoteMemberClick( wndHandler, wndControl, eMouseButton )
...
function CRB_Warparty:OnRosterPromoteMemberYesClick( wndHandler, wndControl, eMouseButton )
	local guildCurr = self.wndMain:GetData()
	local tMember = self.wndMain:FindChild("RosterGrid"):GetData()
...
-----------------------------------------------------------------------------------------------
-- CRB_Warparty Instance
-----------------------------------------------------------------------------------------------
local CRB_WarpartyInst = CRB_Warparty:new()
CRB_WarpartyInst:Init()
