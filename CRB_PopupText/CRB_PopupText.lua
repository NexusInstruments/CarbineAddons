-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_PopupText
...
function CRB_PopupText:Init()
    Apollo.RegisterAddon(self)
end

function CRB_PopupText:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_PopupText.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_PopupText:OnDocumentReady()
...
function CRB_PopupText:OnCharacterCreated()
...
function CRB_PopupText:OnPopupText_ShowEpisodeAlert(strArgMessage)
...
function CRB_PopupText:OnPopupText_ShowPathAlert(strArgMessage, tContent) -- Arguments are legacy
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
	end
...
--[[ Old
function CRB_PopupText:OnPopupText_DestroyAnimationStepOne()
	if self.wndMain and self.wndMain:IsValid() then
		local nLeft, nTop, nRight, nBottom = self.wndMain:GetAnchorOffsets()
...
function CRB_PopupText:OnPopupText_DestroyAnimationStepTwo()
	if self.wndMain and self.wndMain:IsValid() then
		local nLeft, nTop, nRight, nBottom = self.wndMain:GetAnchorOffsets()
...
function CRB_PopupText:OnPopupText_DestroyAnimationStepThree_Datachron()
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_PopupText:OnPopupText_DestroyAnimationStepThree_QuestLog()
	if self.wndMain and self.wndMain:IsValid() then
...
local CRB_PopupTextInst = CRB_PopupText:new()
CRB_PopupTextInst:Init()
