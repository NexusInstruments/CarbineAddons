...
function CRB_ArenaTeam:Init()
    Apollo.RegisterAddon(self)
end

function CRB_ArenaTeam:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ArenaTeam.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_ArenaTeam:OnDocumentReady()
	...
	-- loading windows
    self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_ArenaTeamForm", nil, self)
	self.wndMain:Show(false, true)
end

function CRB_ArenaTeam:OnWindowManagementReady()
	Event_FireGenericEvent("WindowManagementAdd", {wnd = self.wndMain, strName = Apollo.GetString("Guild_GuildTypeArena"), nSaveVersion=2})
end

function CRB_ArenaTeam:OnClose()
	self.eCurrentType = nil
	self.timerArenaTeamUpdate:Stop()
	self.wndMain:Close()
end

function CRB_ArenaTeam:RequestUpdate()
...
-----------------------------------------------------------------------------------------------
-- CRB_ArenaTeam Functions
-----------------------------------------------------------------------------------------------
function CRB_ArenaTeam:OnShowArenaInfo(eTeamType, tPos)
...
function CRB_ArenaTeam:ResetRosterMemberButtons()
...
-- The buttons
function CRB_ArenaTeam:OnRosterAddMemberClick(wndHandler, wndControl)
...
function CRB_ArenaTeam:OnRosterRemoveMemberClick(wndHandler, wndControl)
	self:ResetRosterMemberButtons()
...
-- The Pop Up Bubbles
function CRB_ArenaTeam:OnRosterAddMemberCloseBtn() -- The Window Close Event can also route here
	self.wndMain:FindChild("AddMemberEditBox"):SetText("")
	self.wndMain:FindChild("AddMemberContainer"):Close()
	self.wndMain:FindChild("RosterOptionBtnAdd"):SetCheck(false)
end

function CRB_ArenaTeam:OnRosterRemoveMemberCloseBtn()
	self.wndMain:FindChild("RemoveMemberContainer"):Close()
	self.wndMain:FindChild("RosterOptionBtnRemove"):SetCheck(false)
end

function CRB_ArenaTeam:OnRosterRemoveMemberYesClick(wndHandler, wndControl)
...
function CRB_ArenaTeam:OnAddMemberEditBoxReturn(wndHandler, wndControl, strText)
...
function CRB_ArenaTeam:OnAddMemberConfirmBtn(wndHandler, wndControl)
...
function CRB_ArenaTeam:OnConfirmDisbandBtn(wndHandler, wndControl)
	if wndHandler and wndHandler:GetData() then
		wndHandler:GetData():Disband()
	end
end

function CRB_ArenaTeam:OnDisbandContainerCloseBtn(wndHandler, wndControl)
	local wndDisband = self.wndMain:FindChild("RosterDisbandBtn")
	wndDisband:FindChild("DisbandContainer"):Show(false)
	wndDisband:SetCheck(false)
end

function CRB_ArenaTeam:OnConfirmLeaveBtn(wndHandler, wndControl)
	if wndHandler and wndHandler:GetData() then
...
function CRB_ArenaTeam:OnLeaveContainerCloseBtn(wndHandler, wndControl)
	local wndLeave = self.wndMain:FindChild("RosterLeaveBtn")
	wndLeave:FindChild("LeaveContainer"):Show(false)
	wndLeave:SetCheck(false)
end

function CRB_ArenaTeam:OnRosterLeaveBtn(wndHandler, wndControl)
	self:ResetRosterMemberButtons()
	wndControl:FindChild("LeaveContainer"):Invoke()
end

function CRB_ArenaTeam:OnRosterDisbandBtn(wndHandler, wndControl)
	self:ResetRosterMemberButtons()
	wndControl:FindChild("DisbandContainer"):Invoke()
end

function CRB_ArenaTeam:OnInviteToGroupClick(wndHandler, wndControl)
	self:ResetRosterMemberButtons()
	local wndRoster = self.wndMain:FindChild("RosterGrid")
	local tSelected = wndRoster:GetData()
...
function CRB_ArenaTeam:OnGuildRoster(guildCurr, tRoster)
...
function CRB_ArenaTeam:OnRosterGridItemClick(wndControl, wndHandler, iRow, iCol)
	local wndRoster = self.wndMain:FindChild("RosterGrid")
...
function CRB_ArenaTeam:OnGuildPvp(guildSelected)
	if guildSelected == self.wndMain:GetData() then
		self:UpdatePvpRating()
	end
end

function CRB_ArenaTeam:UpdatePvpRating()
...
-----------------------------------------------------------------------------------------------
-- CRB_ArenaTeam Invite Window
-----------------------------------------------------------------------------------------------

function CRB_ArenaTeam:OnGuildInvite( strGuildName, strInvitorName, eGuildType )
...
function CRB_ArenaTeam:OnArenaTeamInviteAccept(wndHandler, wndControl)
	GuildLib.Accept()
...
function CRB_ArenaTeam:OnReportArenaInviteSpamBtn(wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_ReportPlayerArenaInvite")
	self.wndArenaTeamInvite:Destroy()
end

function CRB_ArenaTeam:OnDecline()
	GuildLib.Decline()
	if self.wndArenaTeamInvite then
		self.wndArenaTeamInvite:Destroy()
	end
...
function CRB_ArenaTeam:OnGuildMemberChange(guildCurr)
...
function CRB_ArenaTeam:OnGuildResult( guildTeam, strName, nRank, eResult )
...
function CRB_ArenaTeam:HelperConvertToTime(nDays)
	if nDays == 0 then 
...
-----------------------------------------------------------------------------------------------
-- CRB_ArenaTeam Instance
-----------------------------------------------------------------------------------------------
local CRB_ArenaTeamInst = CRB_ArenaTeam:new()
CRB_ArenaTeamInst:Init()
