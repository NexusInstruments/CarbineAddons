-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_TradeskillTrainer
...
function CRB_TradeskillTrainer:Init()
    Apollo.RegisterAddon(self)
end

function CRB_TradeskillTrainer:OnLoad()
    self.xmlDoc = XmlDoc.CreateFromFile("CRB_TradeskillTrainer.xml")
    self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_TradeskillTrainer:OnDocumentReady()
...
function CRB_TradeskillTrainer:OnInvokeTradeskillTrainer(unitTrainer)
	if not self.wndMain or not self.wndMain:IsValid() then
		self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_TradeskillTrainerForm", nil, self)
...
function CRB_TradeskillTrainer:OnClose()
...
function CRB_TradeskillTrainer:OnWindowClosed(wndHandler, wndControl)
	self:OnClose()
end

function CRB_TradeskillTrainer:OnProfListItemClick(wndHandler, wndControl) -- wndHandler is "ListItemBtn", data is tradeskill id
...
function CRB_TradeskillTrainer:OnHobbyListItemClick(wndHandler, wndControl)
...
function CRB_TradeskillTrainer:OnLearnTradeskillBtn(wndHandler, wndControl)
...
function CRB_TradeskillTrainer:OnSwapTradeskillBtn(wndHandler, wndControl) --SwapTradeskillBtn1 or SwapTradeskillBtn2, data is nTradeskillId
	if not wndHandler or not wndHandler:GetData() then
		return
	end
...
local CRB_TradeskillTrainerInst = CRB_TradeskillTrainer:new()
CRB_TradeskillTrainerInst:Init()
