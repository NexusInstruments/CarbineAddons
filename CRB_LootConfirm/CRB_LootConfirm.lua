...
-----------------------------------------------------------------------------------------------
-- CRB_LootConfirm Module Definition
-----------------------------------------------------------------------------------------------
local CRB_LootConfirm = {}
...
-- Initialization
-----------------------------------------------------------------------------------------------
function CRB_LootConfirm:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
...
function CRB_LootConfirm:Init()
    Apollo.RegisterAddon(self)
end

-----------------------------------------------------------------------------------------------
-- CRB_LootConfirm OnLoad
-----------------------------------------------------------------------------------------------
function CRB_LootConfirm:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_LootConfirm.xml")
...
function CRB_LootConfirm:OnDocumentReady()
...
function CRB_LootConfirm:OnWindowManagementReady()
...
function CRB_LootConfirm:OnLootBindCheck(tLootDetail)
	if tLootDetail then
...
function CRB_LootConfirm:OnLootTakenBy(tLootDetail)
...
function CRB_LootConfirm:OnLootListItemCheck(wndHandler, wndControl)
...
function CRB_LootConfirm:OnLootListItemGenerateTooltip(wndControl, wndHandler) -- wndHandler is VendorListItemIcon
...
function CRB_LootConfirm:RedrawAll()
	local nTotalItems = 0
	local itemCurr = nil
...
function CRB_LootConfirm:HelperPrereqFailed(tCurrItem)
	return tCurrItem and tCurrItem:IsEquippable() and not tCurrItem:CanEquip()
end

function CRB_LootConfirm:OnLootCurr()
...
function CRB_LootConfirm:OnLootBtnMouseEnter(wndHandler, wndControl)
	self.timerRefresh:Stop()
end

function CRB_LootConfirm:OnLootBtnMouseExit(wndHandler, wndControl)
	if self.wndMain:IsShown() then
		self.timerRefresh:Start()
	end
end

function CRB_LootConfirm:OnCloseBtn()	
	self.wndMain:Close()
end

function CRB_LootConfirm:OnClose()
	self.arLootList = {}
	self.arItemList = {}
	self.nItemIndex = 1
...
local CRB_LootConfirmInst = CRB_LootConfirm:new()
