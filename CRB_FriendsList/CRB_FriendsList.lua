-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_FriendsList
...
-- Initialization
-----------------------------------------------------------------------------------------------
function CRB_FriendsList:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_FriendsList:Init()
	Apollo.RegisterAddon(self)
end

-----------------------------------------------------------------------------------------------
-- CRB_FriendsList OnLoad
-----------------------------------------------------------------------------------------------
function CRB_FriendsList:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_FriendsList.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_FriendsList:OnDocumentReady()
...
function CRB_FriendsList:OnGenericEvent_InitializeFriends(wndParent)
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() then
		return
	end

    self.tWndRefs.wndMain 					= Apollo.LoadForm(self.xmlDoc, "CRB_FriendsListForm", wndParent, self)
...
self.tWndRefs.wndMain:FindChild("OptionsBtn"):AttachWindow(self.tWndRefs.wndMain:FindChild("AdvancedOptionsContainer"))
	...
	self:OnCRB_FriendsListOn()
end

function CRB_FriendsList:OnGenericEvent_DestroyFriends()
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() then
...
function CRB_FriendsList:OnSave(eType)
...
function CRB_FriendsList:OnRestore(eType, tSavedData)
...
-----------------------------------------------------------------------------------------------
-- CRB_FriendsList Functions
-----------------------------------------------------------------------------------------------
-- Define general functions here
function CRB_FriendsList:OnTopLevelWindowMove(wndControl, nOldLeft, nOldTop, nOldRight, nOldBottom)
...
function CRB_FriendsList:OnFriendshipLoaded()
...
function CRB_FriendsList:OnCRB_FriendsListOn()
...
-----------------------------------------------------------------------------------------------
function CRB_FriendsList:OnFriendshipAdd(nFriendId)
...
function CRB_FriendsList:OnFriendshipUpdate(nFriendId) -- Gotcha: will fire for every list the player is on, so removing might also have this event
...
function CRB_FriendsList:OnFriendshipUpdateOnline(nFriendId)
...
function CRB_FriendsList:OnFriendshipRemove(nFriendId) -- removes from all lists
...
function CRB_FriendsList:OnFriendshipRemoveFromList(nFriendId, nTab) -- removes from specific list
...
function CRB_FriendsList:OnUpdateLocationsTimer()
...
function CRB_FriendsList:OnFriendshipLocation(tLocations)
...
-----------------------------------------------------------------------------------------------
function CRB_FriendsList:OnFriendshipSuggestedAdd(nSuggestedId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipSuggestedUpdate(nSuggestedId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipSuggestedRemove(nSuggestedId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:DrawList(nList) -- pass the list we're updating so we can call against all of them
...
function CRB_FriendsList:HelperAddOrUpdateMemberWindow(nFriendId, tFriend)
...
function CRB_FriendsList:HelperRemoveMemberWindow(nFriendId, nList)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:DrawControls(nList)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_FriendsList:OnFriendTabBtn(wndHandler, wndControl)
...
function CRB_FriendsList:UpdateControls()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_FriendsList:UpdateControlsSuggested()
	local tFriend = nil
...
function CRB_FriendsList:HelperDrawMemberWindowFriend(wndPlayerInfo, tFriend)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_FriendsList:HelperDrawMemberWindowFriendInviteRequest(wnd, tInvite)
    if tInvite == nil then
        return
    end
...
function CRB_FriendsList:HelperDrawMemberWindowIgnore(wndPlayerInfo, tFriend)
	wndPlayerInfo:FindChild("Faction"):SetText(ktFaction[tFriend.nFactionId])
	self:HelperFormatNote(wndPlayerInfo:FindChild("NoteIcon"), tFriend.strNote)
end

function CRB_FriendsList:HelperDrawMemberWindowAccountFriend(wnd, tFriend)
...
function CRB_FriendsList:HelperDrawMemberWindowAccountFriendInviteRequest(wnd, tInvite)
    if tInvite == nil then
        return
    end
...
function CRB_FriendsList:HelperDrawMemberWindowRival(wndPlayerInfo, tFriend)
	self:HelperFormatPathAndClass(wndPlayerInfo, tFriend.nClassId, tFriend.nPathId)
	self:HelperFormatNote(wndPlayerInfo:FindChild("NoteIcon"), tFriend.strNote)
...
function CRB_FriendsList:HelperDrawMemberWindowSuggested(wndPlayerInfo, tFriend)
	self:HelperFormatPathAndClass(wndPlayerInfo, tFriend.nClassId, tFriend.nPathId)
	wndPlayerInfo:FindChild("Level"):SetText(tFriend.nLevel)
...
function CRB_FriendsList:HelperFormatNote(wndNoteIcon, strNote)
	wndNoteIcon:Show(strNote ~= nil and strNote ~= "")
	wndNoteIcon:SetTooltip(strNote or "")
end

function CRB_FriendsList:HelperFormatPathAndClass(wndEntry, nClassId, nPathId)
...
function CRB_FriendsList:HelperConvertToTime(nDays)
...
function CRB_FriendsList:HelperConvertToTimeTilExpired(nDays)
	local tTimeData =
	{
		["name"]	= "",
		["count"]	= nil,
...
-----------------------------------------------------------------------------------------------
-- CRB_FriendsListForm Button Functions
-----------------------------------------------------------------------------------------------

function CRB_FriendsList:OnAccountFriendBtn(wndHandler, wndControl, eMouseButton) -- TODO REFACTOR: Can just store the name instead
...
function CRB_FriendsList:OnFriendBtn(wndHandler, wndControl, eMouseButton)
	local idFriend = wndControl:GetParent():GetData()
	if idFriend == nil then
		return false
	end
...
function CRB_FriendsList:OnFriendBtnUncheck(wndHandler, wndControl, eMouseButton)
	if eMouseButton == GameLib.CodeEnumInputMouse.Right then
...
function CRB_FriendsList:OnAddBtn(wndHandler, wndControl)
...
function CRB_FriendsList:OnAddPlayerTextChanged(wndHandler, wndControl, strText)
...
function CRB_FriendsList:OnFriendAddConfrimBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then return end
...
function CRB_FriendsList:OnAddRadioBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then return end
...
function CRB_FriendsList:OnNoteWindowEdit(wndHandler, wndControl, strNewNote)
...
function CRB_FriendsList:OnNoteConfirmBtn(wndHandler, wndControl)
	local wndNote = wndControl:GetParent()
	local tFriend = wndNote:GetData()
...
function CRB_FriendsList:OnBlockBtn(wndHandler, wndControl)
	local bBlocked = not wndControl:IsChecked() -- art is reversed
	FriendshipLib.SetLock(bBlocked)
end

function CRB_FriendsList:OnAccountBlockBtn(wndHandler, wndControl)
	local bBlocked = not wndControl:IsChecked() -- art is reversed
...
function CRB_FriendsList:OnSelfNoteConfirmBtn( wndHandler, wndControl)
	if wndHandler ~= wndControl then return end
...
function CRB_FriendsList:OnUpdateStatusDelayTimer( wndHandler, wndControl)
	FriendshipLib.SetPublicNote(self.strPublicNote)
end

function CRB_FriendsList:OnSelfNoteWindowEdit( wndHandler, wndControl, strNewNote )
	if wndHandler ~= wndControl then return end
	local wndNote = wndControl:GetParent()
	local tFriend = wndNote:GetData()
...
function CRB_FriendsList:OnNameConfirmBtn( wndHandler, wndControl)
	local wndNote = wndControl:GetParent()
	FriendshipLib.SetPublicDisplayName(wndNote:FindChild("NameEditBox"):GetText())
	self.tWndRefs.wndMain:FindChild("AdvancedOptionsContainer"):Close()
end

function CRB_FriendsList:OnNameWindowEdit( wndHandler, wndControl, strText )
	local wndNote = wndControl:GetParent()
...
function CRB_FriendsList:OnNameBtn( wndHandler, wndControl)
	local tFriend = wndControl:GetData()
...
function CRB_FriendsList:OnChangeAccountStatusBtn( wndHandler, wndControl)
...
function CRB_FriendsList:OnRemoveFriendConfirmBtn(wndHandler, wndControl)
...
function CRB_FriendsList:OnStatusDropdownBtn( wndHandler, wndControl)
	if wndHandler ~= wndControl then return end
	local nPresenceState = FriendshipLib.GetPersonalStatus().nPresenceState
...
function CRB_FriendsList:OnStatusDropdownOptionBtn(wndHandler, wndControl)
...
function CRB_FriendsList:OnEventGeneric_ConfirmRemoveAccountFriend(nFriendId)
...
function CRB_FriendsList:OnFriendSortToggle(wndHandler, wndControl, eMouseButton)
...
function CRB_FriendsList:PerformLastSort()
	local eCurrentTab = self.tWndRefs.wndListContainer:GetData()
	local tLastSort = self.arLastSorts[eCurrentTab]
	local strLastSort = tLastSort[2]
...
function CRB_FriendsList:SortDefaultOther(wndLeft, wndRight)
	return self:SortName(false, wndLeft, wndRight)
end

function CRB_FriendsList:SortDefaultFriends(wndLeft, wndRight)
...
function CRB_FriendsList:SortType(bDesc, wndLeft, wndRight)
	local strLeft = wndLeft:GetName()
	local strRight = wndRight:GetName()
...
function CRB_FriendsList:SortName(bDesc, wndLeft, wndRight)
...
function CRB_FriendsList:SortLevel(bDesc, wndLeft, wndRight)
...
function CRB_FriendsList:SortClass(bDesc, wndLeft, wndRight)
	local strNameLeft = wndLeft:GetName()
	local strNameRight = wndRight:GetName()
...
function CRB_FriendsList:SortPath(bDesc, wndLeft, wndRight)
	local strNameLeft = wndLeft:GetName()
	local strNameRight = wndRight:GetName()
...
function CRB_FriendsList:SortStatus(bDesc, wndLeft, wndRight)
	local strLeft = wndLeft:GetName()
	local strRight = wndRight:GetName()
...
function CRB_FriendsList:SortNote(bDesc, wndLeft, wndRight)
	local strNameLeft = wndLeft:GetName()
	local strNameRight = wndRight:GetName()
...
function CRB_FriendsList:OnFriendshipResult(strName, eResult)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_FriendsList:OnFriendshipRequest(tRequest)
...
function CRB_FriendsList:OnFriendshipRequestWithdrawn(tRequest, eResult)
	self.wndRequest:Show(false)
...
function CRB_FriendsList:OnButtonInitialYes(wndHandler, wndControl)
...
function CRB_FriendsList:OnButtonInitialNo(wndHandler, wndControl)
	local wndInvite = wndControl:GetParent():GetParent()
...
function CRB_FriendsList:OnAccept( wndHandler, wndControl)
	local wndInvite = wndControl:GetParent():GetParent()
...
function CRB_FriendsList:OnAccountFriendInviteBtn(wndHandler, wndControl)
...
function CRB_FriendsList:OnAccountFriendInviteBtnUncheck(wndHandler, wndControl)
	self.wndSelectedFriend = nil
	self:UpdateControls()
end

function CRB_FriendsList:OnAcceptAccountFriendRequest(wndHandler, wndControl)
...
function CRB_FriendsList:OnRejectAccountFriendRequest(wndHandler, wndControl)
	local wndInvite = wndControl:GetParent()
    local tInviteId = wndInvite:GetData()
    if tInviteId ~= nil then
...
function CRB_FriendsList:OnReportAccountInviteSpamBtn(wndHandler, wndControl)
	local wndInvite = wndControl:GetParent()
    local tInviteId = wndInvite:GetData()
    if tInviteId ~= nil then
        --FriendshipLib.AccountInviteRespond(tInviteId, false)
...
function CRB_FriendsList:OnFriendshipInvitesRecieved(tInviteList)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipInviteRemoved(nId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipAccountInvitesRecieved(tInviteList)
...
function CRB_FriendsList:OnFriendshipAccountInviteRemoved(nId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipAccountFriendsRecieved(tFriendAccountList)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipAccountFriendRemoved(nId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_FriendsList:OnFriendshipAccountDataUpdate(nId)
	self:FriendshipAccountUpdate(nId)
end

function CRB_FriendsList:OnFriendshipAccountCharacterLevelUpdate(nId)
	self:FriendshipAccountUpdate(nId)
	-- TODO We will want to hook up something here to notify the player a friend just level'ed up
end

function CRB_FriendsList:OnFriendshipAccountPersonalStatusUpdate()
	self:UpdateControls()
end
...
function CRB_FriendsList:GetFriendshipWindowByFriendId(nId)
...
function CRB_FriendsList:FriendshipAccountUpdate(nId)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_FriendsList:OnUpdateLastOnlineTimer()
 	if self.tWndRefs.wndListContainer:GetData() ~= LuaCodeEnumTabTypes.Friend then
		return
	end
...
function CRB_FriendsList:OnFriendInviteBtn( wndHandler, wndControl)
	local tInviteId = wndControl:GetParent():GetData()
	if tInviteId == nil then
		return false
	end
...
function CRB_FriendsList:OnFriendInviteBtnUncheck(wndHandler, wndControl)
	self.wndSelectedFriend = nil
	self:UpdateControls()
end
...
function CRB_FriendsList:OnConfirmRemoveAccountFriendFormNo( wndHandler, wndControl)
...
function CRB_FriendsList:OnConfirmRemoveAccountFriendFormYes( wndHandler, wndControl)
	local wndParent = wndControl:GetParent()
	if wndParent == nil then
		return
	end
...
-----------------------------------------------------------------------------------------------
-- CRB_FriendsList Text Validation
-----------------------------------------------------------------------------------------------

function CRB_FriendsList:FactoryProduce(wndParent, strFormName, tObject)
...
function CRB_FriendsList:OnOfflineBtn( wndHandler, wndControl, eMouseButton )
end

-----------------------------------------------------------------------------------------------
-- CRB_FriendsList Instance
-----------------------------------------------------------------------------------------------
local CRB_FriendsListInst = CRB_FriendsList:new()
CRB_FriendsListInst:Init()
