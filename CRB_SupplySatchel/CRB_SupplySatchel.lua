-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_SupplySatchel
-- Copyright (c) NCsoft. All rights reserved
-----------------------------------------------------------------------------------------------

require "Window"

local CRB_SupplySatchel = {}
...
function CRB_SupplySatchel:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
	return o
...
function CRB_SupplySatchel:Init()
	Apollo.RegisterAddon(self, false, "", {"Util"})
end

function CRB_SupplySatchel:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_SupplySatchel.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_SupplySatchel:OnDocumentReady()
...
-----------------------------------------------------------------------------------------------
-- CRB_SupplySatchelForm Functions
-----------------------------------------------------------------------------------------------
function CRB_SupplySatchel:OnLootstackItemSentToTradeskillBag(tItem)
	if tItem then
...
function CRB_SupplySatchel:OnToggleVisibility(wndHandler, wndControl, eMouseButton)
...
function CRB_SupplySatchel:InitializeSatchel()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
	end

	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_SupplySatchelForm", nil, self)
...
function CRB_SupplySatchel:OnInitializeSatchelPart2()
...
function CRB_SupplySatchel:OnUnloadSatchel()
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
	end
...
function CRB_SupplySatchel:OnUpdateInventory()
	if self.wndMain and self.wndMain:IsShown() then
		self:PopulateSatchel(false)
	end
end

function CRB_SupplySatchel:OnToggleTradeskillInventoryFromBag(tAnchors)
	if not self.wndMain or not self.wndMain:IsValid() then
		self:InitializeSatchel()
	end
end

function CRB_SupplySatchel:OnShowAll( wndHandler, wndControl, eMouseButton )
	self:PopulateSatchel(true)
end

function CRB_SupplySatchel:OnResize()
...
function CRB_SupplySatchel:ResizeCategory(tCat)
...
function CRB_SupplySatchel:OnResizeTimer()
...
function CRB_SupplySatchel:OnMainWindowMouseResized( wndHandler, wndControl, eMouseButton)
...
function CRB_SupplySatchel:OnQueryDragDrop( wndHandler, wndControl, nX, nY, wndSource, strType, nData, eResult )
...
function CRB_SupplySatchel:OnDragDrop( wndHandler, wndControl, nX, nY, wndSource, strType, nData, bDragDropHasBeenReset )
	if strType == "DDBagItem" then
		local itemCurrent = wndSource:GetItem(nData)
		if itemCurrent and itemCurrent:CanMoveToSupplySatchel() then
...
function CRB_SupplySatchel:OnMainWindowClosed( wndHandler, wndControl )
	Apollo.CreateTimer("UnloadSatchel", knUnloadWaitTime, false)
end
...
function CRB_SupplySatchel:OnSearchBoxChanged(wndHandler, wndControl)
	self:PopulateSatchel(true)
end

function CRB_SupplySatchel:OnSearchClearBtn(wndHandler, wndControl)
	self.wndMain:FindChild("SearchBox"):SetText("")
	self:PopulateSatchel(true)
	wndHandler:SetFocus() -- Focus on close button to steal focus from input
end

function CRB_SupplySatchel:PopulateSatchel(bRescroll)
...
function CRB_SupplySatchel:OnItemMouseButtonDown( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY, bDoubleClick, bStopPropagation )
...
function CRB_SupplySatchel:OnItemMouseMove( wndHandler, wndControl, nLastRelativeMouseX, nLastRelativeMouseY )
	if wndHandler ~= wndControl then return end
...
function CRB_SupplySatchel:OnItemMouseButtonUp( wndHandler, wndControl, eMouseButton, nLastRelativeMouseX, nLastRelativeMouseY )
	if wndHandler ~= wndControl then return end
...
function CRB_SupplySatchel:HelperSearchNameMatch(strBase, strInput)
...
-----------------------------------------------------------------------------------------------
-- CRB_SupplySatchel Instance
-----------------------------------------------------------------------------------------------
local CRB_SupplySatchelInst = CRB_SupplySatchel:new()
CRB_SupplySatchelInst:Init()
