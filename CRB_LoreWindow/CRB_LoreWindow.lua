-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_LoreWindow
...
function CRB_LoreWindow:Init()
    Apollo.RegisterAddon(self)
end

function CRB_LoreWindow:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_LoreWindow.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_LoreWindow:OnDocumentReady()
...
function CRB_LoreWindow:OnInterfaceMenuListHasLoaded()
...
function CRB_LoreWindow:Initialize()
	self.wndColDisplay = nil
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_LoreWindowForm", nil, self)
...
function CRB_LoreWindow:OnDatacubeUpdated(idArg, bIsVolume)
	if idArg then
...
function CRB_LoreWindow:OnShowLoreWindow(tArticleData)
...
function CRB_LoreWindow:OnToggleLoreWindow(tArticleData)
	if not self.wndMain or not self.wndMain:IsValid() then
		self:Initialize()
	end
...
function CRB_LoreWindow:OpenToSpecificArticle(idArg, bIsVolume)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_LoreWindow:OnCloseBtn(wndHandler, wndControl)
...
function CRB_LoreWindow:InitializeCollections()
	-- Build zone dropdown
	local tZonesAtLoad = {}
...
function CRB_LoreWindow:MainRedrawCollections()
...
function CRB_LoreWindow:RedrawFromUI()
	self:MainRedrawCollections()
end

function CRB_LoreWindow:DrawColHeaderItems(tCurrZone, wndHeader, strHeader)
...
function CRB_LoreWindow:SpawnAndDrawColReader(tArticleData, wndOrigin) -- wndOrigin can be nil
...
function CRB_LoreWindow:OnDestroyColDisplay(wndHandler, wndControl)
	if self.wndColDisplay and self.wndColDisplay:IsValid() then
...
function CRB_LoreWindow:OnPlayPauseCheck(wndHandler, wndControl)
	local tArticleData = wndHandler:GetData()
	DatacubeLib.PlayDatacubeSound(tArticleData.nDatacubeId)
end

function CRB_LoreWindow:OnPlayPauseUncheck(wndHandler, wndControl)
	local tArticleData = wndHandler:GetData()
...
function CRB_LoreWindow:OnDatacubeStopped()
	if self.wndColDisplay and self.wndColDisplay:IsValid() then
		self.wndColDisplay:FindChild("PlayPauseButton"):SetCheck(false)
	end
end

function CRB_LoreWindow:OnDatacubeTimer()
	if self.wndColDisplay and self.wndColDisplay:IsValid() then
...
function CRB_LoreWindow:OnDropdownZoneBtn(wndHandler, wndControl) -- wndHandler is "DropdownZoneBtn" and its data is tCurrZone
...
function CRB_LoreWindow:OnColTopDropdownToggle(wndHandler, wndControl) -- ColTopDropdownBtn Zone Picker
	self.wndColMainScroll:Enable(not wndHandler:IsChecked())
end

function CRB_LoreWindow:OnColTopDropdownClosed(wndHandler, wndControl) -- ColTopDropdownBtn Zone Picker Window
	self.wndColMainScroll:Enable(true)
end

function CRB_LoreWindow:OnColHeaderToggle(wndHandler, wndControl) -- E.G. "Datacubes (1/6)"
...
function CRB_LoreWindow:OnColBtnToSpawnReader(wndHandler, wndControl) -- ColDatacubeBtn, ColTalesBtn, ColJournalChildBtn
...
function CRB_LoreWindow:OnColBtnToDespawnReader(wndHandler, wndControl) -- ColDatacubeBtn, ColTalesBtn, ColJournalChildBtn, MainColArticleDisplay
	self:OnDestroyColDisplay()
end

function CRB_LoreWindow:OnMainNavGACheck(wndHandler, wndControl)
	self:OnDestroyColDisplay()
end

function CRB_LoreWindow:OnMainNavColCheck(wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_CloseGAReader")
end

function CRB_LoreWindow:OnCompactLore()
...
function CRB_LoreWindow:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
function CRB_LoreWindow:HelperDrawDropdownZoneProgress(wndCurr, idCurrZone, strCurrZoneName) -- wndCurr is "DropdownZoneItem" -- TODO: Refactor if possible
...
function CRB_LoreWindow:FactoryProduce(wndParent, strFormName, tObject)
...
local CRB_LoreWindowInst = CRB_LoreWindow:new()
CRB_LoreWindowInst:Init()
