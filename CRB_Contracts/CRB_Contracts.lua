-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Contracts
...
{
	[ContractsLib.ContractQuality.None]			= { strActive = "",											strAvailable = "CRB_Contracts:sprContracts_Difficulty01",		strOverview = Apollo.GetString("Contract_Level1Repeatable"), },
	[ContractsLib.ContractQuality.Good]			= { strActive = "CRB_Contracts:sprContracts_Difficulty01",		strAvailable = "CRB_Contracts:sprContracts_Difficulty01",		strOverview = Apollo.GetString("Contract_Level1Repeatable"), },
	[ContractsLib.ContractQuality.Excellent]	= { strActive = "CRB_Contracts:sprContracts_Difficulty02",		strAvailable = "CRB_Contracts:sprContracts_Difficulty02",		strOverview = Apollo.GetString("CRB_2"), },
	[ContractsLib.ContractQuality.Superb]		= { strActive = "CRB_Contracts:sprContracts_Difficulty03",		strAvailable = "CRB_Contracts:sprContracts_Difficulty03",		strOverview = Apollo.GetString("CRB_3"), },
	[ContractsLib.ContractQuality.Legendary]	= { strActive = "",											strAvailable = "CRB_Contracts:sprContracts_Difficulty03",		strOverview = Apollo.GetString("CRB_4"), },
}

local ktContractTypeArt =
{
	[0]		= { strActive = "CRB_Contracts:sprContracts_Type01",		strAvailable = "CRB_Contracts:sprContracts_Type01",		strOverview = Apollo.GetString("CombatLogOptions_General"), },
	[122]	= { strActive = "CRB_Contracts:sprContracts_Type01",		strAvailable = "CRB_Contracts:sprContracts_Type01",		strOverview = Apollo.GetString("CombatLogOptions_General"), },
	[123]	= { strActive = "CRB_Contracts:sprContracts_Type02",		strAvailable = "CRB_Contracts:sprContracts_Type02",		strOverview = Apollo.GetString("CRB_Kill"), },
	[124]	= { strActive = "CRB_Contracts:sprContracts_Type03",		strAvailable = "CRB_Contracts:sprContracts_Type03",		strOverview = Apollo.GetString("CRB_Collection"), },
	[125]	= { strActive = "CRB_Contracts:sprContracts_Type04",		strAvailable = "CRB_Contracts:sprContracts_Type04",		strOverview = Apollo.GetString("CRB_Completion"), },
}

local knSaveVersion = 2

function CRB_Contracts:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_Contracts:Init()
    Apollo.RegisterAddon(self)
end

function CRB_Contracts:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Contracts.xml")
	self.xmlDoc:RegisterCallback("OnDocLoaded", self)
end

function CRB_Contracts:OnDocLoaded()
...
function CRB_Contracts:OnInterfaceMenuListHasLoaded()
...
function CRB_Contracts:OpenContracts()
	if self.tWndRefs.wndMain ~= nil and self.tWndRefs.wndMain:IsValid() then
		return
	end
	
	self.wndAbandonConfirm = nil
	local wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_ContractsForm", nil, self)
	wndMain:Invoke()
	...
	fnContractWindowSetup(wndPvEContracts, Apollo.GetString("Contracts_AvailableContractPve"), "CRB_Contracts:sprContracts_PvE")
	fnContractWindowSetup(wndPvPContracts, Apollo.GetString("Contracts_AvailableContractPvp"), "CRB_Contracts:sprContracts_PvP")
...
function CRB_Contracts:CloseContracts()
...
function CRB_Contracts:ToggleContracts()
	if self.tWndRefs.wndMain ~= nil and self.tWndRefs.wndMain:IsValid() then
		self:CloseContracts()
	else
		self:OpenContracts()
...
function CRB_Contracts:OnClose(wndHandler, wndControl)
...
function CRB_Contracts:OnContractStateChanged(contract, eState)
	if self.tWndRefs.wndMain == nil or not self.tWndRefs.wndMain:IsValid() then
		return
	end
			...
			if eState == Quest.QuestState_Accepted then
				...
				for idx, wndContract in pairs(wndContract:FindChild("ActiveContracts:ActiveContractContainer"):GetChildren()) do
					if contract == wndContract:GetData() then
						...
						wndActivatedContractFlash:Show(true)
						wndActivatedContractFlash:SetSprite("CRB_Contracts:sprContracts_SlottedAnim")
...
function CRB_Contracts:OnRewardTrackUpdated(rewardTrack)
	if self.tWndRefs.wndMain == nil or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_Contracts:RedrawAll()
...
function CRB_Contracts:RedrawContract(wndContract, eContractType, eRewardType, tActiveContractList, tAvailableContracts)
...
function CRB_Contracts:DrawActiveContracts(wndContainer, eContractType, eRewardType, tActiveContractList)
		...
		if contractActive == nil then
			if contractOnWindow ~= nil then
				if contractOnWindow:GetQuest():GetState() == Quest.QuestState_Completed then
					...
					wndActivatedContractFlash:Show(true)
					wndActivatedContractFlash:SetSprite("CRB_Contracts:sprContracts_SlottedTurnIn")
				else
					local wndAbandonedContractFlash = wndActive:FindChild("AbandonedContractFlash")
					wndAbandonedContractFlash:Show(true)
					wndAbandonedContractFlash:SetSprite("CRB_Contracts:sprContracts_SlottedReverse")
				end
...
function CRB_Contracts:DrawRewards(wndContainer, eContractType, eRewardType, tActiveContractList)
...
function CRB_Contracts:DrawRewardPoint(wndContainer, wndRewardPoint, rtRewardTrack, tReward, wndRewardProgressBar, wndRewardList, bMoveIntoPlace)
	local arRewards = rtRewardTrack:GetAllRewards()
	local nRewardMax = arRewards[#arRewards].nCost
	...
	if tReward.bIsClaimed then
		if bMoveIntoPlace then
			wndRewardPoint:FindChild("ActionBtn"):ChangeArt("CRB_Contracts:btnContracts_MilestoneMarkerComplete")
			wndRewardPoint:FindChild("AchievedBG"):SetSprite("")
		else
			wndRewardPoint:FindChild("ActionBtn"):ChangeArt("CRB_Contracts:btnContracts_MilestoneMarkerFinal")
			if self.tWndRefs.wndPvPContracts == wndContainer then
				wndRewardPoint:FindChild("HighlightBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneFinalCapPvP")
				wndRewardPoint:FindChild("AchievedBG"):SetSprite("")
			else 
				wndRewardPoint:FindChild("HighlightBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneFinalCapPvE")
				wndRewardPoint:FindChild("AchievedBG"):SetSprite("")
			end
		end
	...
	elseif tReward.bCanClaim then
		if bMoveIntoPlace then
			wndRewardPoint:FindChild("ActionBtn"):ChangeArt("CRB_Contracts:btnContracts_MilestoneMarkerReady")
			wndRewardPoint:FindChild("AchievedBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneMarkerReadyAnim")
		else
			wndRewardPoint:FindChild("ActionBtn"):ChangeArt("CRB_Contracts:btnContracts_MilestoneMarkerFinalReady")
			if self.tWndRefs.wndPvPContracts == wndContainer then
				wndRewardPoint:FindChild("HighlightBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneFinalCapPvP")
				wndRewardPoint:FindChild("AchievedBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneFinalCapPvPFlash")
			else 
				wndRewardPoint:FindChild("HighlightBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneFinalCapPvE")
				wndRewardPoint:FindChild("AchievedBG"):SetSprite("CRB_Contracts:sprContracts_MilestoneFinalCapPvEFlash")
			end
		end
		...
		if bMoveIntoPlace then
			wndRewardPoint:FindChild("ActionBtn"):ChangeArt("CRB_Contracts:btnContracts_MilestoneMarkerStart")
			wndRewardPoint:FindChild("AchievedBG"):SetSprite("")
		else
			wndRewardPoint:FindChild("ActionBtn"):ChangeArt("CRB_Contracts:btnContracts_MilestoneMarkerFinal")
...
function CRB_Contracts:DrawRewardListEntry(wndEntry, rtRewardTrack, tReward)
...
function CRB_Contracts:DrawPeriodicContracts(wndContainer, tAvailableContracts, nActiveContractCount)
	...
	for idx, contractAvailable in pairs(tAvailableContracts or {}) do
		...
		if queQuest then
			...
			if contractAvailable:IsAchieved() then
				wndSelectBtn:ChangeArt("CRB_Contracts:btnContracts_CheckBoxSlotted")
			elseif contractAvailable:IsAccepted() then
				wndSelectBtn:ChangeArt("CRB_Contracts:btnContracts_CheckBoxSlotted")
			elseif contractAvailable:IsCompleted() then
				wndSelectBtn:ChangeArt("CRB_Contracts:btnContracts_CheckBoxLocked")
			elseif nActiveContractCount == ContractsLib.kMaxActiveContracts then
				wndSelectBtn:ChangeArt("CRB_Contracts:btnContracts_CheckBoxLocked")
			else
				wndSelectBtn:ChangeArt("CRB_Contracts:btnContracts_CheckBox")
			end
...
function CRB_Contracts:DrawContractOverview(wndContainer, contract)
...
function CRB_Contracts:BuildContractTooltip(contract, nActiveContractCount)
	local queQuest = contract:GetQuest()
	local strTooltip = ""
...
function CRB_Contracts:OnAbandonBtnSignal(wndHandler, wndControl)
...
function CRB_Contracts:OnAbandonConfirmYesBtn(wndHandler, wndControl)
	wndHandler:GetData():Abandon()
	Sound.Play(Sound.PlayUIContractAbandonContract)
	self:OnAbandonConfirmClose()
end

function CRB_Contracts:OnAbandonConfirmClose(wndHandler, wndControl)
...
function CRB_Contracts:OnRankLineItemMouseUp(wndHandler, wndControl)
...
function CRB_Contracts:OnAcceptBtnSignal(wndHandler, wndControl, eMouseButton)
	wndHandler:GetData():Accept()
	Sound.Play(Sound.PlayUIContractAcceptContract)
end

function CRB_Contracts:OnActiveContractTurnInBtn(wndHandler, wndControl)
	local contract = wndHandler:GetData()
...
function CRB_Contracts:OnContractObjectiveUpdated(contract)
	if contract:IsAchieved() then
		local queQuest = contract:GetQuest()
...
function CRB_Contracts:DrawToast(contract, queQuest, tObjective)
...
function CRB_Contracts:OnToastAnim1()
...
function CRB_Contracts:OnToastFormMouseUp(wndHandler, wndControl)
...
function CRB_Contracts:OnToastFormMouseEnter(wndHandler, wndControl)
...
function CRB_Contracts:OnToastFormMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:FindChild("ToastTitle") then
		wndHandler:FindChild("ToastTitle"):SetTextColor("UI_BtnTextBlueNormal")
	end
end

function CRB_Contracts:ShowPvPContracts(wndHandler, wndControl, eMouseButton)
...
function CRB_Contracts:ShowPvEContracts(wndHandler, wndControl, eMouseButton)
...
function CRB_Contracts:OnRewardPointBtnSignal(wndHandler, wndControl, eMouseButton)
...
function CRB_Contracts:OnRewardConfirmTakeBtnSignal(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
...
function CRB_Contracts:OnRewardItemSelectionBtnCheck(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
...
function CRB_Contracts:OnRewardItemSelectionBtnUncheck(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
	tData.wndRewardListClaimBtn:Enable(false)
end

function CRB_Contracts:OnRewardListClose(wndHandler, wndControl)
	return true
end
...
function CRB_Contracts:OnGenerateRewardItemTooltip(wndHandler, wndControl, eToolTipType, x, y)
	local tData = wndControl:GetData()
...
function CRB_Contracts:OnGenerateCashRewardTooltip(wndHandler, wndControl, eToolTipType, monA, monB)
...
function CRB_Contracts:OnSelectBtnCheck(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
...
function CRB_Contracts:OnSelectBtnUncheck(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
...
function CRB_Contracts:OnRewardListCloseBtnUncheck(wndHandler, wndControl, eMouseButton)
...
function CRB_Contracts:OnRewardPointMouseEnter(wndHandler, wndControl, x, y)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Contracts:OnRewardPointMouseExit(wndHandler, wndControl, x, y)
	if wndHandler ~= wndControl then
		return
	end
...
local CRB_ContractsInst = CRB_Contracts:new()
CRB_ContractsInst:Init()
