-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_CastBar
...
function CRB_CastBar:Init()
	Apollo.RegisterAddon(self)
end

function CRB_CastBar:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CastBar.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_CastBar:OnDocumentReady()
...
function CRB_CastBar:DrawSingleBarFrame(wnd)
...
function CRB_CastBar:DrawSingleBarFrameCircle(wnd)
	local fPercentDone = GameLib.GetSpellThresholdTimePrcntDone(self.tCurrentOpSpell.id)
	wnd:FindChild("Fill"):SetMax(1)
...
function CRB_CastBar:DrawSingleBarFrameTiered(wnd)
	local fPercentDone = GameLib.GetSpellThresholdTimePrcntDone(self.tCurrentOpSpell.id)
...
function CRB_CastBar:OnGenerateTooltip(wndHandler, wndControl, eType, spl)
...
function CRB_CastBar:OnStartSpellThreshold(idSpell, nMaxThresholds, eCastMethod) -- also fires on tier change
...
function CRB_CastBar:OnUpdateSpellThreshold(idSpell, nNewThreshold) -- Updates when P/H/R changes tier or RT tap is performed
...
function CRB_CastBar:OnClearSpellThreshold(idSpell)
...
local CRB_CastBarInstance = CRB_CastBar:new()
CRB_CastBarInstance:Init()
