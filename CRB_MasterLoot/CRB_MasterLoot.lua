...
function CRB_MasterLoot:Init()
	Apollo.RegisterAddon(self)
end

function CRB_MasterLoot:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MasterLoot.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_MasterLoot:OnDocumentReady()
...
function CRB_MasterLoot:OnWindowManagementReady()
...
function CRB_MasterLoot:OnToggleGroupBag()
	self:OnMasterLootUpdate()
end

----------------------------

function CRB_MasterLoot:OnMasterLootUpdate()
...
function CRB_MasterLoot:RefreshMasterLootItemList(tMasterLootItemList)
...
function CRB_MasterLoot:RefreshMasterLootLooterList(tItem)
	if tItem == nil then
...
function CRB_MasterLoot:RefreshLooterItemList(tLooterItemList)
...
function CRB_MasterLoot:OnGroup_Left()
	if self.wndMasterLoot:IsShown() then
...
function CRB_MasterLoot:OnItemGenerateTooltip(wndHandler, wndControl, eToolTipType, x, y)
...
function CRB_MasterLoot:OnItemMouseButtonUp(wndHandler, wndControl, eMouseButton) -- Both LooterItemButton and ItemButton
...
function CRB_MasterLoot:OnItemCheck(wndHandler, wndControl, eMouseButton)
	if eMouseButton ~= GameLib.CodeEnumInputMouse.Right then
		local tItemInfo = wndHandler:GetData()
...
function CRB_MasterLoot:OnItemUncheck(wndHandler, wndControl, eMouseButton)
	if eMouseButton ~= GameLib.CodeEnumInputMouse.Right then
...
function CRB_MasterLoot:OnCharacterMouseButtonUp(wndHandler, wndControl, eMouseButton)
	if eMouseButton == GameLib.CodeEnumInputMouse.Right then
...
function CRB_MasterLoot:OnCharacterCheck(wndHandler, wndControl, eMouseButton)
	if eMouseButton ~= GameLib.CodeEnumInputMouse.Right then
...
function CRB_MasterLoot:OnCharacterUncheck(wndHandler, wndControl, eMouseButton)
	if eMouseButton ~= GameLib.CodeEnumInputMouse.Right then
		self.wndMasterLoot:FindChild("Assignment"):Enable(false)
	end
end
...
function CRB_MasterLoot:OnAssignDown(wndHandler, wndControl, eMouseButton)
	local tData = wndControl:GetData()
...
function CRB_MasterLoot:OnCloseMasterWindow()
	self.locSavedMasterWindowLoc = self.wndMasterLoot:GetLocation()
	self.wndMasterLoot:Show(false)
end
...
function CRB_MasterLoot:OnCloseLooterWindow()
	self.locSavedLooterWindowLoc = self.wndLooter:GetLocation()
	self.wndLooter:Show(false)
end
...
function CRB_MasterLoot:OnLootAssigned(objItem, strLooter)
...
function CRB_MasterLoot:OnSave(eType)
...
function CRB_MasterLoot:OnRestore(eType, tSavedData)
...
local MasterLoot_Singleton = CRB_MasterLoot:new()
