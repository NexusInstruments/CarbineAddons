-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_QuestLog
...
function CRB_QuestLog:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_QuestLog.xml")-- CRB_QuestLog will always be kept in memory, so save parsing it over and over
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)

	Apollo.RegisterEventHandler("InterfaceMenuListHasLoaded", "OnInterfaceMenuListHasLoaded", self)
end

function CRB_QuestLog:OnDocumentReady()
...
function CRB_QuestLog:OnInterfaceMenuListHasLoaded()
...
function CRB_QuestLog:Initialize()
    ...
    self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_QuestLogForm", g_wndProgressLog:FindChild("ContentWnd_1"), self)
...
function CRB_QuestLog:OnGenericEvent_ShowQuestLog(queTarget)
...
function CRB_QuestLog:DestroyAndRedraw() -- TODO, remove as much as possible that calls this
...
function CRB_QuestLog:RedrawLeftTreeFromUI()
	self:RedrawLeftTree()
	self:ResizeTree()
end

function CRB_QuestLog:RedrawFromUI()
	self:RedrawEverything()
end

function CRB_QuestLog:RedrawEverything()
	self:RedrawLeftTree()
	self:ResizeTree()
...
function CRB_QuestLog:RedrawRight()
	local bLeftSideHasResults = #self.wndLeftSideScroll:GetChildren() ~= 0
	self.wndLeftSideScroll:SetText(bLeftSideHasResults and "" or Apollo.GetString("QuestLog_NoResults"))
	self.wndQuestInfoControls:Show(bLeftSideHasResults)
	self.wndRightSide:Show(bLeftSideHasResults)
...
function CRB_QuestLog:RedrawLeftTree()
...
function CRB_QuestLog:HelperSetupMiddleLevelWindow(wndMiddle, epiEpisode)
...
function CRB_QuestLog:HelperSetupFakeMiddleLevelWindow(wndMiddle, strText)
...
function CRB_QuestLog:HelperSetupBottomLevelWindow(wndBot, queQuest)
...
function CRB_QuestLog:ResizeTree()
...
function CRB_QuestLog:ResizeRight()
...
function CRB_QuestLog:DrawRightSide(queSelected)
...
function CRB_QuestLog:OnGroupUpdate()
...
function CRB_QuestLog:DrawUnknownRightSide(queSelected)
...
function CRB_QuestLog:OnTopLevelBtnCheck(wndHandler, wndControl)
	self:RedrawLeftTree()
	self:ResizeTree()
end

function CRB_QuestLog:OnTopLevelBtnUncheck(wndHandler, wndControl)
	self:ResizeTree()
end

function CRB_QuestLog:OnMiddleLevelBtnCheck(wndHandler, wndControl)
...
function CRB_QuestLog:OnMiddleLevelBtnUncheck(wndHandler, wndControl)
	self:ResizeTree()
end

function CRB_QuestLog:OnBottomLevelBtnCheck(wndHandler, wndControl) -- From Button or OnQuestObjectiveUpdated
...
function CRB_QuestLog:OnBottomLevelBtnUncheck(wndHandler, wndControl)
	self.wndQuestInfoControls:Show(false)
	self.wndRightSide:Show(false)
end

function CRB_QuestLog:OnBottomLevelBtnDown( wndHandler, wndControl, eMouseButton )
...
function CRB_QuestLog:OnQuestTrackBtn(wndHandler, wndControl) -- QuestTrackBtn
	local queSelected = self.wndRightSide:GetData()
...
function CRB_QuestLog:OnQuestShareBtn(wndHandler, wndControl) -- QuestShareBtn
	local queSelected = self.wndRightSide:GetData()
	queSelected:Share()
end

function CRB_QuestLog:OnQuestCallBtn(wndHandler, wndControl) -- QuestCallBtn or QuestInfoCostumeWindow
	local queSelected = self.wndRightSide:GetData()
	CommunicatorLib.CallContact(queSelected)
	Event_FireGenericEvent("ToggleCodex") -- Hide codex, not sure if we want this
end

function CRB_QuestLog:OnQuestAbandonBtn(wndHandler, wndControl) -- QuestAbandonBtn
	local queSelected = self.wndRightSide:GetData()
	queSelected:Abandon()
...
function CRB_QuestLog:OnQuestHideBtn(wndHandler, wndControl) -- QuestInfoControlsHideBtn
	local queSelected = self.wndRightSide:GetData()
...
function CRB_QuestLog:OnQuestAbandonPopoutClose(wndHandler, wndControl) -- QuestAbandonPopoutClose
	self.wndMain:FindChild("QuestAbandonConfirm"):Show(false)
end
...
function CRB_QuestLog:OnQuestStateChanged(queUpdated, eState)
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_QuestLog:OnQuestObjectiveUpdated(queUpdated)
	local queCurrent = self.wndRightSide:GetData()
	if queCurrent and queCurrent:GetId() == queUpdated:GetId() then
		self:RedrawEverything()
	end
...
function CRB_QuestLog:OnDestroyQuestObject(queTarget) -- QuestStateChanged, QuestObjectiveUpdated
...
function CRB_QuestLog:OnQuestTrackedChanged(queUpdated, bTracked)
...
function CRB_QuestLog:OnDialog_QuestShare(queToShare, unitTarget)
...
function CRB_QuestLog:OnShareCancel(wndHandler, wndControl)
...
function CRB_QuestLog:OnShareAccept(wndHandler, wndControl)
	local queToShare = self.wndShare:GetData()
	if queToShare then
...
function CRB_QuestLog:OnShareTimeout()
	self:OnShareCancel()
end
...
function CRB_QuestLog:HelperBuildRewardsRec(wndReward, tRewardData, bReceived)
...
function CRB_QuestLog:OnRewardIconMouseUp(wndHandler, wndControl, eMouseButton)
...
function CRB_QuestLog:HelperBuildObjectiveTitleString(queQuest, tObjective, bIsTooltip)
...
function CRB_QuestLog:HelperBuildObjectiveProgBar(queQuest, tObjective, wndObjective, bComplete)
...
function CRB_QuestLog:CheckLeftSideFilters(queQuest)
...
function CRB_QuestLog:HelperPrereqFailed(tCurrItem)
	return tCurrItem and tCurrItem:IsEquippable() and not tCurrItem:CanEquip()
end

function CRB_QuestLog:HelperPrefixTimeString(fTime, strAppend, strColorOverride)
...
function CRB_QuestLog:FactoryCacheProduce(wndParent, strFormName, strKey)
...
local CRB_QuestLogInst = CRB_QuestLog:new()
CRB_QuestLogInst:Init()
