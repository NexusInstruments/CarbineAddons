-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Crafting
...
function CRB_Crafting:Init()
	Apollo.RegisterAddon(self)
end

function CRB_Crafting:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Crafting.xml") -- QuestLog will always be kept in memory, so save parsing it over and over
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Crafting:OnDocumentReady()
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_CraftingForm", nil, self)
...
function CRB_Crafting:OnWindowManagementReady()
	--Event_FireGenericEvent("WindowManagementAdd", {wnd = self.wndMain, strName = Apollo.GetString("DialogResponse_CraftingStation")})
end

function CRB_Crafting:OnCrafting_TimerCraftingStationCheck() -- Hackish: These are async from the rest of the UI (and definitely can't handle data being set)
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:FindChild("NoStationBlocker"):Show(not CraftingLib.IsAtCraftingStation())
	end
end

function CRB_Crafting:OnGenericEvent_StartCircuitCraft(idSchematic)
...
function CRB_Crafting:OnPreviewStartCraft(wndHandler, wndControl) -- PreviewStartCraftBtn, data is idSchematic
...
function CRB_Crafting:OnCraftBtnClicked(wndHandler, wndControl) -- CraftButton, data is idSchematic
	if self.luaSchematic then
		local tCurrentCraft = CraftingLib.GetCurrentCraft()
...
function CRB_Crafting:OnCraftingInterrupted()
	self.timerBtn:Stop()
	self.wndMain:FindChild("PostCraftBlocker"):Show(false)
	self.wndMain:FindChild("PostCraftBlocker"):FindChild("MouseBlockerBtn"):Show(false)
end

function CRB_Crafting:OnCircuitCrafting_CraftBtnTimer()
...
function CRB_Crafting:OnCloseBtn(wndHandler, wndControl)
...
function CRB_Crafting:ExitAndReset() -- Botch Craft calls this directly
...
function CRB_Crafting:OnP2PTradeExitAndReset()
	local tCurrentCraft = CraftingLib.GetCurrentCraft()
...
function CRB_Crafting:OnShowTutorialsBtnToggle(wndHandler, wndControl)
	self:DrawTutorials(wndHandler:IsChecked())
end

function CRB_Crafting:DrawTutorials(bArgShow)
...
function CRB_Crafting:OnTutorialItemBtn(wndHandler, wndControl)
...
function CRB_Crafting:OnTutorialPopupCloseBtn()
	self.wndTutorialPopup:Show(false)
end

local CRB_CraftingInst = CRB_Crafting:new()
CRB_CraftingInst:Init()
