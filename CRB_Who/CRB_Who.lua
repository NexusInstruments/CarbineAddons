...
function CRB_Who:Init()
    Apollo.RegisterAddon(self)
end

function CRB_Who:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Who.xml")
...
function CRB_Who:OnRestore(eType, tSavedData)
...
function CRB_Who:OnDocumentReady()
...
function CRB_Who:OnInterfaceMenuListHasLoaded()
	local tData = {
		"ToggleWhoWindow", 
		"", 
...
function CRB_Who:OnObjectiveTrackerLoaded(wndForm)
...
function CRB_Who:OnWindowManagementReady()
...
function CRB_Who:OnWindowManagementUpdate(tSettings)
...
function CRB_Who:HelperLoadForms()
	self:HelperDestroyForms()
	
	self.tWndRefs.wndMain 				= Apollo.LoadForm(self.xmlDoc, "CRB_WhoForm", nil, self)
...
function CRB_Who:HelperAttachToObjectiveTracker()
...
function CRB_Who:HelperDestroyForms()
	self.bFormLoaded = false
...
function CRB_Who:OnTimer()
	self:HelperResetUI()
end

function CRB_Who:OnUnitCreated(unit)
...
function CRB_Who:OnUnitDestroyed(unit)
	if unit:GetType() == "Player" then
...
function CRB_Who:OnWhoResponse(arResponse, eWhoResult, strResponse)	
...
function CRB_Who:OnWhoButtonClicked(wndHandler, wndControl)
...
function CRB_Who:OnToggleShowNearbyPlayers()
...
function CRB_Who:OnCancel(wndHandler, wndControl)
	self.tWndRefs.wndMain:Close()
end

function CRB_Who:OnSearchResultsContent(wndHandler, wndControl)
	self.bShowSearchResults = true
	
	self:HelperResetUI()
end

function CRB_Who:OnNearbyPlayersContent(wndHandler, wndControl)
	self.bShowSearchResults = false
	
	self:HelperResetUI()
end

function CRB_Who:HelperDelayResetUI()
...
function CRB_Who:HelperResetUI()
	self.bTimerRunning = false
	self.timer:Stop()
...
function CRB_Who:OnWhoSortToggle(wndHandler, wndControl)	
	--Class
...
function CRB_Who:OnResizeContainer(wndEpisodeGroup, bIsShown)
...
function CRB_Who:OnShowNearbyPlayersChecked(wndHandler, wndControl)
...
function CRB_Who:OnShowNearbyPlayersChanged(wndHandler, wndControl)
...
function CRB_Who:OnEpisodeGroupControlBackerMouseEnter(wndHandler, wndControl)
...
function CRB_Who:OnEpisodeGroupControlBackerMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl then
...
function CRB_Who:OnEpisodeGroupMinimizedBtnChecked(wndHandler, wndControl, eMouseButton)
	self.bMinimized = true
	
	self:HelperResetUI()
end

function CRB_Who:OnEpisodeGroupMinimizedBtnUnChecked(wndHandler, wndControl, eMouseButton)
	self.bMinimized = false
	
	self:HelperResetUI()
end

function CRB_Who:OnEpisodeGroupDockBtnChecked(wndHandler, wndControl, eMouseButton)
	self.bFloating = true
	
	self:HelperResetUI()
end

function CRB_Who:OnEpisodeGroupDockBtnUnChecked(wndHandler, wndControl, eMouseButton)
	self.bFloating = false
...
function CRB_Who:OnListItemMouseEnter(wndHandler, wndControl)
	-- Has Mouse
...
function CRB_Who:OnListItemMouseExit(wndHandler, wndControl)
	-- Has Mouse
	local bHasMouse = wndControl:GetParent():FindChild("ListItemMouseCatcher"):ContainsMouse()
...
function CRB_Who:OnListItemClicked(wndHandler, wndControl, eMouseButton, x, y)
...
function CRB_Who:FactoryProduce(wndParent, strFormName, oObject)
