...
function Who:HelperResetUI()
	...
	for unitName, unit in pairs(self.tNearbyPlayers) do
		if unit ~= nil and unit:IsValid() and not unit:IsInYourGroup() and not unit:IsThePlayer() then
	...
	if not self.bShowSearchResults or self.bShowNearbyPlayers then
		...
		self.tWndRefs.wndPlayerForm:Show(self.tWndRefs.wndShowNearbyPlayers:IsChecked() and bFloating and #tTrackerNearbyPlayers > 0 or false)
	else
...
WhoInst:Init()wndControl, eMouseButton )
