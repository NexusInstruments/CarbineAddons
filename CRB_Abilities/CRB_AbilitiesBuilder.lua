-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Abilities
...
function CRB_Abilities:Init()
    Apollo.RegisterAddon(self)
end

function CRB_Abilities:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_AbilitiesBuilder.xml")
...
function CRB_Abilities:OnDocumentReady()
...
function CRB_Abilities:OnPathAbilityUpdated(nPathId)
	self.nSelectedPathId = nPathId
end

function CRB_Abilities:OnInterfaceMenuListHasLoaded()
...
function CRB_Abilities:UpdateInterfaceMenuAlerts()
...
function CRB_Abilities:OnToggleAbilitiesWindow(bAtTrainer)
	if bAtTrainer then
...
function CRB_Abilities:OnPlayerEnteredWorld()
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() and self.tWndRefs.wndMain:IsShown() then
		self:BuildWindow()
	end
end

function CRB_Abilities:OnChangeWorld()
	if self.tWndRefs.wndMain and self.tWndRefs.wndMain:IsValid() and self.tWndRefs.wndMain:IsShown() then
		self:OnCloseFinal()
	end
end

function CRB_Abilities:BuildWindow()
...
function CRB_Abilities:OnCharacterCreated()
	self:RedrawFromScratch()
end

function CRB_Abilities:OnWindowClosed(wndHandler, wndControl)
...
function CRB_Abilities:OnClose()
	if self.bDirtyBit and not GameLib.GetPlayerUnit():IsInCombat() and not self.tWndRefs.wndMain:FindChild("LeaveConfirmationContainer"):IsShown() then
		self.tWndRefs.wndMain:FindChild("LeaveConfirmationContainer"):Invoke()
	else
		self:OnCloseFinal()
...
function CRB_Abilities:OnCloseFinal() -- Window Escape Key also routes here
	self:UpdateInterfaceMenuAlerts()
...
function CRB_Abilities:OnRedrawFromUI()
	self:DrawSpellBook()
end

function CRB_Abilities:OnSetResetClick(wndHandler, wndControl)
	AbilityBook.ClearCachedLASUpdates()
...
function CRB_Abilities:OnSetActivateClick(wndHandler, wndControl)
...
function CRB_Abilities:RedrawFromScratch() -- Draw Slots destroys everything (Level up and keybind change also routes here)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_Abilities:OnAbilitiesTabCheck(wndHandler, wndControl)
...
function CRB_Abilities:OnAmpTabCheck(wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_OpenEldanAugmentation", self.tWndRefs.wndMain:FindChild("BGFrame:AMPBuilderMain"))
...
function CRB_Abilities:DrawSpellBook()
	-- AMP Points
...
function CRB_Abilities:DrawALockedSpell(tHighestTier, nPlayerLevel)
...
function CRB_Abilities:OnUnitEnteredCombat(unit, bIsInCombat)
...
function CRB_Abilities:OnBuyAbilityBtn(wndHandler, wndControl)
	AbilityBook.ActivateSpell(wndHandler:GetData(), true)
	Sound.Play(Sound.PlayUIUnlockAbility)
	--self:RedrawFromScratch()
end

function CRB_Abilities:DrawAnAddSpell(tBaseAbility, tHighestTier)
...
function CRB_Abilities:DrawATiersSpell(tBaseAbility, tHighestTier, nPlayerLevel, nAbilityPoints)
...
function CRB_Abilities:OnSpellbookItemAdd(wndHandler, wndControl)
	self.bDirtyBit = true
...
function CRB_Abilities:OnSpellbookItemSubBtn(wndHandler, wndControl)
	self.bDirtyBit = true
...
function CRB_Abilities:OnSpellbookProgPieceToggle(wndHandler, wndControl) -- SpellbookProgPieceBtn
	self.bDirtyBit = true
	AbilityBook.UpdateSpellTier(wndHandler:GetData().nId, wndHandler:GetData().nTier)
	self:DrawSpellBook()
end

function CRB_Abilities:OnSpellbookAbilityPointsReset(wndHandler, wndControl) -- SpellbookAbilityPointsReset
	self.bDirtyBit = true
...
function CRB_Abilities:OnCurrentSetSlotsClearAll(wndHandler, wndControl)
	self.bDirtyBit = true
...
function CRB_Abilities:OnToggleDirtyBit(bNewValue)
...
function CRB_Abilities:SlotItemMouseEnter(wndHandler, wndControl)
...
function CRB_Abilities:SlotItemMouseExit(wndHandler, wndControl)
	if wndHandler == wndControl and wndHandler:FindChild("SlotCloseBtn") then
		wndHandler:FindChild("SlotCloseBtn"):Show(false)
	end
end

function CRB_Abilities:OnSlotMouseDown(wndHandler, wndControl, eMouseButton, nPosX, nPosY, bDoubleClick)
...
function CRB_Abilities:OnSlotCloseBtnClick(wndHandler, wndControl)
...
function CRB_Abilities:OnStartDragSkillFromSlots(wndHandler, wndControl)
...
function CRB_Abilities:OnQueryDragSkillOverSlotItem(wndHandler, wndControl)
	return Apollo.DragDropQueryResult.Accept -- This event is on the Slot (not the Spellbook) so always accept
end

function CRB_Abilities:OnEndDragSkillOntoSlotItem(wndHandler, wndControl)
...
function CRB_Abilities:OnDragDropEnd()
...
function CRB_Abilities:HelperShowError(strMessage)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_Abilities:OnErrorContainerHideBtn()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
	self.timerHideErrorContainer:Stop()
...
function CRB_Abilities:OnLeaveConfirmationNo()
	self.tWndRefs.wndMain:FindChild("LeaveConfirmationContainer"):Show(false)
end

function CRB_Abilities:OnLeaveConfirmationYes()
	self.tWndRefs.wndMain:FindChild("LeaveConfirmationContainer"):Show(false)
	self:OnCloseFinal()
end
...
function CRB_Abilities:OnSetLeftBtn(wndHandler, wndControl)
	AbilityBook.PrevSpec()
end

function CRB_Abilities:OnSetRightBtn(wndHandler, wndControl)
	AbilityBook.NextSpec()
end

function CRB_Abilities:OnSpecChanged(newSpecIndex, specError)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_Abilities:OnActionSetError(eResult)
...
function CRB_Abilities:OnLevelUpUnlock_TierPointSystem(splAbility)
	self:BuildWindow()
...
function CRB_Abilities:OnLevelUpUnlock_AMPSystem()
	self:BuildWindow()
...
function CRB_Abilities:OnLevelUpUnlock_Class_Ability(splAbility)
...
function CRB_Abilities:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor ~= GameLib.CodeEnumTutorialAnchor.CRB_Abilities then return end
...
function CRB_Abilities:HelperRedrawPoints()
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_Abilities:HelperAddSlot(nAbilityId)
...
function CRB_Abilities:HelperDeleteSlot(wndSlotDisplay, bSkipRedrawing)
...
function CRB_Abilities:HelperFindNextTier(tCurrAbility)
...
function CRB_Abilities:OnGenerateTooltip(wndHandler, wndControl, tType, splTarget)
...
function CRB_Abilities:OnSpellbookProgPieceTooltip(wndHandler, wndControl, tType, splTarget)
	Tooltip.GetSpellTooltipForm(self, wndHandler, wndHandler:GetData(), {bTiers = true})
end

function CRB_Abilities:HelperSortLevelReqThenID(a, b) -- Priority1: Level Req. Priority2: Ability ID.
...
function CRB_Abilities:FactoryProduce(wndParent, strFormName, tObject)
...
local CRB_AbilitiesInst = CRB_Abilities:new()
