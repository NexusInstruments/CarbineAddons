-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_LevelUpUnlocks
...
function CRB_LevelUpUnlocks:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_LevelUpUnlocks:Init()
    Apollo.RegisterAddon(self)
end

function CRB_LevelUpUnlocks:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_LevelUpUnlocks.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_LevelUpUnlocks:OnDocumentReady()
...
function CRB_LevelUpUnlocks:OnInterfaceMenuListHasLoaded()
...
function CRB_LevelUpUnlocks:Initialize()
...
function CRB_LevelUpUnlocks:OnToggleLevelUpUnlockWindow()
...
function CRB_LevelUpUnlocks:OnPlayerLevelChange(nLevel)
	GameLib.GetUnlocksForLevel(nLevel, GameLib.LevelUpUnlockSystem.Level)
	self:OnClose()
	self:OnCloseToReminder()
end

function CRB_LevelUpUnlocks:OnPathLevelUp(nLevel)
...
function CRB_LevelUpUnlocks:DisplayLevelUpUnlockPermanentWindow(nSpecificLevel)
...
function CRB_LevelUpUnlocks:DisplayLevelUpUnlockDetailsWindow() -- Also from XML, such as the fanfare window
...
function CRB_LevelUpUnlocks:BuildLevelItem(tUnlock, wndParent, nCurrentLevel)
...
function CRB_LevelUpUnlocks:OnLevelUpPickerBtn(wndHandler, wndControl) -- Dropdown button in Permanent UI, view another level
...
function CRB_LevelUpUnlocks:OnLevelUpFilterBtn(wndHandler, wndControl) -- Filter buttons in Permanent UI
	self.wndPermanent:FindChild("LevelUpPickerList"):SetVScrollPos(0)
	self.wndPermanent:FindChild("LevelUpPickerListFrame"):Show(false)
	local nLevel
	local bShowPath = self.wndPermanent:FindChild("LevelUpPathFilterBtn"):IsChecked()
...
function CRB_LevelUpUnlocks:OnLevelUpItemActionBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_LevelUpUnlocks:OnLevelUpUnlocksMarkAllAsSeenBtn(wndHandler, wndControl, eMouseButton)
	for idx, tUnlock in pairs(GameLib.GetPendingLevelUpUnlocks() or {}) do
		GameLib.MarkLevelUpUnlockViewed(tUnlock.nId, true)
	end
	self:OnClose()
...
function CRB_LevelUpUnlocks:OnReplayLevelUp( wndHandler, wndControl, eMouseButton )
...
function CRB_LevelUpUnlocks:OnCloseToDetailsWindow()
	self:OnClose()
	self:DisplayLevelUpUnlockDetailsWindow()
end

function CRB_LevelUpUnlocks:OnCloseToReminder() -- Also from XML
	self:OnClose()
...
function CRB_LevelUpUnlocks:OnCloseToPermanentWindow()
...
function CRB_LevelUpUnlocks:OnClose() -- Lots of things will call this
	self.timerAutoHide:Stop()
...
function CRB_LevelUpUnlocks:FactoryProduce(wndParent, strFormName, tObject)
...
local CRB_LevelUpUnlocksInst = CRB_LevelUpUnlocks:new()
CRB_LevelUpUnlocksInst:Init()
