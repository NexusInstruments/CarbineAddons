-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_CrowdControlGameplay
...
local CRB_CrowdControlGameplay = {}

function CRB_CrowdControlGameplay:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_CrowdControlGameplay:Init()
    Apollo.RegisterAddon(self)
end

function CRB_CrowdControlGameplay:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CrowdControlGameplay.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_CrowdControlGameplay:OnDocumentReady()
...
function CRB_CrowdControlGameplay:OnActivateCCStateStun(eChosenDirection)
...
function CRB_CrowdControlGameplay:OnRemoveCCStateStun() -- Also from lua
	if self.wndProgress and self.wndProgress:IsValid() then
		self.wndProgress:Destroy()
		self.wndProgress = nil
	end
...
function CRB_CrowdControlGameplay:OnUpdateCCStateStun(fProgress) -- Updates Progress Bar
...
function CRB_CrowdControlGameplay:OnCalculateTimeRemaining()
...
function CRB_CrowdControlGameplay:OnStunVGPressed(bPushed)
	if self.wndProgress and self.wndProgress:IsValid() then
...
local CRB_CrowdControlGameplayInst = CRB_CrowdControlGameplay:new()
CRB_CrowdControlGameplayInst:Init()
