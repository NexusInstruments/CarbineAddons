...
function CRB_Dialog:Init()
	Apollo.RegisterAddon(self)
end

---------------------------------------------------------------------------------------------------
-- CRB_Dialog EventHandlers
---------------------------------------------------------------------------------------------------
function CRB_Dialog:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Dialog.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Dialog:OnDocumentReady()
	if  self.xmlDoc == nil then
		return
	end
	Apollo.LoadSprites("UI\\CRB_Dialog\\DialogSprites.xml") -- Old
...
---------------------------------------------------------------------------------------------------
function CRB_Dialog:OnDialog_Close()
...
function CRB_Dialog:OnDialog_ShowState(eState, queCurrent)
	local idQuest = 0
...
function CRB_Dialog:DrawResponses(eState, idQuest, tResponseList)
...
function CRB_Dialog:OnResponseBtnClick(wndHandler, wndControl, eMouseButton) -- ResponseItemBtn
...
function CRB_Dialog:OnLootItemMouseUp(wndHandler, wndControl, eMouseButton)
...
function CRB_Dialog:HelperComputeIconPath(eResponseType)
...
function CRB_Dialog:HelperComputeRewardTextColor(idReward, tChoiceRewardData)
	if idReward == 0 then
...
function CRB_Dialog:HelperComputeRewardIcon(wndCurr, idReward, tChoiceRewardData)
	if idReward == 0 then
		return
	end
...
function CRB_Dialog:HelperDrawLootItem(wndCurrReward, tCurrReward, bSimple)
	local strIconSprite = ""
...
function CRB_Dialog:OnResponseItemMouseUp(wndHandler, wndControl, eMouseButton)
	if eMouseButton == GameLib.CodeEnumInputMouse.Right and wndHandler:GetData() then
		Event_FireGenericEvent("GenericEvent_ContextMenuItem", wndHandler:GetData())
	end
end
...
function CRB_Dialog:DrawItemBubble(wndArg, eState, idQuest)
...
function CRB_Dialog:DrawNpcBubble(wndArg, eState, idQuest)
	-- Text
...
function CRB_Dialog:SortRewardItems(tRewards)
...
function CRB_Dialog:PositionNpcBubble(wndArg)
...
function CRB_Dialog:HelperExpandForText(nNewTop, wndArg)
...
function CRB_Dialog:OnDestroyTooltip(wndHandler, wndControl)
	if wndHandler == wndControl then
		wndHandler:SetTooltipDoc(nil)
	end
end

function CRB_Dialog:OnGenerateTooltip(wndHandler, wndControl, eType, arg1, arg2)
	if wndHandler ~= wndControl	then
		return
	end
...
function CRB_Dialog:OnWindowClosed(wndHandler, wndControl) -- The 'esc' key from xml
	if wndHandler:GetId() ~= wndControl:GetId() then return end
	DialogSys.End()
end

function CRB_Dialog:OnUpdateTimer(strVarName, nCount)
	if self.wndNpc and self.wndNpc:IsShown() then
		self:PositionNpcBubble(self.wndNpc)
	end
end

function CRB_Dialog:HelperPrereqFailed(itemCurr)
...
-- CRB_Dialog instance
---------------------------------------------------------------------------------------------------
local CRB_DialogInst = CRB_Dialog:new()
CRB_DialogInst:Init()
