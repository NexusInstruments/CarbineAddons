...
---------------------------------------------------------------------------------------------------
-- CRB_GroupFrame initialization
---------------------------------------------------------------------------------------------------
function CRB_GroupFrame:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
...
function CRB_GroupFrame:Init()
	Apollo.RegisterAddon(self)
end

function CRB_GroupFrame:OnSave(eType)
...
function CRB_GroupFrame:OnRestore(eType, tSavedData)
...
---------------------------------------------------------------------------------------------------
-- CRB_GroupFrame EventHandlers
---------------------------------------------------------------------------------------------------
function CRB_GroupFrame:OnLoad()
	self.xmlOptionsDoc = XmlDoc.CreateFromFile("CRB_GroupFrameOptions.xml")
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_GroupFrame.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_GroupFrame:OnDocumentReady()
	...
	---------------------------------------------------------------------------------------------------
	-- CRB_GroupFrame Member Variables
	---------------------------------------------------------------------------------------------------
...
function CRB_GroupFrame:OnWindowManagementReady()
...
function CRB_GroupFrame:GroupDisplayOptions_TEMP()
	-- TEMP HACK: Try again in case this loads first
	Event_FireGenericEvent("GenericEvent_InitializeGroupLeaderOptions", self.wndGroupHud:FindChild("GroupControlsBtn"))
end
...
function CRB_GroupFrame:LoadPortrait(idx)
...
function CRB_GroupFrame:OnGroupInvited(strInviterName) -- builds the invite when I recieve it
...
function CRB_GroupFrame:OnGroupJoinRequest(strInviterName) -- builds the invite when I recieve it
...
function CRB_GroupFrame:OnGroupReferral(nMemberIndex, strTarget) -- builds the invite when I receive it
	-- undone need token passed as context
	-- a join message means that someone has requested to join our existing party
...
function CRB_GroupFrame:OnInviteTimer()
...
function CRB_GroupFrame:OnGroupInviteDialogAccept()
...
function CRB_GroupFrame:OnGroupInviteDialogDecline()
...
function CRB_GroupFrame:OnReportGroupInviteSpamBtn()
...
function CRB_GroupFrame:OnRaidInfoResponse(arRaidInfo)
...
function CRB_GroupFrame:DestroyGroup()
...
function CRB_GroupFrame:PostChangeToChannel(nPrevValue, nNextValue, tDescriptionTable, strChangeString, strUnknownChangeString)
...
function CRB_GroupFrame:OnGroupUpdated()
	if GroupLib.InRaid() then
		return
	end
...
function CRB_GroupFrame:OnGroupLootRulesChanged()
	if GroupLib.InRaid() then
		return
	end
...
function CRB_GroupFrame:OnGroupControlsCheck(wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_UpdateGroupLeaderOptions")
end

function CRB_GroupFrame:OnGroupPortraitClick(wndHandler, wndControl, eMouseButton)
...
function CRB_GroupFrame:OnGroupBagBtn()
	Event_FireGenericEvent("GenericEvent_ToggleGroupBag")
end

function CRB_GroupFrame:OnMasterLootUpdate()
...
function CRB_GroupFrame:OnKick()
...
function CRB_GroupFrame:OnLocate()
	if self.nGroupMemberClicked == nil then
		return
	end
...
function CRB_GroupFrame:OnPromote()
	if self.nGroupMemberClicked == nil then
		return
	end
...
function CRB_GroupFrame:ShowConfirmLeaveDisband(nType)
...
function CRB_GroupFrame:OnLeaveGroup()
	self:ShowConfirmLeaveDisband(1)
end

function CRB_GroupFrame:OnDisbandGroup()
	self:ShowConfirmLeaveDisband(0)
end

function CRB_GroupFrame:OnConfirmLeave()
	GroupLib.LeaveGroup()
	self:DestroyGroup()
end

function CRB_GroupFrame:OnConfirmDisband()
...
function CRB_GroupFrame:OnCancelLeave()
...
-- Format Members
---------------------------------------------------------------------------------------------------
function CRB_GroupFrame:DrawMemberPortrait(tPortrait, tMemberInfo)
...
function CRB_GroupFrame:HelperUpdateHealth(tPortrait, tMemberInfo)
...
function CRB_GroupFrame:SetBarValue(wndBar, fMin, fValue, fMax)
	wndBar:SetMax(fMax)
...
function CRB_GroupFrame:OnUpdateTimer(strVar, nValue)
...
function CRB_GroupFrame:OnGroupAdd(strMemberName) -- Someone else joined my group
	local strMsg = String_GetWeaselString(Apollo.GetString("GroupJoin"), strMemberName)
	self:AddToQueue(ktMessageIcon.Accept, strMsg)
	self:OnGroupUpdated()
end

function CRB_GroupFrame:OnGroupJoin() -- I joined a group
	self:OnGroupUpdated()
...
function CRB_GroupFrame:OnGroupRemove(strMemberName, eReason) -- someone else left the group
...
function CRB_GroupFrame:OnGroupMemberPromoted(strMemberName, bSelf) -- I've been promoted
	if bSelf then
...
function CRB_GroupFrame:OnGroupOperationResult(strMemberName, eResult)
...
function CRB_GroupFrame:OnGroupAcceptInvite() -- I've accepted an invitation
	self.wndGroupInviteDialog:Show(false)
end

function CRB_GroupFrame:OnGroupDeclineInvite() -- I've declined an invitation
	self.wndGroupInviteDialog:Show(false)
end

function CRB_GroupFrame:OnLootRollUpdate()
--[[
...
function CRB_GroupFrame:OnGroupLeft(eReason)
	local unitMe = GameLib.GetPlayerUnit()
	if unitMe == nil then
		return
    end
...
function CRB_GroupFrame:OnGroupMemberFlags(nMemberIndex, bIsFromPromotion, tChangedFlags)
...
function CRB_GroupFrame:OnGroupReadyCheck(nMemberIndex, strMessage)
	local tMember = GroupLib.GetGroupMember(nMemberIndex)
...
function CRB_GroupFrame:OnGroupInviteResult(strCharacterName, eResult)

	Apollo.DPF("CRB_GroupFrame:OnGroupInviteResult")
...
function CRB_GroupFrame:OnGroupRequestResult(strCharacterName, eResult, bIsJoin)
	Apollo.DPF("CRB_GroupFrame:OnGroupRequestResult")
...
function CRB_GroupFrame:CloseGroupHUD() -- see if the HUD can be closed
...
-- Message Queue
---------------------------------------------------------------------------------------------------
function CRB_GroupFrame:AddToQueue(nMessageIcon, strMessageText)
...
function CRB_GroupFrame:ProcessAlerts()
	self:ClearFields()
...
function CRB_GroupFrame:DisplayAlert(nMessageIcon, strMessageText)
...
---------------------------------------------------------------------------------------------------
function CRB_GroupFrame:OnGroupMessageTimer()
	self.wndGroupMessage:FindChild("MessageBirthAnimation"):ToFront()
	self.wndGroupMessage:FindChild("MessageBirthAnimation"):SetSprite("sprWinAnim_BirthSmallTemp")
...
---------------------------------------------------------------------------------------------------
function CRB_GroupFrame:ClearFields() --clear everything
	for idx =1, #karMessageIconString do
		self.wndGroupMessage:FindChild(karMessageIconString[idx]):Show(false)
	end
...
function CRB_GroupFrame:OnGroupWrongInstance()
...
function CRB_GroupFrame:HelperResizeGroupContents()
...
function CRB_GroupFrame:OnGroupMentor(tMemberList, bCurrentlyMentoring, bUpdateOnly)
...
function CRB_GroupFrame:OnToggleMentorItem(wndHandler, wndCtrl)
...
function CRB_GroupFrame:OnMentorPlayerBtn(wndHandler, wndCtrl)
...
function CRB_GroupFrame:OnCancelMentoringBtn(wndHandler, wndCtrl)
...
function CRB_GroupFrame:OnMentorCloseBtn(wndHandler, wndCtrl)
	self.wndMentor:Show(false)
	self.fMentorTimerStartTime = nil
	Apollo.StopTimer("MentorTimer")
...
function CRB_GroupFrame:OnMentorTimer()
...
function CRB_GroupFrame:OnGroupMentorLeftAOI(nTimeUntilMentoringDisabled, bClearUI)
	if bClearUI then
...
function CRB_GroupFrame:OnMentorAOICloseBtn()
	self.wndMentorAOI:Show(false)
	Apollo.StopTimer("MentorAOITimer")
...
function CRB_GroupFrame:OnMentorAOITimer()
	-- This is the timer that's shown on the window
...
function CRB_GroupFrame:OnAcceptRequest()
	self.wndRequest:Show(false)
	GroupLib.AcceptRequest()
end

function CRB_GroupFrame:OnDenyRequest()
	self.wndRequest:Show(false)
...
function CRB_GroupFrame:OnRaidOkay( wndHandler, wndControl, eMouseButton )
	if self.wndRaidNotice and self.wndRaidNotice:IsValid() then
...
---------------------------------------------------------------------------------------------------
-- CRB_GroupFrame instance
---------------------------------------------------------------------------------------------------
local GroupFrameInst = CRB_GroupFrame:new()
CRB_GroupFrame:Init()
