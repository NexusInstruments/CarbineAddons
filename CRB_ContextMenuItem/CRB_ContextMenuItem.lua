-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ContextMenuItem
...
local CRB_ContextMenuItem = {}

function CRB_ContextMenuItem:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_ContextMenuItem:Init()
    Apollo.RegisterAddon(self)
end

function CRB_ContextMenuItem:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ContextMenuItem.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_ContextMenuItem:OnDocumentReady()
...
function CRB_ContextMenuItem:InitializeItemObject(itemArg)
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
	end

	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_ContextMenuItemForm", "TooltipStratum", self)
...
function CRB_ContextMenuItem:OnDecorPreviewOpen(nArgDecorId) -- Currently unable to offer Link to Chat, which requires the object
	Event_FireGenericEvent("GenericEvent_LoadDecorPreview", nArgDecorId)
end

function CRB_ContextMenuItem:OnSplitItemStack(itemArg) -- This is a common enough shortcut (Shift + Left Click) to skip the menu
	Event_FireGenericEvent("GenericEvent_SplitItemStack", itemArg)
end

function CRB_ContextMenuItem:OnRegularBtn(wndHandler, wndControl) -- Can be any of the buttons
	...
	elseif strButtonName == "BtnUnlockCostume" then
		Event_FireGenericEvent("GenericEvent_CostumeUnlock", itemArg)
	end
...
function CRB_ContextMenuItem:HelperValidateEditRunes(itemArg)
...
function CRB_ContextMenuItem:HelperValidatePreview(itemArg)
...
function CRB_ContextMenuItem:HelperValidateCostumeUnlock(itemArg)
	if not itemArg then
		return false
	end
...
local CRB_ContextMenuItemInst = CRB_ContextMenuItem:new()
CRB_ContextMenuItemInst:Init()
