-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Achievements
...
function CRB_Achievements:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_Achievements:Init()
    Apollo.RegisterAddon(self)
end

-----------------------------------------------------------------------------------------------
-- CRB_Achievements OnLoad
-----------------------------------------------------------------------------------------------

function CRB_Achievements:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Achievements.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Achievements:OnDocumentReady()
...
function CRB_Achievements:OnInterfaceMenuListHasLoaded()
	Event_FireGenericEvent("InterfaceMenuList_NewAddOn", Apollo.GetString("CRB_Achievements"), {"ToggleAchievementWindow", "CRB_Achievements", "Icon_Windows32_UI_CRB_InterfaceMenu_Achievements"})
end

function CRB_Achievements:OnAchievementUpdated(achUpdated) -- TODO: Pinpoint the redraw instead of a full redraw
...
function CRB_Achievements:UpdateAchievementEntry(achUpdated)
	local wndRightScroll = self.wndMain:FindChild("RightScroll")
...
function CRB_Achievements:ToggleWindow(achPassedAchievement)
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_AchievementsForm", g_wndProgressLog:FindChild("ContentWnd_4"), self)
...
function CRB_Achievements:BuildCategoryTree()
...
function CRB_Achievements:ResizeTree()
	local wndLeftScroll = self.wndMain:FindChild("BGLeft:LeftScroll")
...
function CRB_Achievements:UnselectAll() -- TODO: REFACTOR, this mostly simulates a global radio group
...
function CRB_Achievements:OnTopGroupSelect(wndHandler, wndControl)
...
function CRB_Achievements:OnMiddleGroupSelect(wndHandler, wndControl)
...
function CRB_Achievements:OnBottomItemSelect(wndHandler, wndControl)
...
function CRB_Achievements:LoadRightScreenFromLeftScreen(wndCurr)
...
function CRB_Achievements:LoadSummaryScreen() -- TODO: Figure out why this is being called called when the Achievement list is filtered/built
	self:UnselectAll()
...
function CRB_Achievements:OnRecentUpdateBtn(wndHandler, wndControl)
...
function CRB_Achievements:OnZoomToAchievementIfValid(achArg)
...
function CRB_Achievements:OnHeaderFilterBtn() -- Route through here so we know when to clear the search focus (and more importantly, when not to)
	self.wndMain:FindChild("BGHeader:HeaderSearchBox"):ClearFocus()
	self:BuildRightPanel()
end


function CRB_Achievements:BuildRightPanel()
	local wndRightScroll = self.wndMain:FindChild("RightScroll")
...
function CRB_Achievements:BuildSimpleAchievement(wndContainer, achData)
...
function CRB_Achievements:BuildChecklistAchievement(achData)
...
function CRB_Achievements:UpdateChecklistAchievement(achData, wndContainer)
...
function CRB_Achievements:BuildTieredAchievement(achData)
	local wndContainer = Apollo.LoadForm(self.xmlDoc, "AchievementSimple", self.wndMain:FindChild("RightScroll"), self)
...
function CRB_Achievements:BuildTieredItem(achData, wndTierBox)
...
function CRB_Achievements:UpdateTierItem(achData, wndTierItem)
	if achData:IsCurrencyShown() then
...
function CRB_Achievements:OnTierItemCheck(wndHandler, wndControl) -- wndHandler is "TierItemBtn"
	-- TODO: Quick hack: Both the select and unselect route here to simulate DisallowUnselect
	self.wndMain:FindChild("BGHeader:HeaderSearchBox"):ClearFocus()
...
function CRB_Achievements:OnAchievementExpanderBtn(wndHandler, wndControl) -- the actual button so we can clear search
	self.wndMain:FindChild("BGHeader:HeaderSearchBox"):ClearFocus()
	self:OnAchievementExpand(wndHandler:GetData() or wndHandler:GetParent()) -- TODO refactor
end

function CRB_Achievements:OnAchievementExpand(wndParent) -- the function it triggers
	local achSelected = wndParent:GetData()
...
function CRB_Achievements:OnClearSearchBtn(wndHandler, wndControl)
	local wndHeader = self.wndMain:FindChild("BGHeader")
...
function CRB_Achievements:OnTabPlayerBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_Achievements:OnTabGuildBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_Achievements:HelperCheckFilters(achFiltered)
	local bResult = false
	local wndHeader = self.wndMain:FindChild("BGHeader")
...
function CRB_Achievements:GetTableSize(tArg)
    local nCounter = 0
    if tArg ~= nil then
...
function CRB_Achievements:HelperNumberToRomanNumerals(nValue)
	local strRoman = ""
...
function CRB_Achievements:LoadByName(strForm, wndParent, strCustomName)
...
function CRB_Achievements:ResizeHelper(wndContainer)
...
local CRB_AchievementsInst = CRB_Achievements:new()
CRB_AchievementsInst:Init()
