...
function CRB_Datachron:Init()
	Apollo.RegisterAddon(self)
end

function CRB_Datachron:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Datachron.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_Datachron:OnDocumentReady()
	...
	g_wndDatachron 			= Apollo.LoadForm(self.xmlDoc, "CRB_Datachron", "FixedHudStratumHigh", self) -- Do not rename. This is global and used by other forms as a parent.
...
function CRB_Datachron:SetPath()
...
function CRB_Datachron:OnGenericEvent_RestoreDatachron()
	if not g_wndDatachron:IsShown() then
		self:OnRestoreDatachron()
	end
end

function CRB_Datachron:OnMinimizeDatachron()
...
function CRB_Datachron:OnRestoreDatachron()
...
function CRB_Datachron:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor ~= GameLib.CodeEnumTutorialAnchor.CRB_Datachron then return end
...
function CRB_Datachron:OnCommPotraitClicked()
	self:OnCommPlayBtn() -- From Comm Display
end

function CRB_Datachron:HideCommDisplay()
	Event_FireGenericEvent("HideCommDisplay")
	self.idCreature = nil
	self:DrawCallSystem()
end

function CRB_Datachron:SetCommunicatorCreature(idCreature)
...
function CRB_Datachron:OnShowResurrectDialog()
	self:DrawCallSystem("TurnOffButtons")
end

function CRB_Datachron:OnPlayerResurrected()
	self:ProcessDatachronState()
end

function CRB_Datachron:OnDialog_Close() -- User clicks done in the dialog speech bubbles
	self.timerNPCBubbleFade:Stop()
...
function CRB_Datachron:OnNpcBubbleFade()
...
function CRB_Datachron:OnCommDisplay_Closed()
...
function CRB_Datachron:OnCommunicator_SpamVOEnded()
	if self.strCurrentState == "SpamOngoing" then
		self:DrawCallSystem("SpamEnd")
	end
end

function CRB_Datachron:OnCommunicator_EndIncoming()
...
function CRB_Datachron:OnDialog_ShowState(eState, queCurr) -- Talking to an NPC
	local idQuest = 0
	if queCurr then
...
function CRB_Datachron:OnCommunicator_ShowQuestMsg(idMsg, idCreature, queSource, strText)
...
function CRB_Datachron:OnCommShowSpamMsg(idMsg, idCreature, strText, bPriorty)
...
function CRB_Datachron:OnQuestStateChanged()
...
function CRB_Datachron:OnDatachron_HideCallPulse() -- One click hide from the HUD Interact, until a new state
...
function CRB_Datachron:OnCommPlayBtn()
...
function CRB_Datachron:OnDenyCallbackBtn(wndHandler, wndControl) -- CommCallDeny
...
function CRB_Datachron:OnQueuedCallsItemClick(wndHandler, wndControl)
...
function CRB_Datachron:OnQueuedCallsIgnoreBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl or not wndHandler:GetParent() or not wndHandler:GetParent():GetData() then
		return
	end
...
function CRB_Datachron:BuildCallbackList()
	local bResult = false
...
function CRB_Datachron:DrawCallSystem(strNewState)
...
function CRB_Datachron:ProcessDatachronState()
...
function CRB_Datachron:ProcessSpamQueue()
	if self.strCurrentState == "DialogOngoing" then
		self:HideCommDisplay()
	end
...
function CRB_Datachron:Datachron_MaxMissedCallTimer()
...
local CRB_DatachronInst = CRB_Datachron:new()
CRB_DatachronInst:Init()
