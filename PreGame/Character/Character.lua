...
local ktIcons = {
	[keTutorialTab.Experience] = {
		[keTutorialType.Novice] = "charactercreate:sprCharC_Finalize_SkillLevel1",
		[keTutorialType.Intermediate] = "charactercreate:sprCharC_Finalize_SkillLevel2",
		[keTutorialType.Expert] = "charactercreate:sprCharC_Finalize_SkillLevel3",
	},
...
local keNoClass = PreGameLib.CodeEnumClass.Warrior
local keNoRace = PreGameLib.CodeEnumRace.Human
local keNoGender = PreGameLib.CodeEnumGender.Male
...
function Character:Print(strText, bAppend)
	self.wndSubSection:FindChild("Title"):SetText(strText)
end
...
function Character:OnActorMouseEnter(actor)
	if self.wndRaceContent == nil or not self.wndRaceContent:IsValid() or not self.tSelectedAcorInfo or self.tSelectedAcorInfo.actor ~= actor then
		return
	end
	
	self.tSelectedAcorInfo.wndDescription:Show(true)
end
...
function Character:UpdateCharacterModelAndPosition()
	if self.bChangedDetailOrSwitchedTab then
		if self.eRace then
			...
			local nPossibleCharacterCreateIndex = self:GetCharacterCreateId(self.eFaction, eRaceExternal, self.eClass, self.eRace ~= PreGameLib.CodeEnumRace.Chua and self.eGender or PreGameLib.CodeEnumGender.Male)
...
function Character:HelperEnableEnterBtn()
	if g_controls then
		local bEnable = self.eTutorialLevel ~= nil and self.eRace ~= nil and self.eClass ~= nil and self.ePath ~= nil and self.eFaction ~= nil and self.eGender ~= nil and self.strName ~= nil and CharacterScreenLib.IsCharacterNameValid(self.strName)
		g_controls:FindChild("EnterBtn"):Enable(bEnable)
		self.wndGlobal:FindChild("Header:FinalizeTab:Icon"):SetSprite(bEnable and kstrSelectedTabIcon or kstrDefaultTabIcon)
	end
end
	...
	if g_eState == LuaEnumState.Create then
		...
		if string.len(self.strName) > 0 and tCreation then
			...
			if self.wndExperienceContent:FindChild("ExpertBtn"):IsChecked() then
				tSkipTutorialCreationIds = CharacterScreenLib.GetCharacterCreationIdsByValues(PreGameLib.CodeEnumCharacterCreationStart.Nexus, tCreation.factionId, tCreation.classId, tCreation.raceId, tCreation.genderId)
			elseif self.wndExperienceContent:FindChild("NoviceBtn"):IsChecked()  then
				tSkipTutorialCreationIds = CharacterScreenLib.GetCharacterCreationIdsByValues(PreGameLib.CodeEnumCharacterCreationStart.PreTutorial, tCreation.factionId, tCreation.classId, tCreation.raceId, tCreation.genderId)
			end
			
			if tSkipTutorialCreationIds and tSkipTutorialCreationIds.arEnabledIds and tSkipTutorialCreationIds.arEnabledIds[1] then
...
function Character:CheckForConflicts(eRace, eClass)
	...
	if tClasses then
		if not tClasses[eClass] then --this race does not support this class
			bShowAlert = true
		end
	end
	...
	if bShowAlert then
		...
		elseif eCurTab == keTutorialTab.Class or eCurTab == keTutorialTab.Finalize and wndOptionToggles:FindChild("ClassOptionToggle:Btn"):IsChecked() then
			strMessage = PreGameLib.String_GetWeaselString(Apollo.GetString("PreGame_ConflictRaceDesc"), strRace, strClass)
		end
...
function Character:OnExperienceBtnCheck(wndHandler, wndControl, eMouseButton, bDoubleClick)
	...
	local strExperience = ""
	if self.eTutorialLevel == keTutorialType.Novice then
		strExperience = Apollo.GetString("CRB_Tradeskill_Novice")
	elseif self.eTutorialLevel == keTutorialType.Intermediate then
		strExperience = Apollo.GetString("PreGame_NewToWildStar")
	elseif self.eTutorialLevel == keTutorialType.Expert then
...
function Character:PlacePointer(eFaction, eGender, eRace, tDisplayInfo)
	...
	local wndClassAvailabilityIcons = wndPointer:FindChild("ClassAvailabilityIcons")
	...
	local tClasses = self.tAvailableClassesForRace[eRace]
	for eClass, bValue in pairs(tClasses) do
	...
	local tClasses = self.tAvailableClassesForRace[eRace]
	for eClass, bValue in pairs(tClasses) do
		if eClass == PreGameLib.CodeEnumClass.Engineer then
			wndClassAvailabilityIcons:FindChild("ico_Engineer"):SetBGColor("UI_WindowBGDefault")
		elseif eClass == PreGameLib.CodeEnumClass.Esper then
			wndClassAvailabilityIcons:FindChild("ico_Esper"):SetBGColor("UI_WindowBGDefault")
		elseif eClass == PreGameLib.CodeEnumClass.Medic then
			wndClassAvailabilityIcons:FindChild("ico_Medic"):SetBGColor("UI_WindowBGDefault")
		elseif eClass == PreGameLib.CodeEnumClass.Spellslinger then
			wndClassAvailabilityIcons:FindChild("ico_Spellslinger"):SetBGColor("UI_WindowBGDefault")
		elseif eClass == PreGameLib.CodeEnumClass.Stalker then
			wndClassAvailabilityIcons:FindChild("ico_Stalker"):SetBGColor("UI_WindowBGDefault")
		elseif eClass == PreGameLib.CodeEnumClass.Warrior then
			wndClassAvailabilityIcons:FindChild("ico_Warrior"):SetBGColor("UI_WindowBGDefault")
		end
	end
...
function Character:SetAllOptionData()
	...
	wndBtns:FindChild("NoviceBtn"):SetData(keTutorialType.Novice)
	wndBtns:FindChild("IntermediateBtn"):SetData(keTutorialType.Intermediate)
	wndBtns:FindChild("ExpertBtn"):SetData(keTutorialType.Expert)
	...
	wndBtns:FindChild("NoviceEntry:ExperienceBtn"):SetData(keTutorialType.Novice)
	wndBtns:FindChild("IntermediateEntry:ExperienceBtn"):SetData(keTutorialType.Intermediate)
	wndBtns:FindChild("ExperiencedEntry:ExperienceBtn"):SetData(keTutorialType.Expert)
	...
	wndBtns = self.wndClassContent:FindChild("Buttons")
	wndBtns:FindChild("btn_Engineer"):SetData(PreGameLib.CodeEnumClass.Engineer)
	wndBtns:FindChild("btn_Esper"):SetData(PreGameLib.CodeEnumClass.Esper)
	wndBtns:FindChild("btn_Medic"):SetData(PreGameLib.CodeEnumClass.Medic)
	wndBtns:FindChild("btn_Spellslinger"):SetData(PreGameLib.CodeEnumClass.Spellslinger)
	wndBtns:FindChild("btn_Stalker"):SetData(PreGameLib.CodeEnumClass.Stalker)
	wndBtns:FindChild("btn_Warrior"):SetData(PreGameLib.CodeEnumClass.Warrior)

	for idx, wndClassBtn in pairs(wndBtns:GetChildren()) do
...
function Character:HelperLoadClassAndRaceRelations()
	...
	for idx, tCreation in pairs(self.arCharacterCreateOptions) do
		...
		if not tClassesForRace[tCreation.classId]  then
			self.tAvailableClassesForRace[eRaceInteral][tCreation.classId] = true
		end
		...
		if not self.tAvailableRacesForClass[tCreation.classId][eRaceInteral] then
			self.tAvailableRacesForClass[tCreation.classId][eRaceInteral] = true
		end
	end
end
...
function Character:SetConflictWarnings()
	...
	for idx, tTutorialAndFinalizeBtns in pairs(self.tTutorialAndCustomizationLink[keTutorialTab.Class]) do
		...
		if self.eRace then
			bClassIsAvailable = tEnabledClasses[eClass] or false
		end

		local wndTutorialIcon = tTutorialAndFinalizeBtns.wndTutorialBtn:FindChild("Icon")
		local wndTutorialConflict = tTutorialAndFinalizeBtns.wndTutorialBtn:FindChild("Conflict")
		local wndFinalizeConflict = tTutorialAndFinalizeBtns.wndFinalizeBtn:FindChild("Conflict")
		wndTutorialIcon:SetOpacity(bClassIsAvailable and 1 or 0.25)
		wndTutorialConflict:Show(not bClassIsAvailable)
		wndFinalizeConflict:Show(not bClassIsAvailable)
	...
	for idx, tTutorialAndFinalizeBtns in pairs(self.tTutorialAndCustomizationLink[keTutorialTab.Race]) do
		...
		if self.eClass then
			bRaceIsAvailable = tEnabledRaces[tRaceData.eRace] or false
		end
		...
		wndTutorialConflict:Show(not bRaceIsAvailable)
		wndFinalizeConflict:Show(not bRaceIsAvailable)
...
function Character:OnPresetBtn(wndHandler, wndControl)
	...
	if strPreset and strPreset ~= "" then
		g_arActors.primary:SetBySliderCodes(strPreset)
		g_arActors.shadow:SetBySliderCodes(strPreset)
	end
...
--eFaction is required.  If any of the other variables aren't set, we'll get a random value
function Character:GetCharacterCreateId(eFaction, eRace, eClass, eGender)
	...
	self.bUsingDefaultIds = false
	--if we pass no race or an invalid one, get a new one
	if not eRace or not self.mapCharacterCreateOptions[eFaction][eRace] then
		local tRaces = {}
		for idx, idOption in pairs(self.mapCharacterCreateOptions[eFaction]) do
			if not eClass or self.mapCharacterCreateOptions[eFaction][idx][eClass] then
				table.insert(tRaces, idx)
			end
		end

		eRace = tRaces[keNoRace]--Using Default ID
	end

	--if we pass no class or an invalid one, get a new one
	if not eClass or not self.mapCharacterCreateOptions[eFaction][eRace][eClass] then
		local tClasses = {}
		for idx, idOption in pairs(self.mapCharacterCreateOptions[eFaction][eRace]) do
			table.insert(tClasses, idx)
		end

		eClass = tClasses[keNoClass]--Using Default ID
	end

	-- if we pass a gender....you get the idea
	if not eGender or not self.mapCharacterCreateOptions[eFaction][eRace][eClass][eGender] then
		local tGenders = {}
		for idx, idOption in pairs(self.mapCharacterCreateOptions[eFaction][eRace][eClass]) do
			table.insert(tGenders, idx)
		end

		eGender = tGenders[keNoGender]--Using Default ID
	end
