-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_ContextMenuPlayer
...
function CRB_ContextMenuPlayer:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_ContextMenuPlayer:Init()
    Apollo.RegisterAddon(self)
end

function CRB_ContextMenuPlayer:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_ContextMenuPlayer.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_ContextMenuPlayer:OnDocumentReady()
...
function CRB_ContextMenuPlayer:SharedInitialize(wndParent)
	if self.wndMain and self.wndMain:IsValid() then
		self.wndMain:Destroy()
		self.wndMain = nil
	end

	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_ContextMenuPlayerForm", "TooltipStratum", self)
	self.wndMain:Invoke()
...
function CRB_ContextMenuPlayer:InitializeFriend(wndParent, nFriendId)
...
function CRB_ContextMenuPlayer:RedrawAllFriend()
...
function CRB_ContextMenuPlayer:Initialize(wndParent, strTarget, unitTarget, tOptionalCharacterData) -- unitTarget may be nil
	self:SharedInitialize(wndParent)
...
function CRB_ContextMenuPlayer:RedrawAll()
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_ContextMenuPlayer:ResizeAndRedraw()
	local wndButtonList = self.wndMain:FindChild("ButtonList")
...
function CRB_ContextMenuPlayer:CheckWindowBounds()
...
function CRB_ContextMenuPlayer:ProcessContextClick(eButtonType)
	if not self.wndMain or not self.wndMain:IsValid() then
		return
	end
...
function CRB_ContextMenuPlayer:OnTargetUnitChanged(unitNewTarget)
...
function CRB_ContextMenuPlayer:OnMainWindowClosed(wndHandler, wndControl)
	if self.wndMain and self.wndMain:IsValid() then
...
function CRB_ContextMenuPlayer:OnRegularBtn(wndHandler, wndControl)
	self:ProcessContextClick(wndHandler:GetData())
	self:OnMainWindowClosed()
end

function CRB_ContextMenuPlayer:OnBtnRegularMouseDown(wndHandler, wndControl)
	self:OnRegularBtn(wndHandler:GetParent(), wndHandler:GetParent())
end

function CRB_ContextMenuPlayer:OnBtnCheckboxMouseDown(wndHandler, wndControl)
...
function CRB_ContextMenuPlayer:OnBtnRegularMouseEnter(wndHandler, wndControl)
	wndHandler:GetParent():FindChild("BtnText"):SetTextColor("UI_BtnTextBlueFlyBy")
end

function CRB_ContextMenuPlayer:OnBtnRegularMouseExit(wndHandler, wndControl)
	wndHandler:GetParent():FindChild("BtnText"):SetTextColor("UI_BtnTextBlueNormal")
end

function CRB_ContextMenuPlayer:OnEventRequestResize()
...
function CRB_ContextMenuPlayer:HelperBuildPetDismissButton(wndButtonList, eButtonType, strButtonText, unitTarget)
...
function CRB_ContextMenuPlayer:HelperBuildRegularButton(wndButtonList, eButtonType, strButtonText)
	-- TODO 2nd argument probably shouldn't be a string, and doesn't need to be localized
...
function CRB_ContextMenuPlayer:HelperEnableDisableRegularButton(wndCurr, bEnable)
...
function CRB_ContextMenuPlayer:FactoryProduce(wndParent, strFormName, tObject)
...
local CRB_ContextMenuPlayerInst = CRB_ContextMenuPlayer:new()
CRB_ContextMenuPlayerInst:Init()
