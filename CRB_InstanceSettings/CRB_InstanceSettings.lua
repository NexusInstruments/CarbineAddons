-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_InstanceSettings
...
local CRB_InstanceSettings = {}

--local knSaveVersion = 1

function CRB_InstanceSettings:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_InstanceSettings:Init()
    Apollo.RegisterAddon(self)
end

function CRB_InstanceSettings:OnSave(eType)
...
function CRB_InstanceSettings:OnRestore(eType, tSavedData)
...
function CRB_InstanceSettings:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_InstanceSettings.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_InstanceSettings:OnDocumentReady()
...
function CRB_InstanceSettings:OnShowRestricted()
	self:DestroyAll()
...
function CRB_InstanceSettings:OnShowDialog(tData)
	self:DestroyAll()
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc , "CRB_InstanceSettingsForm", nil, self)
	self.bHidingInterface = false
...
function CRB_InstanceSettings:OnInstanceResetResult(bResetWasSuccessful)
...
function CRB_InstanceSettings:OnExitInstance()
	GameLib.LeavePendingRemovalInstance()
end

function CRB_InstanceSettings:OnPendingWorldRemovalWarning()
...
function CRB_InstanceSettings:OnPendingWorldRemovalCancel()
...
function CRB_InstanceSettings:OnMessageDisplayTimer()
...
function CRB_InstanceSettings:OnPendingRemovalTimer()
	if self.wndPendingRemoval then
...
function CRB_InstanceSettings:OnNoExistingInstance()
...
function CRB_InstanceSettings:OnOK()
...
function CRB_InstanceSettings:OnReset()
...
function CRB_InstanceSettings:OnHideDialog(bNeedToNotifyServer)
...
function CRB_InstanceSettings:OnCancel()
	self:OnHideDialog(true) -- we must tell the server about this 
end

function CRB_InstanceSettings:OnChangeWorld()
	self:OnPendingWorldRemovalWarning()
end

function CRB_InstanceSettings:DestroyAll()
...
function CRB_InstanceSettings:EnableRally( wndHandler, wndControl, eMouseButton )
	if self.bScalingIsAllowed then
		self.wndMain:FindChild("LevelScalingButton"):Enable(true)
		self.wndMain:FindChild("LevelScalingButton"):Show(true)
		self.wndMain:FindChild("ContentFrameScaling"):Show(true)
...
function CRB_InstanceSettings:DisableRally( wndHandler, wndControl, eMouseButton )
	if self.bScalingIsAllowed then
		self.wndMain:FindChild("LevelScalingButton"):Show(false)
		self.wndMain:FindChild("ContentFrameScaling"):Show(true)
...
local CRB_InstanceSettingsInst = CRB_InstanceSettings:new()
CRB_InstanceSettingsInst:Init()
