-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Circles
...
function CRB_Circles:Init()
    Apollo.RegisterAddon(self)
end

function CRB_Circles:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CirclesMain.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Circles:OnRestore(eType, tSavedData)
...
function CRB_Circles:OnDocumentReady()
...
function CRB_Circles:OnGenericEvent_InitializeCircles(wndParent, guildCurr)
...
function CRB_Circles:OnGenericEvent_DestroyCircles()
...
function CRB_Circles:FullRedrawOfRoster()
...
function CRB_Circles:OnGuildRoster(guildCurr, tRoster) -- Event from CPP
	...
	if guildCurr == self.tWndRefs.wndMain:GetData() then -- Since CRB_Circles and Guild can be up at the same time
...
function CRB_Circles:BuildRosterList(guildCurr, tRoster)
...
function CRB_Circles:OnRosterGridItemClick(wndControl, wndHandler, iRow, iCol, eMouseButton)
...
function CRB_Circles:OnRanksButtonSignal()
...
function CRB_Circles:OnAddRankBtnSignal(wndControl, wndHandler)
	local wndRankPopout = self.tWndRefs.wndMain:FindChild("RosterScreen:RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
...
function CRB_Circles:OnViewRankBtnSignal(wndControl, wndHandler)
...
function CRB_Circles:OnRankSettingsSaveBtn(wndControl, wndHandler)
	local wndRankPopout = self.tWndRefs.wndMain:FindChild("RosterScreen:RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
	local wndSettings = wndRankPopout:FindChild("RankSettingsEntry")
...
function CRB_Circles:OnRankSettingsDeleteBtn(wndControl, wndHandler)
	local wndRankPopout = self.tWndRefs.wndMain:FindChild("RosterScreen:RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
	local wndSettings = wndRankPopout:FindChild("RankSettingsEntry")
...
function CRB_Circles:OnRankSettingsNameChanging(wndControl, wndHandler, strText)
	self:HelperValidateAndRefreshRankSettingsWindow(self.tWndRefs.wndRankPopout:FindChild("RankSettingsEntry"))
end

function CRB_Circles:OnRankSettingsPermissionBtn(wndControl, wndHandler)
	self:HelperValidateAndRefreshRankSettingsWindow(self.tWndRefs.wndRankPopout:FindChild("RankSettingsEntry"))
end

function CRB_Circles:HelperValidateAndRefreshRankSettingsWindow(wndSettings)
...
function CRB_Circles:OnRankSettingsCloseBtn(wndControl, wndHandler)
	local wndRankPopout = self.tWndRefs.wndMain:FindChild("RosterScreen:RankPopout")
	local wndRankContainer = wndRankPopout:FindChild("RankContainer")
	local wndSettings = wndRankPopout:FindChild("RankSettingsEntry")
...
function CRB_Circles:OnGuildRankChange(guildCurr)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() then
		return
	end
...
function CRB_Circles:OnRankPopoutCloseBtn(wndControl, wndHandler)
...
function CRB_Circles:OnOfflineBtn(wndHandler, wndControl)
	self.bShowOffline = wndControl:IsChecked()
	
	self:FullRedrawOfRoster()
end

function CRB_Circles:ResetRosterMemberButtons()
	-- Defaults
	...
	if guildCurr and eMyRank then
		...
		wndDemoteBtn:Enable(bSomeRowIsPicked and bTargetIsUnderMyRank and wndRosterGrid:GetData().nRank ~= 10) -- CRB_Circles can't go below 10
...
function CRB_Circles:OnRosterAddMemberClick(wndHandler, wndControl)
...
function CRB_Circles:OnRosterRemoveMemberClick(wndHandler, wndControl)
...
function CRB_Circles:OnRosterPromoteMemberClick(wndHandler, wndControl) -- wndHandler is "RosterOptionBtnPromote"
...
function CRB_Circles:OnRosterEditNoteSave(wndHandler, wndControl)
	wndHandler:SetFocus()
...
function CRB_Circles:OnRosterAddMemberCloseBtn()
...
function CRB_Circles:OnEditNoteBtn(wndHandler, wndControl)
	if wndControl ~= wndHandler then 
		return 
	end
	local wndNote = self.tWndRefs.wndMain:FindChild("EditNotesContainer")
...
function CRB_Circles:OnAddMemberYesClick(wndHandler, wndControl) -- wndHandler is 'AddMemberEditBox' or 'AddMemberYesBtn', and its data is 'AddMemberEditBox'
	local guildCurr = self.tWndRefs.wndMain:GetData()
...
function CRB_Circles:OnRosterPromoteMemberYesClick(wndHandler, wndControl) -- wndHandler is 'PromoteMemberYesBtn'
	local guildCurr = self.tWndRefs.wndMain:GetData()
	local tMember = self.tWndRefs.wndMain:FindChild("RosterScreen:RosterGrid"):GetData()
...
function CRB_Circles:OnRosterDemoteMemberYesClick(wndHandler, wndControl) -- wndHandler is 'RosterOptionBtnDemote' data should be guildCurr
	local guildCurr = self.tWndRefs.wndMain:GetData()
	local tMember = self.tWndRefs.wndMain:FindChild("RosterScreen:RosterGrid"):GetData()
...
function CRB_Circles:OnRosterRemoveMemberYesClick(wndHandler, wndControl) -- wndHandler is 'RemoveMemberYesBtn'
	local guildCurr = self.tWndRefs.wndMain:GetData()
	local tMember = self.tWndRefs.wndMain:FindChild("RosterScreen:RosterGrid"):GetData()
...
function CRB_Circles:OnRosterLeaveYesClick(wndHandler, wndControl) -- wndHandler is "LeaveBtnYesBtn"
	local guildCurr = self.tWndRefs.wndMain:GetData()
...
function CRB_Circles:OnGuildResult(guildCurr, strName, nRank, eResult)
...
function CRB_Circles:OnCircleAlertDisplayTimer()
	self.tWndRefs.wndMain:FindChild("AlertMessage"):Show(false)
	self.tWndRefs.wndMain:Close()
end
...
function CRB_Circles:OnCircleInvite( strGuildName, strInvitorName, guildType )
...
function CRB_Circles:OnCircleInviteAccept(wndHandler, wndControl)
	GuildLib.Accept()
...
function CRB_Circles:OnCircleInviteDecline() -- This can come from a variety of sources
	GuildLib.Decline()
	if self.wndCircleInvite then
		self.wndCircleInvite:Destroy()
	end
...
function CRB_Circles:OnReportCircleInviteSpamBtn()
...
function CRB_Circles:OnRosterSortToggle(wndHandler, wndControl)
...
function CRB_Circles:SortRoster(tArg, strLastClicked)
...
function CRB_Circles:OnOptionsCloseClick(wndControl)
...
function CRB_Circles:OnGenerateGridTooltip(wndHandler, wndControl, eType, iRow, iColumn)
...
function CRB_Circles:HelperConvertPathToString(ePath)
	local strResult = ""
...
function CRB_Circles:HelperConvertToTime(nDays)
	if nDays == 0 then
...
local CRB_CirclesInst = CRB_Circles:new()
CRB_CirclesInst:Init()
