...
function CRB_PlayerPath:Init()
	Apollo.RegisterAddon(self)
end

function CRB_PlayerPath:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_PlayerPath.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_PlayerPath:OnDocumentReady()
...
function CRB_PlayerPath:OnInterfaceMenuListHasLoaded()
...
function CRB_PlayerPath:OnPathShowFromPL(pepEpisode)
...
function CRB_PlayerPath:OnShowFromDatachron(pmMission) -- used to open to a specific mission
	if not self.wndMissionLog then
		self:Initialize()
	end
...
function CRB_PlayerPath:Initialize()
...
function CRB_PlayerPath:UpdateUIFromEvent() -- Arguments for this can vary
	self:PathRefresh(nil, true)
end

function CRB_PlayerPath:OnBigZoneBtnPress(wndHandler, wndControl)
...
function CRB_PlayerPath:HelperCheckTheFirstCategory(wndControl)--optional parameter
...
function CRB_PlayerPath:OnExpandCategories(wndHandler, wndControl)
...
function CRB_PlayerPath:OnCollapseCategories(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_PlayerPath:OnLootEpisodeRewards(wndHandler, wndControl)
...
function CRB_PlayerPath:OnMissionHighlightTimer()
...
function CRB_PlayerPath:OnMissionHighlightMouseDown(wndHandler, wndControl)
...
function CRB_PlayerPath:PathRefresh(pepEpisode, bFullRedraw) -- A lot of events route here, with the first argument not necessarily valid
...
function CRB_PlayerPath:OnTopRightResetBtn(wndHandler, wndControl)
	self.wndMissionLog:FindChild("TopRight"):SetSprite("CRB_WindowAnimationSprites:sprWinAnim_BirthSmallTemp")
	self:OnRedrawLevels(nil)
end

function CRB_PlayerPath:OnTopRightUpOrDownClick(wndHandler, wndControl)
	self:OnRedrawLevels(wndHandler:GetData())
end

function CRB_PlayerPath:OnRedrawLevels(nArgPreviewLevel)
	if not self.wndMissionLog then
		return
	end
...
function CRB_PlayerPath:DrawMissions(tSelectedEpisode, bFullRedraw)
	if bFullRedraw then
...
function CRB_PlayerPath:HelperDrawCategoryMissions(wndRedraw, tSelectedEpisode)
...
function CRB_PlayerPath:ResizeItems()
...
function CRB_PlayerPath:DrawMissionItem(wnd, pmMission)
...
function CRB_PlayerPath:OnGenerateTooltip(wndHandler, wndControl, eType, arg1, arg2)
	local xml = nil
...
function CRB_PlayerPath:HelperComputeMissionProgress(pmMission)
	local strResult = ""
...
function CRB_PlayerPath:HelperComputeIconPath(pmMission)
...
function CRB_PlayerPath:DrawRewardItem(idx, wndReward, tReward) -- TODO: This is for zone completion, remove it when possible
...
function CRB_PlayerPath:HelperFindTopBigIcon(nArgLevel, nArgCurrentLevel)
...
function CRB_PlayerPath:HelperCalcTime(fSeconds)
...
function CRB_PlayerPath:FactoryProduce(wndParent, strFormName, strCategoryName)
...
-- Global
----------------------------------------------------------------------------------------------------------
local CRB_PlayerPathInstance = CRB_PlayerPath:new()
CRB_PlayerPathInstance:Init()
