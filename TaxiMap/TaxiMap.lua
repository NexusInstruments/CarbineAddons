...
function TaxiMap:OnDocumentReady()
	...
	Apollo.RegisterEventHandler("CharacterCreated", 				"OnInterfaceMenuListHasLoaded", self)
...
function TaxiMap:PopulateTaxiMap()
	...
	elseif self.tZoneInfo then
		...
		if tRapidTransportNodes then
			self:HelperPlaceTaxiNodes(tRapidTransportNodes)
		end
...
function TaxiMap:HelperPlaceTaxiNodes(tNodes)
	if not tNodes then
		return
	end
