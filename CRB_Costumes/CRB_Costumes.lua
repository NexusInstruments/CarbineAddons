-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Costumes
...
function CRB_Costumes:Init()
	Apollo.RegisterAddon(self)
end

function CRB_Costumes:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Costumes.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Costumes:OnDocumentReady()
...
function CRB_Costumes:OnInterfaceMenuLoaded()
...
function CRB_Costumes:OnMannequinInit()
	Apollo.RegisterEventHandler("DyeLearned",	"OnDyeLearned", self)
...
function CRB_Costumes:SharedInit()
...
-- CRB_Costumes
----------------------
function CRB_Costumes:OnCostumeBtnChecked(wndHandler, wndControl)
...
function CRB_Costumes:SwapCostume()
...
function CRB_Costumes:RedrawCostume()
...
function CRB_Costumes:FillSlot(eSlot, bIsVisible, itemShown, bIsEquippedItem)
...
function CRB_Costumes:EmptySlot(eSlot, bIsVisible)
...
function CRB_Costumes:OnSlotClick(wndHandler, wndControl, eMouseButton)
...
function CRB_Costumes:OnVisibleBtnCheck(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_Costumes:OnPreviewBtnChecked(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:OnPageUp(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:OnPageDown(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:OnSearchContent(wndHandler, wndControl, strText)
...
function CRB_Costumes:OnClearSearch()
...
function CRB_Costumes:HelperUpdatePageItems(nPageNumber)	
...
function CRB_Costumes:MapEquipment()
...
function CRB_Costumes:OnCostumeChanged(idCostume)
...
-- Dyes
----------------------
function CRB_Costumes:GetDyeList()
...
function CRB_Costumes:OnDyeChannelChecked(wndHandler, wndControl)
	self:GetDyeList()
	self:ShowDyeContent()
...
function CRB_Costumes:OnDyeChannelDeselect(wndHandler, wndControl)
	local tSlotData = wndHandler:GetParent():GetData()
...
function CRB_Costumes:OnSelectAllDyeChannel(wndHandler, wndControl)
...
function CRB_Costumes:OnDeselectAllDyeChannel(wndHandler, wndControl)
	local nSlotChannel = wndHandler:GetData()
...
function CRB_Costumes:OnDyeColorChecked(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:UpdateSlotDye(eSlot, nChannel, tDyeInfo)
...
function CRB_Costumes:UpdateItemDye(eSlot)
...
function CRB_Costumes:ClearSlotDyeSelection(eSlot)
...
function CRB_Costumes:ResetChannelControls()
...
-- Mannequin
----------------------
function CRB_Costumes:OnPoseCheck(wndHandler, wndControl)
...
-- Window Controls
----------------------
function CRB_Costumes:OnReset(wndHandler, wndControl)
	if self.costumeDisplayed then
...
function CRB_Costumes:OnWindowClosed()
	Event_FireGenericEvent("GenerciEvent_CostumesWindowClosed")
end

function CRB_Costumes:OnClose()
...
function CRB_Costumes:OnConfirmClose()
...
function CRB_Costumes:OnCloseCancel()
	self:ToggleOverlay(keOverlayType.None)
...
-- DragDrop Handlers
----------------------
function CRB_Costumes:OnDragDropQuery(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
...
function CRB_Costumes:OnDragDropEnd(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	if strType ~= "DDBagItem" then
		return
	end
...
----------------------
function CRB_Costumes:OnItemUnlock(itemUnlock)
...
function CRB_Costumes:OnCloseUnlock(wndHandler, wndControl)
	if self.wndUnlock then
...
function CRB_Costumes:OnUnlockResult(itemData, eResult)
	if self.wndMain then
...
function CRB_Costumes:OpenItemUnlock(wndHandler, wndControl)
	if self.wndUnlockItems then
		return
	end
...
function CRB_Costumes:BuildUnlockList()
...
function CRB_Costumes:OnUnlockItemSelect(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:CloseItemUnlock(wndHandler, wndControl)
	if not self.wndUnlockItems then
		return
	end
...
function CRB_Costumes:OnWardrobeContextShow(wndHandler, wndControl, eMouseBtn, bDoubleClick)
...
function CRB_Costumes:OnContextEquip(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:OnRemoveWardrobeItem(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:OnContextMenuClose(wndHandler, wndControl)
...
----------------------
function CRB_Costumes:OnSheatheCheck(wndHandler, wndControl)
	self.bIsSheathed = wndControl:IsChecked()
	self.wndPreview:SetSheathed(self.bIsSheathed)
end

function CRB_Costumes:OnRotateRight()
	self.wndPreview:ToggleLeftSpin(true)
end

function CRB_Costumes:OnRotateRightCancel()
	self.wndPreview:ToggleLeftSpin(false)
end

function CRB_Costumes:OnRotateLeft()
	self.wndPreview:ToggleRightSpin(true)
end

function CRB_Costumes:OnRotateLeftCancel()
	self.wndPreview:ToggleRightSpin(false)
end

function CRB_Costumes:OnGenerateTooltipEquipped(wndHandler, wndControl, eType, itemCurr, idx)
	if wndHandler ~= wndControl then
		return
	end
...
function CRB_Costumes:OnGenerateTooltipPreview(wndHandler, wndControl, eType, itemCurr, idx)
	if wndHandler ~= wndControl then
		return
	end
...
-- Helpers
----------------------
function CRB_Costumes:UpdateCost()
...
function CRB_Costumes:HideContentContainer()
	self.wndMain:FindChild("ContentContainer"):Show(false)
	self.wndMain:FindChild("HelpContentContainer"):Show(true)
end

function CRB_Costumes:HelperToggleEquippedBtn()
...
function CRB_Costumes:ShowDyeContent()
...
function CRB_Costumes:ShowCostumeContent()
	local wndContainer = self.wndMain:FindChild("ContentContainer")
...
function CRB_Costumes:SetHelpString(strText)
...
function CRB_Costumes:OnForgetResult(itemRemoved, eResult)
...
function CRB_Costumes:OnSaveResult(eCostumeType, nCostumeIdx, eResult)
...
function CRB_Costumes:OnHideError()
	self:ToggleOverlay(keOverlayType.None)
end

function CRB_Costumes:CheckForTokens()
...
function CRB_Costumes:OnUseTokenToggle(wndHandler, wndControl)
	self.bUseToken = wndHandler:IsChecked()
	self:UpdateCost()
end

function CRB_Costumes:ToggleOverlay(eType)
...
-- Instance
----------------------
local CRB_CostumesInstance = CRB_Costumes:new()
