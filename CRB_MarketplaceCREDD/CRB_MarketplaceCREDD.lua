-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_MarketplaceCREDD
...
function CRB_MarketplaceCREDD:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
...
function CRB_MarketplaceCREDD:Init()
    Apollo.RegisterAddon(self)
end

function CRB_MarketplaceCREDD:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_MarketplaceCREDD.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_MarketplaceCREDD:OnDocumentReady()
...
function CRB_MarketplaceCREDD:OnInterfaceMenuListHasLoaded()
...
function CRB_MarketplaceCREDD:Initialize()
	...
	local wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_MarketplaceCREDDForm", nil, self)
...
function CRB_MarketplaceCREDD:OnCREDDExchangeWindowClose()
...
function CRB_MarketplaceCREDD:OnToggleCREDDExchangeWindow()
...
function CRB_MarketplaceCREDD:OnClose(wndHandler, wndControl, eMouseButton)
...
function CRB_MarketplaceCREDD:OnOpenListingsBtn(wndHandler, wndControl)
	Event_FireGenericEvent("InterfaceMenu_ToggleMarketplaceListings")
end

function CRB_MarketplaceCREDD:OnOpenInventoryBtn(wndHandler, wndControl)
...
function CRB_MarketplaceCREDD:OnHeaderLogCheck(wndHandler, wndControl) -- Same Radio Group as OnHeaderTabCheck
	self.tWindowMap["ConfirmationBlocker"]:Show(false)
	CREDDExchangeLib.GetCREDDHistory() -- Leads to OnCREDDExchangeInfoResults
end

function CRB_MarketplaceCREDD:OnHeaderTabCheck(wndHandler, wndControl)
	self.tWindowMap["ConfirmationBlocker"]:Show(false)
	CREDDExchangeLib.RequestExchangeInfo() -- Leads to OnCREDDExchangeInfoResults
end

function CRB_MarketplaceCREDD:OnRefreshMarketBtn(wndHandler, wndControl, eMouseButton)
...
function CRB_MarketplaceCREDD:OnCREDDExchangeInfoResults(arMarketStats, arOrders) -- Main Redraw
	local wndMain = self.tWindowMap["Main"]
...
function CRB_MarketplaceCREDD:RefreshEscrow()
	-- Escrow
...
function CRB_MarketplaceCREDD:RefreshBoundCredd()
	local wndMain = self.tWindowMap["Main"]
...
function CRB_MarketplaceCREDD:OnCashInputChanged(wndHandler, wndControl)
	wndHandler:SetText(math.max(0, tonumber(wndHandler:GetAmount() or 0)))
	self:RefreshBoundCredd() -- Will validate the buttons
end

function CRB_MarketplaceCREDD:OnGenerateTooltipFullStats(wndHandler, wndControl, eType, nX, nY) -- Same as from Commodity Exchange
	...
	local nLastCount = 0
	local wndFullStats = wndHandler:LoadTooltipForm("CRB_MarketplaceCREDD.xml", "FullStatsFrame", self)
	for nRowIdx = 1, 3 do
		local strBuy = ""
...
function CRB_MarketplaceCREDD:OnMoreMarketStatsMouseExit(wndHandler, wndControl)
...
function CRB_MarketplaceCREDD:OnConfirmationClose(wndHandler, wndControl)
	self.tWindowMap["ConfirmationBlocker"]:Show(false)
end

function CRB_MarketplaceCREDD:OnCreddTransactionBtn(wndHandler, wndControl, eMouseButton)
	wndHandler:SetFocus()
...
function CRB_MarketplaceCREDD:OnCREDDExchangeOrderSubmitted(wndHandler, wndControl)
...
function CRB_MarketplaceCREDD:OnCREDDExchangeOperationResults(eOperationType, eResult)
...
function CRB_MarketplaceCREDD:OnHidePostResultNotification() -- Both Timer and Mouse Click
...
function CRB_MarketplaceCREDD:OnCREDDOperationHistoryResults(tHistory)
...
function CRB_MarketplaceCREDD:HelperLogConvertToTimeString(nDays)
	local tTimeData =
	{
		["name"]	= "",
		["count"]	= nil,
...
local CRB_MarketplaceCREDDInst = CRB_MarketplaceCREDD:new()
CRB_MarketplaceCREDDInst:Init()
