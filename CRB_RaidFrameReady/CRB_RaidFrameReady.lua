-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_RaidFrameReady
-- Copyright (c) NCsoft. All rights reserved
-----------------------------------------------------------------------------------------------

require "Window"

local CRB_RaidFrameReady = {}

local knReadyCheckTimeout = 14 -- in seconds
local knReadyCheckcooldown = 16 -- in seconds

function CRB_RaidFrameReady:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_RaidFrameReady:Init()
    Apollo.RegisterAddon(self)
end

function CRB_RaidFrameReady:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_RaidFrameReady.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_RaidFrameReady:OnDocumentReady()
...
function CRB_RaidFrameReady:OnCharacterCreated()
...
function CRB_RaidFrameReady:OnSlashReadyCheck()
	GroupLib.ReadyCheck(Apollo.GetString("RaidFrame_AreYouReady")) -- Sanitized in code
end

function CRB_RaidFrameReady:OnGroup_ReadyCheck(nMemberIdx, strMessage) -- This displays the "Are you ready?" pop up
...
function CRB_RaidFrameReady:OnRaid_OneSecTimer()
...
function CRB_RaidFrameReady:OnRaid_ReadyCheckTimeout() -- You took too long to reply
	Apollo.StopTimer("Raid_ReadyCheckTimeout")
	if self.wndReadyCheckPopup and self.wndReadyCheckPopup:IsValid() then
		self.wndReadyCheckPopup:Destroy()
		self.wndReadyCheckPopup = nil
...
function CRB_RaidFrameReady:OnReadyCheckResponse(wndHandler, wndControl) -- ReadyCheckYesBtn, ReadyCheckNoBtn
...
function CRB_RaidFrameReady:DestroyAll()
	if self.wndReadyResults and self.wndReadyResults:IsValid() then
		self.wndReadyResults:Destroy()
		self.wndReadyResults = nil
	end
...
function CRB_RaidFrameReady:HelperBuildNotReadyString()
...
local CRB_RaidFrameReadyInst = CRB_RaidFrameReady:new()
CRB_RaidFrameReadyInst:Init()
