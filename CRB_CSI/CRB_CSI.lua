...
function CRB_CSI:Init()
    Apollo.RegisterAddon(self)
end

function CRB_CSI:OnSave(eType)
...
function CRB_CSI:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Account then
		return
	end
...
function CRB_CSI:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_CSI.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_CSI:OnDocumentReady()
	...
	Apollo.RegisterEventHandler("ProgressClickWindowDisplay", "OnProgressClickWindowDisplay", self)	-- Starting a CRB_CSI
...
function CRB_CSI:OnProgressClickWindowDisplay(bShow)
	local tActiveCSI = CSIsLib.GetActiveCSI()
...
function CRB_CSI:BuildYesNo(tActiveCSI)
...
function CRB_CSI:OnYesNo_WindowClosed(wndHandler) -- wndHandler is "CSI_YesNo"
	self.locYesNoLocation = wndHandler:GetLocation()
	wndHandler:Destroy()
end

function CRB_CSI:OnYesNo_NoPicked(wndHandler, wndControl) -- wndHandler are the "NoButton/CloseButton", and the data is the window "CSI_YesNo"
...
function CRB_CSI:OnYesNo_YesPicked(wndHandler, wndControl) -- wndHandler is "YesButton", and the data is the window "CSI_YesNo"
	if not wndHandler or not wndHandler:GetData() then 
		return 
	end 
	self.locYesNoLocation = wndHandler:GetData():GetLocation()
...
function CRB_CSI:BuildPressAndHold(tActiveCSI, strBodyText)
...
function CRB_CSI:BuildRapidTap(tActiveCSI)
	local wndCurr = Apollo.LoadForm(self.xmlDoc, "CSI_Progress", nil, self)
	wndCurr:Show(true) -- to get the animation
	wndCurr:FindChild("ProgressButton:KeyText"):SetText(GameLib.GetKeyBinding("Interact"))
...
function CRB_CSI:BuildPrecisionTap(tActiveCSI, strBodyText)
...
function CRB_CSI:BuildKeypad(tActiveCSI)
	if not tActiveCSI then
		self.wndKeypad:Show(false)
	    return
	end
...
function CRB_CSI:BuildMemory(tActiveCSI)
...
function CRB_CSI:OnProgressClickWindowCompletionLevel(nPercentage, bIsReversed) -- Updates Progress Bar
...
function CRB_CSI:OnSetProgressClickTimes(nWidth, nLocation1, nLocation2, nSwingCount)
	if not self.wndProgress or not self.wndProgress:IsValid() then
		return
	end
...
function CRB_CSI:OnProgressClickHighlightTime(idx, nPercentageHighlight)
...
function CRB_CSI:HelperComputeProgressFailOrWin()
...
function CRB_CSI:HelperSuccess()
...
function CRB_CSI:HelperFail()
	Sound.Play(Sound.PlayUIAlertPopUpMessageReceived)
	self.nMetronomeMisses = self.nMetronomeMisses + 1
end

-----------------------------------------------------------------------------------------------
-- UI and CRB_CSI Lib Events
-----------------------------------------------------------------------------------------------

function CRB_CSI:OnCancel(wndHandler, wndControl)
...
function CRB_CSI:OnCSIKeyPressed(bKeyDown)
	if bKeyDown then
		self:OnButtonDown()
	else
		self:OnButtonUp()
...
function CRB_CSI:OnCalculateTimeRemaining()
	local tCSI = CSIsLib.GetActiveCSI()
...
function CRB_CSI:OnKeypadSignal(wndDisplayed, wndControl)
	local tCSI = CSIsLib.GetActiveCSI()
...
function CRB_CSI:OnKeypadEnter(wndHandler, wndControl)
	local tCSI = CSIsLib.GetActiveCSI()
...
function CRB_CSI:OnKeypadClear(wndHandler, wndControl)
	local tCSI = CSIsLib.GetActiveCSI()
	if tCSI == nil or tCSI.eType ~= CSIsLib.ClientSideInteractionType_Keypad or not CSIsLib.IsCSIRunning() then
		return
    end
...
function CRB_CSI:OnMemoryStart(wndHandler, wndControl)
	if not CSIsLib.IsCSIRunning() then
...
function CRB_CSI:OnAcceptProgressInput(bShouldAccept)
...
function CRB_CSI:OnHighlightProgressOption(nOption)
...
function CRB_CSI:OnMemoryDisplayTimer() -- When we're done the flash
	if not self.wndMemory or not self.wndMemory:IsValid() then
		return
	end
...
function CRB_CSI:OnMemoryBtn(wndHandler, wndControl)
...
function CRB_CSI:HelperBuildRandomString(nArgLength)
    local strResult = ""
...
function CRB_CSI:HelperVerifyCSIMemory()
...
---------------------------------------------------------------------------------------------------
function CRB_CSI:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
	if eAnchor == nil then
		return false
	end
