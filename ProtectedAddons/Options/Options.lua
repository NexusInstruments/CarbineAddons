...
function OptionsAddon:OnDocumentReady()
	...
	Apollo.RegisterEventHandler("OpenOptions", "OnRefreshOptionsDialog", self)
	...
	self.wndIntegrations = Apollo.LoadForm(self.xmlDoc, "IntegrationsDialog", nil, self)
	Apollo.LoadForm(self.xmlDoc, "IntegrationsOptionsControls", self.wndIntegrations:FindChild("GroupContainer:IntegrationsDialogControls"), self)
	...
	self.wndControls = Apollo.LoadForm(self.xmlDoc, "ControlsDialog", nil, self)
	Apollo.LoadForm(self.xmlDoc, "ControlsOptions", self.wndControls:FindChild("GroupContainer:ControlDialogControls"), self)
	
	self.wndSearchEditBox = self.wndAddons:FindChild("SearchEditBox")

	self.OptionsDlg:FindChild("OptionsList"):ArrangeChildrenVert(Window.CodeEnumArrangeOrigin.LeftOrTop)
	
	self.bAddonsTimerCreated = false
	self.OptionsDlg:FindChild("OptionsList"):SetRadioSel("OptionsGroup", keOptionsCategory.Targeting)
	self:OnOptionsCheck()
	...
	self.mapCB2CVs =  -- these are auto-mapped options than don't need custom handlers
	{
		...
		-- Controls
		{wnd = self.wndControls:FindChild("UseButtonDownBtn"),					consoleVar = "spell.useButtonDownForAbilities",					requiresRestart = false, wndSubControl = self.wndControls:FindChild("HoldToContinueCastingBtn")},
		{wnd = self.wndControls:FindChild("HoldToContinueCastingBtn"),			consoleVar = "spell.holdToContinueCasting",						requiresRestart = false, wndRequiredControl = self.wndControls:FindChild("UseButtonDownBtn")},
		...
		-- Integrations
		{wnd = self.wndIntegrations:FindChild("AlienFxBtn"),					consoleVar = "alienwarefx.enable",								requiresRestart = false},
	
		-- Audio
		{wnd = self.wndSounds:FindChild("PlayHoloCrypt"),							consoleVar = "sound.holoCryptVOEnable",							requiresRestart = false},
	}
	...
	self.mapDDParents =
	{
		...
		{wnd = self.wndVideo:FindChild("DropToggleTexFilter"),			consoleVar = "lod.textureFilter", 					radio = "TextureFiltering",		requiresRestart = false},
	...
	wndTelelgraphColors:ArrangeChildrenVert(Window.CodeEnumArrangeOrigin.LeftOrTop)
	...
	if Is64BitClient() then
		for strPreset, tPreset in pairs(ktVideoSettingLevels64) do
		...
		for strPreset, tPreset in pairs(ktVideoSettingLevels32) do
			self.wndVideo:FindChild("DropTogglePresetSettings"):FindChild(strPreset):SetData(self.wndVideo:FindChild("DropTogglePresetSettings"))
		end
	end
	...
	if not Apollo.GetConsoleVariable("ui.enableAddonProfiling") then
		Apollo.StopTimer("AddonsUpdateTimer")
	end
	...
	if not Is64BitClient() then
		...
		self.wndVideo:FindChild("TextureResolutionUltra"):Enable(false)
	end

	local wndUseButtonDownBtn = self.wndControls:FindChild("UseButtonDownBtn")
	if Apollo.GetConsoleVariable("spell.holdToContinueCasting") and not Apollo.GetConsoleVariable("spell.useButtonDownForAbilities") then
		wndUseButtonDownBtn:SetCheck(true)
		self:OnMappedOptionsCheckbox(wndUseButtonDownBtn, wndUseButtonDownBtn)
	end
end
...
function OptionsAddon:OnOptionsCheck()
	...
	local nOptions = self.OptionsDlg:FindChild("OptionsList"):GetRadioSel("OptionsGroup")
	
	self.wndAddons:Show(nOptions == keOptionsCategory.Addons and IsInGame())
	self.wndTargeting:Show(nOptions == keOptionsCategory.Targeting)
	self.wndControls:Show(nOptions == keOptionsCategory.Controls)
	self.wndSounds:Show(nOptions == keOptionsCategory.Sound)
	self.wndVideo:Show(nOptions == keOptionsCategory.Video)
	self.wndIntegrations:Show(nOptions == keOptionsCategory.Integrations)

	if nOptions ~= keOptionsCategory.Video and self.nOptionsTimer ~= 0 then -- unsaved graphic change
		self:OnChangeCancelBtn()
	end

	if nOptions == keOptionsCategory.Addons then
		self.wndAddons:SetAnchorOffsets(self.wndAddons:GetAnchorOffsets())
		self:OnAddonsCheck(true)
	elseif nOptions == keOptionsCategory.Targeting then
	...
	elseif nOptions == keOptionsCategory.Controls then
		self:InitControls()
		self.wndControls:SetAnchorOffsets(self.wndControls:GetAnchorOffsets())
	elseif nOptions == keOptionsCategory.Sound then
		self.wndSounds:SetAnchorOffsets(self.wndSounds:GetAnchorOffsets())
		self.wndSounds:FindChild("CombatMusicFlair"):SetCheck(Apollo.GetConsoleVariable("sound.intenseBattleMusic"))
		self.wndSounds:FindChild("CinematicSubtitles"):SetCheck(Apollo.GetConsoleVariable("draw.subtitles"))
		self.wndSounds:FindChild("PlayInBackground"):SetCheck(Apollo.GetConsoleVariable("sound.playInBackground"))
	elseif nOptions == keOptionsCategory.Video then
		self.wndVideo:SetAnchorOffsets(self.wndVideo:GetAnchorOffsets())
		self:EnableVideoControls()
	elseif nOptions == keOptionsCategory.Integrations then
		self:EnableIntegrationControls()
	end
end

function OptionsAddon:InitControls()
	self.wndControls:FindChild("DashDoubleTapBtn"):SetCheck(Apollo.GetConsoleVariable("player.showDoubleTapToDash"))
	self.wndControls:FindChild("UseAbilityQueueBtn"):SetCheck(Apollo.GetConsoleVariable("player.abilityQueueMax") > 0)

	self.wndControls:FindChild("AbilityQueueLengthBlocker"):Show(Apollo.GetConsoleVariable("player.abilityQueueMax") == 0)
	self.wndControls:FindChild("AbilityQueueLengthSliderBar"):SetValue(Apollo.GetConsoleVariable("player.abilityQueueMax") or 0)
	self.wndControls:FindChild("AbilityQueueLengthEditBox"):SetText(string.format("%.0f", Apollo.GetConsoleVariable("player.abilityQueueMax")))

	self.wndControls:FindChild("MouseTurnSpeedSliderBar"):SetValue(Apollo.GetConsoleVariable("player.turningSpeedMouse") or 0)
	self.wndControls:FindChild("MouseTurnSpeedEditBox"):SetText(string.format("%.2f", Apollo.GetConsoleVariable("player.turningSpeedMouse")))

	self.wndControls:FindChild("KeyboardTurnSpeedSliderBar"):SetValue(Apollo.GetConsoleVariable("player.turningSpeedKeyboard") or 0)
	self.wndControls:FindChild("KeyboardTurnSpeedEditBox"):SetText(string.format("%.2f", Apollo.GetConsoleVariable("player.turningSpeedKeyboard")))
end
...
function OptionsAddon:FillConfigureList()
	...
	wndList:ArrangeChildrenVert(Window.CodeEnumArrangeOrigin.LeftOrTop, SortConfigButtons)
end
...
function OptionsAddon:OnAddonsCheck(bReload)
	...
	local wndStatUpdates = self.wndAddons:FindChild("EnableRealTimeUpdatesBtn")
	if wndStatUpdates then
		wndStatUpdates:SetCheck(Apollo.GetConsoleVariable("ui.enableAddonProfiling"))
		self:OnEnableAddonStatUpdates( self.wndAddons, wndStatUpdates )
	end
...
function OptionsAddon:OnAddonsUpdateTimer()
	...
	self.OptionsDlg:FindChild("CampBtn"):Enable(IsInGame())
...
function OptionsAddon:OnEnableAddonStatUpdates( wndHandler, wndControl, eMouseButton )
	local bEnable = wndControl:IsChecked()
	Apollo.SetConsoleVariable("ui.enableAddonProfiling", bEnable)
	
	if bEnable then
		Apollo.StartTimer("AddonsUpdateTimer")
	else
		Apollo.StopTimer("AddonsUpdateTimer")
	end
end
...
function OptionsAddon:InitOptionsControls()
	...
	for idx, mapping in pairs(self.mapSB2CVs or {}) do
		if mapping.wnd ~= nil then
			...
			if mapping.bSoundSlider and wndBlocker then
				...
				self.wndSounds:FindChild("PlayHoloCrypt"):Enable(not bMute)
			end
		end
	end
	...
	for idx, parent in pairs(self.mapDDParents or {}) do
		if parent.wnd ~= nil and parent.consoleVar ~= nil and parent.radio ~= nil then
			...
			elseif parent.consoleVar == "lod.textureLodMin" and Apollo.GetConsoleVariable("gfx.jpgdstfmt") == 1 then
	...
	local bHasIntegration = AlienFxLib.HasDevice()
	self.OptionsDlg:FindChild("IntegrationsBtn"):Show(bHasIntegration)
	if not bHasIntegration then
		self.OptionsDlg:FindChild("IntegrationsBtn"):SetCheck(false)
		self.wndIntegrations:Show(false)
	end
	...
	self.OptionsDlg:FindChild("CampBtn"):Enable(bIsInGame)
...
function OptionsAddon:OnMappedOptionsCheckbox(wndHandler, wndControl)
	...
	Apollo.SetConsoleVariable(tMapping.consoleVar, bIsChecked)
	self:EnableVideoControls()
	self:OnMappedOptionsCheckboxHider()
	...
	if tMapping.wndRequiredControl and bIsChecked then
		tMapping.wndRequiredControl:SetCheck(true)
		self:OnMappedOptionsCheckbox(tMapping.wndRequiredControl, tMapping.wndRequiredControl)
	end

	if tMapping.wndSubControl and not bIsChecked then
		tMapping.wndSubControl:SetCheck(false)
		self:OnMappedOptionsCheckbox(tMapping.wndSubControl, tMapping.wndSubControl)
	end
end
...
function OptionsAddon:OnTextureResolutionRadio(wndHandler, wndControl)
	...
	Apollo.SetConsoleVariable("gfx.jpgdstfmt", 0)
	local wndParent = wndControl:GetParent()
	wndParent:GetParent():SetText(wndControl:GetText())
	wndParent:Close()
end

function OptionsAddon:OnTextureUltraRadio(wndHandler, wndControl)
	Apollo.SetConsoleVariable("lod.textureLodMin", 0)
	Apollo.SetConsoleVariable("gfx.jpgdstfmt", 1)
	local wndParent = wndControl:GetParent()
	wndParent:GetParent():SetText(wndControl:GetText())
	wndParent:Close()
end
...
function OptionsAddon:RefreshPresetVideoSelection()
	...
	if not self.bCustomVideoSettings then
		local tSettings = ktVideoSettingLevels32
		if Is64BitClient() then
			tSettings =	ktVideoSettingLevels64
		end

		for strPreset, tPreset in pairs(tSettings) do
			local bAllMatching = true
			for strConsoleVar, default in pairs(tPreset) do
				local oValue = Apollo.GetConsoleVariable(strConsoleVar)
				if type(oValue)  == "number" then--Numbers
					local nDeviation = math.abs(default - oValue)
					if nDeviation > knEpsilon then
						bAllMatching = false
						break
					end
				elseif oValue ~= default then--Strings
					bAllMatching = false
					break
				end
			end
...
function OptionsAddon:OnAbilityQueueLengthChanged(wndHandler, wndControl, fValue, fOldValue)
	...
	self.wndControls:FindChild("AbilityQueueLengthEditBox"):SetText(string.format("%.0f", fValue))
end
...
function OptionsAddon:OnMouseTurnSpeedChanged(wndHandler, wndControl, fValue, fOldValue)
	...
	self.wndControls:FindChild("MouseTurnSpeedEditBox"):SetText(strRound)
end
...
function OptionsAddon:OnKeyboardTurnSpeedChanged(wndHandler, wndControl, fValue, fOldValue)
	...
	self.wndControls:FindChild("KeyboardTurnSpeedEditBox"):SetText(strRound)
end
...
function OptionsAddon:OnMuteCheck( wndHandler, wndControl)
	...
	self.wndSounds:FindChild("PlayHoloCrypt"):Enable(false)
...
function OptionsAddon:OnMuteUncheck( wndHandler, wndControl)
	...
	self.wndSounds:FindChild("PlayHoloCrypt"):Enable(true)
...
function OptionsAddon:OnVideoPresetSetting(wndHandler, wndControl, eMouseButton)
	if wndControl:GetName() ~= "Custom" then
		if Is64BitClient() then
			for strConsoleVar, default in pairs(ktVideoSettingLevels64[wndControl:GetName()]) do
				Apollo.SetConsoleVariable(strConsoleVar, default)
			end
		else
			for strConsoleVar, default in pairs(ktVideoSettingLevels32[wndControl:GetName()]) do
				Apollo.SetConsoleVariable(strConsoleVar, default)
			end
		end
	end
...
function OptionsAddon:OnRequestCampSignal(wndHandler, wndControl, eMouseButton)
	RequestCamp()
end

function OptionsAddon:OnExitGameSignal(wndHandler, wndControl, eMouseButton)
	ExitGame()
end

function OptionsAddon:OnCloseSignal(wndHandler, wndControl, eMouseButton)
	CloseOptions()
end

function OptionsAddon:EnableIntegrationControls()
