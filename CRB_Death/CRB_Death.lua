-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Death
...
-- EventHandlers
---------------------------------------------------------------------------------------------------
function CRB_Death:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Death.xml")
	self.xmlDoc:RegisterCallback("OnDocLoaded", self)
end


function CRB_Death:OnDocLoaded()
...
function CRB_Death:OnSave(eType)
...
function CRB_Death:OnRestore(eType, tSavedData)
...
-- Interface
---------------------------------------------------------------------------------------------------
function CRB_Death:OnCharacterCreated()
...
function CRB_Death:OnShowResurrectDialog(bPlayerIsDead, bEnableRezHere, bEnableRezHoloCrypt, bEnableRezExitInstance, bEnableCasterRez, bHasCasterRezRequest, nRezCost, fTimeBeforeRezable, fTimeBeforeWakeHere, fTimeBeforeForceRez)
...
function CRB_Death:OnUpdateResurrectDialog(bEnableRezHere, bEnableRezHoloCrypt, bEnableRezExitInstance, bEnableCasterRez, bHasCasterRezRequest, nRezCost, fTimeBeforeRezable, fTimeBeforeWakeHere, fTimeBeforeForceRez)
...
------------------------------------//------------------------------
function CRB_Death:OnTenthSecTimer() -- there's no reason we can't run the whole thing off of this
...
function CRB_Death:CasterResurrectedPlayer(strCasterName)
	self.bHasCasterRezRequest = true;
end

function CRB_Death:OnResurrectHere()
...
------------------------------------//------------------------------
function CRB_Death:OnResurrectCaster()
	if GameLib.GetPlayerUnit() ~= nil then
...
------------------------------------//------------------------------
function CRB_Death:OnResurrectEldan()
	if GameLib.GetPlayerUnit() ~= nil then
...
------------------------------------//------------------------------
function CRB_Death:OnExitInstance()
	self.wndExitConfirm:Invoke()
	self.wndResurrect:Close()
end

function CRB_Death:OnConfirmExit()
	if GameLib.GetPlayerUnit() ~= nil then
...
function CRB_Death:OnCancelExit()
	self.wndExitConfirm:Close()	
	self.wndResurrect:Invoke()
end

------------------------------------//------------------------------
function CRB_Death:OnForcedResurrection()
	self.wndExitConfirm:Show(false)	
	self.wndResurrect:Close()
	self.bDead = false
...
------------------------------------//------------------------------
function CRB_Death:OnScriptResurrection()
	self.wndExitConfirm:Show(false)	
	self.wndResurrect:Close()
	self.bDead = false
...
function CRB_Death:HideIncapacitationBar()
	self.wndExitConfirm:Show(false)	
	self.wndResurrect:Show(false)
end

function CRB_Death:HelperCalcTimeSecondsMS(fTimeMS)
	local fTime = math.floor(fTimeMS / 1000)
	local fMillis = fTimeMS % 1000
	return string.format("%d.%d", fTime, math.floor(fMillis / 100))
end

function CRB_Death:HelperCalcTimeMS(fTimeMS)
...
function CRB_Death:HelperCalcTime(fSeconds)
	local strOutputSeconds = "00"
	if math.floor(fSeconds % 60) >= 10 then
		strOutputSeconds = tostring(math.floor(fSeconds % 60))
	else
...
-- CRB_Death instance
---------------------------------------------------------------------------------------------------
local CRB_DeathInst = CRB_Death:new()
CRB_DeathInst:Init()
