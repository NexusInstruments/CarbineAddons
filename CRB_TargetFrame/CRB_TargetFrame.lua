...
-- Variables shared between UnitFrames/CRB_TargetFrame/ClusterFrame
...
function UnitFrames:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_TargetFrame.xml")
...
function UnitFrames:OnUnitFrameOptionsUpdated()
	...
	self.luaUnitFrame = CRB_TargetFrame:new()
	self.luaTargetFrame = CRB_TargetFrame:new()
	self.luaFocusFrame = CRB_TargetFrame:new()
...
function CRB_TargetFrame:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
	return o
...
function CRB_TargetFrame:Init(xmlDoc, tParams)		
	Apollo.RegisterEventHandler("Tutorial_RequestUIAnchor", 		"OnTutorial_RequestUIAnchor", self)
...
function CRB_TargetFrame:OnFrame()
	self:OnUpdate()
end

function CRB_TargetFrame:OnUpdate()
...
function CRB_TargetFrame:OnSlowUpdate()
...
function CRB_TargetFrame:GetPosition()
	return self.wndMainClusterFrame:GetLocation()
end

function CRB_TargetFrame:SetPosition(locNewLocation)
...
function CRB_TargetFrame:SetTarget(unitTarget)
...
function CRB_TargetFrame:UpdatePrimaryFrame(unitTarget, bTargetChanged) --called from the onFrame; eliteness is frame, diff is rank
	self.wndSimpleFrame:Show(false)
	if unitTarget == nil then
		return
	end
...
function CRB_TargetFrame:UpdateToTFrame(unitToT) -- called on frame
	if unitToT == nil then
		return
	end
...
function CRB_TargetFrame:UpdateClusterFrame(tCluster) -- called on frame
...
function CRB_TargetFrame:HideClusterFrames()
	for idx = 1, #self.arClusterFrames do
		self.arClusterFrames[idx]:SetTarget(nil)
	end
end

function CRB_TargetFrame:SetTargetForFrame(wndFrame, bTargetChanged)
	local unitTarget = self.unitTarget
	if unitTarget:IsInVehicle() then
...
function CRB_TargetFrame:UpdateToT()
...
function CRB_TargetFrame:UpdateInterruptArmor()
...
function CRB_TargetFrame:UpdateCastingBar(wndFrame, unitCaster)
	-- Casting Bar Update
...
-------------------------------------------------------------------------------
function CRB_TargetFrame:ArrangeClusterMembers()
	self.wndClusters:ArrangeChildrenHorz(self.tParams.bFlipClusters and 2 or 0)
end

function CRB_TargetFrame:HelperBuildTooltip(strBody, strTitle, crTitleColor)
...
function CRB_TargetFrame:HelperCalculateConValue(unitTarget)
...
function CRB_TargetFrame:HelperResetTooltips()
	self.wndTargetModel:SetTooltip("")
	self.wndTargetLevel:SetTooltip("")
	self.wndGroupSizeMark:SetTooltip("")
end

function CRB_TargetFrame:SetTargetHealthAndShields(wndTargetFrame, unitTarget)
...
function CRB_TargetFrame:HelperFormatBigNumber(nArg)
	if nArg < 1000 then
...
function CRB_TargetFrame:SetBarValue(wndBar, fMin, fValue, fMax)
	wndBar:SetMax(fMax)
	wndBar:SetFloor(fMin)
	wndBar:SetProgress(fValue)
end

function CRB_TargetFrame:OnGenerateBuffTooltip(wndHandler, wndControl, tType, splBuff)
...
function CRB_TargetFrame:OnMouseButtonDown(wndHandler, wndControl, eMouseButton, x, y)
...
function CRB_TargetFrame:OnQueryDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	if wndHandler ~= wndControl then
...
function CRB_TargetFrame:OnDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nValue)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_TargetFrame:OnKeyBindingUpdated(strKeybind)
...
function CRB_TargetFrame:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
function ClusterFrame:OnFrame()
	...
	if nHealth ~= nil then
		...
		if nHealth ~= self.nHealth
			...
			if nHealth ~= self.nHealth or nMaxHealth ~= self.nMaxHealth then
				local strHealthCurr = nHealth > 0 and CRB_TargetFrame.HelperFormatBigNumber(self, nHealth) or "0"
				local strHealthMax = nMaxHealth > 0 and CRB_TargetFrame.HelperFormatBigNumber(self, nMaxHealth) or "0"
			...
			if unitTarget:GetShieldCapacityMax() ~= 0 then
				if nShield ~= self.nShield or nMaxShield ~= self.nMaxShield then
					local strShieldCurr = nShield > 0 and CRB_TargetFrame.HelperFormatBigNumber(self, nShield) or "0"
					local strShieldMax = nMaxShield > 0 and CRB_TargetFrame.HelperFormatBigNumber(self, nMaxShield) or "0"
	...
	if nLevel == nil then
		...
		local nCon = CRB_TargetFrame.HelperCalculateConValue(self, unitTarget)
		...
		if unitTarget:IsScaled() then
			...
			if nLevel ~= self.nLevel then
				if unitTarget ~= kunitPlayer then
					strRewardFormatted = String_GetWeaselString(Apollo.GetString("TargetFrame_CreatureScales"), nLevel)
					local strLevelTooltip = CRB_TargetFrame.HelperBuildTooltip(self, strRewardFormatted, Apollo.GetString("Adaptive"), kstrScalingHex)
					self.wndTargetLevel:SetTooltip(strLevelTooltip)
				end
			end
		else
			...
			if nCon ~= self.nCon then
				...
				if unitTarget ~= kunitPlayer then
					...
					local strLevelTooltip = CRB_TargetFrame.HelperBuildTooltip(self, strRewardFormatted, karConInfo[nCon][3], karConInfo[nCon][5])
					self.wndTargetLevel:SetTooltip(strLevelTooltip)
				end
			end
		end
