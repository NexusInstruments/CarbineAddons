-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_InterfaceMenuList
...
local CRB_InterfaceMenuList = {}
local knVersion = 2

function CRB_InterfaceMenuList:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_InterfaceMenuList:Init()
    Apollo.RegisterAddon(self)
end

function CRB_InterfaceMenuList:OnSave(eType)
...
function CRB_InterfaceMenuList:OnRestore(eType, tSavedData)
...
function CRB_InterfaceMenuList:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_InterfaceMenuList.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self) 
end

function CRB_InterfaceMenuList:OnDocumentReady()
    ...
    self.wndMain = Apollo.LoadForm(self.xmlDoc , "CRB_InterfaceMenuListForm", "FixedHudStratumHigh", self)
...
function CRB_InterfaceMenuList:OnListShow()
	self.wndList:ToFront()
end

function CRB_InterfaceMenuList:OnCharacterCreated()	
	Apollo.CreateTimer("TimeUpdateTimer", 1.0, true)
end

function CRB_InterfaceMenuList:OnUpdateTimer()
...
function CRB_InterfaceMenuList:OnNewAddonListed(strKey, tParams)
...
function CRB_InterfaceMenuList:IsPinned(strText)
...
function CRB_InterfaceMenuList:FullListRedraw()
...
function CRB_InterfaceMenuList:ButtonListRedraw()
	Apollo.StopTimer("QueueRedrawTimer")
	Apollo.StartTimer("QueueRedrawTimer")
end

function CRB_InterfaceMenuList:OnQueuedRedraw()
	local strUnbound = Apollo.GetString("Keybinding_Unbound")
...
function CRB_InterfaceMenuList:OnSearchEditBoxChanged(wndHandler, wndControl)
	self.wndList:FindChild("SearchClearBtn"):Show(string.len(wndHandler:GetText() or "") > 0)
	self:FullListRedraw()
end

function CRB_InterfaceMenuList:OnSearchClearBtn(wndHandler, wndControl)
...
function CRB_InterfaceMenuList:OnSearchCommitBtn(wndHandler, wndControl)
	self.wndList:FindChild("SearchFlash"):SetSprite("CRB_WindowAnimationSprites:sprWinAnim_BirthSmallTemp")
	self.wndList:FindChild("SearchFlash"):SetFocus()
	self:FullListRedraw()
end
...
function CRB_InterfaceMenuList:OnDrawAlert(strWindowName, tParams)
...
function CRB_InterfaceMenuList:OnLiveEventClosed()
...
function CRB_InterfaceMenuList:OnMenuListItemClick(wndHandler, wndControl)
...
function CRB_InterfaceMenuList:OnPinBtnChecked(wndHandler, wndControl)
	if wndHandler ~= wndControl then return end
...
function CRB_InterfaceMenuList:OnListBtnClick(wndHandler, wndControl) -- These are the five always on icons on the top
	if wndHandler ~= wndControl then return end
...
function CRB_InterfaceMenuList:OnListBtnMouseEnter(wndHandler, wndControl)
...
function CRB_InterfaceMenuList:OnListBtnMouseExit(wndHandler, wndControl) -- Also self.wndMain MouseExit and ButtonList MouseExit
	wndHandler:SetBGColor("9dffffff")
end

function CRB_InterfaceMenuList:OnOpenFullListCheck(wndHandler, wndControl)
	self.wndList:FindChild("SearchEditBox"):SetFocus()
	self:FullListRedraw()
end

function CRB_InterfaceMenuList:OnEventMoreInfoBtn(wndHandler, wndControl)
	Event_FireGenericEvent("LiveEvent_ToggleWindow")
end

function CRB_InterfaceMenuList:LoadByName(strForm, wndParent, strCustomName)
...
function CRB_InterfaceMenuList:OnTutorial_RequestUIAnchor(eAnchor, idTutorial, strPopupText)
...
local CRB_InterfaceMenuListInst = CRB_InterfaceMenuList:new()
