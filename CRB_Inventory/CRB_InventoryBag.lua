...
function InventoryBag:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_InventoryBag.xml")
...
function InventoryBag:OnInterfaceMenuListHasLoaded()
	Event_FireGenericEvent("InterfaceMenuList_NewAddOn", Apollo.GetString("InterfaceMenu_Inventory"), {"InterfaceMenu_ToggleInventory", "CRB_Inventory", "Icon_Windows32_UI_CRB_InterfaceMenu_Inventory"})
end
...
function InventoryBag:OnToggleVisibility()
	...
	if self.wndMain:IsShown() then
		...
		self:OnQuestObjectiveUpdated() -- Populate Virtual CRB_Inventory Btn from reloadui/load
