-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_GuildPerks
...
function CRB_GuildPerks:Init()
    Apollo.RegisterAddon(self)
end

function CRB_GuildPerks:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_GuildPerks.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_GuildPerks:OnDocumentReady()
...
function CRB_GuildPerks:Initialize(wndParent)
	...
	-- load our forms
    self.tWndRefs.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_GuildPerksForm", wndParent, self)
...
function CRB_GuildPerks:OnTogglePerks(wndParent)
	local guildOwner = wndParent:GetParent():GetData()
	if not guildOwner then 
		return 
	end
...
function CRB_GuildPerks:OnClose()
...
function CRB_GuildPerks:BuildTabDisplay(arBankTabsTemp)
...
function CRB_GuildPerks:UpdateTabDisplay()
...
function CRB_GuildPerks:BuildTierDisplays(arTiersTemp)
...
function CRB_GuildPerks:FormatTierDisplays()
	local nLastUnlock = 1
...
function CRB_GuildPerks:BuildPerkEntries(arPerksTemp)
...
function CRB_GuildPerks:FormatPerkWindow(idPerk)
...
function CRB_GuildPerks:UpdateInfluenceDisplay(nCurrent, nBonusRemaining)
...
function CRB_GuildPerks:DrawConfirmWindow(wndBtn, nId, strName, bActivate)
...
function CRB_GuildPerks:OnUpdateEntry(guildOwner, idPerk)
...
function CRB_GuildPerks:UpdateEntryValues(tActivatablePerk, tUpdatedPerk) -- update with new values
...
function CRB_GuildPerks:OnAchievementUpdated(achUpdated)
...
function CRB_GuildPerks:OnInfluenceUpdated(guildOwner, nInfluence, monCash)
	if not self.tWndRefs.wndMain or not self.tWndRefs.wndMain:IsValid() or self.guildOwner == nil or self.guildOwner ~= guildOwner or not self.tWndRefs.wndMain:IsShown() then
		return
	end
...
function CRB_GuildPerks:AdjustCosts(nInfluence)
	self:FormatTierDisplays()
...
function CRB_GuildPerks:DrawMoreInfoWindow(idEntry, nLeft, nTop)
...
-----------------------------------------------------------------------------------------------
-- CRB_GuildPerksForm Functions
-----------------------------------------------------------------------------------------------

function CRB_GuildPerks:OnEntryMouseEnter(wndHandler, wndControl)
...
function CRB_GuildPerks:OnEntryMouseExit(wndHandler, wndControl)
...
function CRB_GuildPerks:OnUnlockBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:OnUnlockTierBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:OnUnlockTabBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:OnUnlockCapBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:OnActivateBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:OnCancel(wndHandler, wndControl)
	self.tWndRefs.wndMain:Show(false)
end

function CRB_GuildPerks:ApproveConfirmBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:ApproveActivateBtn(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return false
	end
...
function CRB_GuildPerks:CancelConfirmBtn()
	self.wndConfirm:Show(false)
end

function CRB_GuildPerks:OnSocialPanelBtn(wndHandler, wndControl)
...
-- Helpers
-----------------------------------------------------------------------------------------------
function CRB_GuildPerks:HelperCalculateTimeRemaining(fDays)
	local tTimeData =
	{
		["name"]	= "",
		["count"]	= nil,
...
function CRB_GuildPerks:OnMouseWheel( wndHandler, wndControl, nLastRelativeMouseX, nLastRelativeMouseY, fScrollAmount, bConsumeMouseWheel )
...
-----------------------------------------------------------------------------------------------
-- CRB_GuildPerks Instance
-----------------------------------------------------------------------------------------------
local CRB_GuildPerksInst = CRB_GuildPerks:new()
CRB_GuildPerksInst:Init()
