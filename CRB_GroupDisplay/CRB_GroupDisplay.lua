...
---------------------------------------------------------------------------------------------------
-- CRB_GroupDisplay initialization
---------------------------------------------------------------------------------------------------
function CRB_GroupDisplay:new(o)
	o = o or {}
	setmetatable(o, self)
	self.__index = self
...
function CRB_GroupDisplay:Init()
	Apollo.RegisterAddon(self)
end

function CRB_GroupDisplay:OnSave(eType)
...
function CRB_GroupDisplay:OnRestore(eType, tSavedData)
...
---------------------------------------------------------------------------------------------------
-- CRB_GroupDisplay EventHandlers
---------------------------------------------------------------------------------------------------
function CRB_GroupDisplay:OnLoad()
	self.xmlOptionsDoc = XmlDoc.CreateFromFile("CRB_GroupDisplayOptions.xml")
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_GroupDisplay.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_GroupDisplay:OnDocumentReady()
	...
	---------------------------------------------------------------------------------------------------
	-- CRB_GroupDisplay Member Variables
	---------------------------------------------------------------------------------------------------
...
function CRB_GroupDisplay:OnWindowManagementReady()
...
function CRB_GroupDisplay:GroupDisplayOptions_TEMP()
	-- TEMP HACK: Try again in case this loads first
	Event_FireGenericEvent("GenericEvent_InitializeGroupLeaderOptions", self.wndGroupHud:FindChild("GroupControlsBtn"))
end
...
function CRB_GroupDisplay:LoadPortrait(idx)
...
function CRB_GroupDisplay:OnGroupInvited(strInviterName) -- builds the invite when I recieve it
...
function CRB_GroupDisplay:OnGroupJoinRequest(strInviterName) -- builds the invite when I recieve it
...
function CRB_GroupDisplay:OnGroupReferral(nMemberIndex, strTarget) -- builds the invite when I receive it
	-- undone need token passed as context
	-- a join message means that someone has requested to join our existing party
...
function CRB_GroupDisplay:OnInviteTimer()
...
function CRB_GroupDisplay:OnGroupInviteDialogAccept()
...
function CRB_GroupDisplay:OnGroupInviteDialogDecline()
...
function CRB_GroupDisplay:OnReportGroupInviteSpamBtn()
...
function CRB_GroupDisplay:OnRaidInfoResponse(arRaidInfo)
...
function CRB_GroupDisplay:DestroyGroup()
...
function CRB_GroupDisplay:PostChangeToChannel(nPrevValue, nNextValue, tDescriptionTable, strChangeString, strUnknownChangeString)
...
function CRB_GroupDisplay:OnGroupUpdated()
	if GroupLib.InRaid() then
		return
	end
...
function CRB_GroupDisplay:OnGroupLootRulesChanged()
	if GroupLib.InRaid() then
		return
	end
...
function CRB_GroupDisplay:OnGroupControlsCheck(wndHandler, wndControl)
	Event_FireGenericEvent("GenericEvent_UpdateGroupLeaderOptions")
end

function CRB_GroupDisplay:OnGroupPortraitClick(wndHandler, wndControl, eMouseButton)
...
function CRB_GroupDisplay:OnGroupBagBtn()
	Event_FireGenericEvent("GenericEvent_ToggleGroupBag")
end

function CRB_GroupDisplay:OnMasterLootUpdate()
...
function CRB_GroupDisplay:OnKick()
...
function CRB_GroupDisplay:OnLocate()
	if self.nGroupMemberClicked == nil then
		return
	end
...
function CRB_GroupDisplay:OnPromote()
	if self.nGroupMemberClicked == nil then
		return
	end
...
function CRB_GroupDisplay:ShowConfirmLeaveDisband(nType)
...
function CRB_GroupDisplay:OnLeaveGroup()
	self:ShowConfirmLeaveDisband(1)
end

function CRB_GroupDisplay:OnDisbandGroup()
	self:ShowConfirmLeaveDisband(0)
end

function CRB_GroupDisplay:OnConfirmLeave()
	GroupLib.LeaveGroup()
	self:DestroyGroup()
end

function CRB_GroupDisplay:OnConfirmDisband()
...
function CRB_GroupDisplay:OnCancelLeave()
...
-- Format Members
---------------------------------------------------------------------------------------------------
function CRB_GroupDisplay:DrawMemberPortrait(tPortrait, tMemberInfo)
...
function CRB_GroupDisplay:HelperUpdateHealth(tPortrait, tMemberInfo)
...
function CRB_GroupDisplay:SetBarValue(wndBar, fMin, fValue, fMax)
	wndBar:SetMax(fMax)
...
function CRB_GroupDisplay:OnUpdateTimer(strVar, nValue)
...
function CRB_GroupDisplay:OnGroupAdd(strMemberName) -- Someone else joined my group
	local strMsg = String_GetWeaselString(Apollo.GetString("GroupJoin"), strMemberName)
	self:AddToQueue(ktMessageIcon.Accept, strMsg)
	self:OnGroupUpdated()
end

function CRB_GroupDisplay:OnGroupJoin() -- I joined a group
	self:OnGroupUpdated()
...
function CRB_GroupDisplay:OnGroupRemove(strMemberName, eReason) -- someone else left the group
...
function CRB_GroupDisplay:OnGroupMemberPromoted(strMemberName, bSelf) -- I've been promoted
	if bSelf then
...
function CRB_GroupDisplay:OnGroupOperationResult(strMemberName, eResult)
...
function CRB_GroupDisplay:OnGroupAcceptInvite() -- I've accepted an invitation
	self.wndGroupInviteDialog:Show(false)
end

function CRB_GroupDisplay:OnGroupDeclineInvite() -- I've declined an invitation
	self.wndGroupInviteDialog:Show(false)
end

function CRB_GroupDisplay:OnLootRollUpdate()
--[[
...
function CRB_GroupDisplay:OnGroupLeft(eReason)
	local unitMe = GameLib.GetPlayerUnit()
	if unitMe == nil then
		return
    end
...
function CRB_GroupDisplay:OnGroupMemberFlags(nMemberIndex, bIsFromPromotion, tChangedFlags)
...
function CRB_GroupDisplay:OnGroupReadyCheck(nMemberIndex, strMessage)
	local tMember = GroupLib.GetGroupMember(nMemberIndex)
...
function CRB_GroupDisplay:OnGroupInviteResult(strCharacterName, eResult)

	Apollo.DPF("CRB_GroupDisplay:OnGroupInviteResult")
...
function CRB_GroupDisplay:OnGroupRequestResult(strCharacterName, eResult, bIsJoin)
	Apollo.DPF("CRB_GroupDisplay:OnGroupRequestResult")
...
function CRB_GroupDisplay:CloseGroupHUD() -- see if the HUD can be closed
...
-- Message Queue
---------------------------------------------------------------------------------------------------
function CRB_GroupDisplay:AddToQueue(nMessageIcon, strMessageText)
...
function CRB_GroupDisplay:ProcessAlerts()
	self:ClearFields()
...
function CRB_GroupDisplay:DisplayAlert(nMessageIcon, strMessageText)
...
---------------------------------------------------------------------------------------------------
function CRB_GroupDisplay:OnGroupMessageTimer()
	self.wndGroupMessage:FindChild("MessageBirthAnimation"):ToFront()
	self.wndGroupMessage:FindChild("MessageBirthAnimation"):SetSprite("sprWinAnim_BirthSmallTemp")
...
---------------------------------------------------------------------------------------------------
function CRB_GroupDisplay:ClearFields() --clear everything
	for idx =1, #karMessageIconString do
		self.wndGroupMessage:FindChild(karMessageIconString[idx]):Show(false)
	end
...
function CRB_GroupDisplay:OnGroupWrongInstance()
...
function CRB_GroupDisplay:HelperResizeGroupContents()
...
function CRB_GroupDisplay:OnGroupMentor(tMemberList, bCurrentlyMentoring, bUpdateOnly)
...
function CRB_GroupDisplay:OnToggleMentorItem(wndHandler, wndCtrl)
...
function CRB_GroupDisplay:OnMentorPlayerBtn(wndHandler, wndCtrl)
...
function CRB_GroupDisplay:OnCancelMentoringBtn(wndHandler, wndCtrl)
...
function CRB_GroupDisplay:OnMentorCloseBtn(wndHandler, wndCtrl)
	self.wndMentor:Show(false)
	self.fMentorTimerStartTime = nil
	Apollo.StopTimer("MentorTimer")
...
function CRB_GroupDisplay:OnMentorTimer()
...
function CRB_GroupDisplay:OnGroupMentorLeftAOI(nTimeUntilMentoringDisabled, bClearUI)
	if bClearUI then
...
function CRB_GroupDisplay:OnMentorAOICloseBtn()
	self.wndMentorAOI:Show(false)
	Apollo.StopTimer("MentorAOITimer")
...
function CRB_GroupDisplay:OnMentorAOITimer()
	-- This is the timer that's shown on the window
...
function CRB_GroupDisplay:OnAcceptRequest()
	self.wndRequest:Show(false)
	GroupLib.AcceptRequest()
end

function CRB_GroupDisplay:OnDenyRequest()
	self.wndRequest:Show(false)
...
function CRB_GroupDisplay:OnRaidOkay( wndHandler, wndControl, eMouseButton )
	if self.wndRaidNotice and self.wndRaidNotice:IsValid() then
...
---------------------------------------------------------------------------------------------------
-- CRB_GroupDisplay instance
---------------------------------------------------------------------------------------------------
local GroupFrameInst = CRB_GroupDisplay:new()
CRB_GroupDisplay:Init()
