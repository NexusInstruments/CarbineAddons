-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_NeedVsGreed
...
function CRB_NeedVsGreed:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_NeedVsGreed:Init()
    Apollo.RegisterAddon(self)
end

function CRB_NeedVsGreed:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_NeedVsGreed.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_NeedVsGreed:OnDocumentReady()
...
function CRB_NeedVsGreed:Close()
...
-- Main Draw Method
-----------------------------------------------------------------------------------------------
function CRB_NeedVsGreed:OnGroupLoot()
...
function CRB_NeedVsGreed:UpdateKnownLoot()
...
function CRB_NeedVsGreed:OnOneSecTimer()
...
function CRB_NeedVsGreed:DrawLoot(tCurrentElement, nItemsInQueue)
	if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
		self:Close()
		self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_NeedVsGreedForm", nil, self)
...
function CRB_NeedVsGreed:OnLootRollAllPassed(itemLooted)
...
function CRB_NeedVsGreed:OnLootRollWon(itemLoot, strWinner, bNeed)
...
function CRB_NeedVsGreed:OnLootRollSelected(itemLoot, strPlayer, bNeed)
	local strNeedOrGreed = nil
	if bNeed then
		strNeedOrGreed = Apollo.GetString("NeedVsGreed_NeedRoll")
	else
...
function CRB_NeedVsGreed:OnLootRollPassed(itemLoot, strPlayer)
	local strResult = String_GetWeaselString(Apollo.GetString("NeedVsGreed_PlayerPassed"), strPlayer, itemLoot:GetChatLinkString())
	Event_FireGenericEvent("GenericEvent_LootChannelMessage", strResult)
end

function CRB_NeedVsGreed:OnLootRoll(itemLoot, strPlayer, nRoll, bNeed)
	local strNeedOrGreed = nil
	if bNeed then
		strNeedOrGreed = Apollo.GetString("NeedVsGreed_NeedRoll")
	else
...
function CRB_NeedVsGreed:OnGiantItemIconMouseUp(wndHandler, wndControl, eMouseButton)
...
function CRB_NeedVsGreed:OnNeedBtn(wndHandler, wndControl)
	GameLib.RollOnLoot(self.wndMain:GetData(), true)
	self:Close()
end

function CRB_NeedVsGreed:OnGreedBtn(wndHandler, wndControl)
	GameLib.RollOnLoot(self.wndMain:GetData(), false)
	self:Close()
end

function CRB_NeedVsGreed:OnPassBtn(wndHandler, wndControl)
	GameLib.PassOnLoot(self.wndMain:GetData())
	self:Close()
end

function CRB_NeedVsGreed:HelperBuildItemTooltip(wndArg, itemCurr, itemModData, tGlyphData)
...
local CRB_NeedVsGreedInst = CRB_NeedVsGreed:new()
CRB_NeedVsGreedInst:Init()
