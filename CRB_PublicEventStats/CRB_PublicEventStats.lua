-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_PublicEventStats
...
local CRB_PublicEventStats = {}

function CRB_PublicEventStats:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_PublicEventStats:Init()
...
function CRB_PublicEventStats:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("CRB_PublicEventStats.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_PublicEventStats:OnDocumentReady()
...
------------------------------------------------------------------------------------
function CRB_PublicEventStats:OnToggleEventStats(peEvent)
...
function CRB_PublicEventStats:OnPublicEventStart(peEvent)
...
function CRB_PublicEventStats:InitializeScoreboard(peEvent)
	...
	if not self.wndScoreboard or not self.wndScoreboard:IsValid() then
		self.wndScoreboard = Apollo.LoadForm(self.xmlDoc , "CRB_PublicEventStatsForm", nil, self)
...
function CRB_PublicEventStats:InitializeZombie(tZombieEvent)
...
function CRB_PublicEventStats:UpdateGrid() -- self.wndScoreboard guaranteed valid and visible
...
function CRB_PublicEventStats:OnLiveStatsUpdate(peEvent)
...
function CRB_PublicEventStats:CheckTeamName(eMatchTeam, wndHeader, tLiveStats, tMatchState)
...
function CRB_PublicEventStats:HelperBuildPvPSharedGrids(wndParent, eEventType)
...
function CRB_PublicEventStats:OnPvPGridClick(wndHandler, wndControl, iRow, iCol, eMouseButton)
...
function CRB_PublicEventStats:OnContextMenuAddRival(wndHandler, wndControl)
...
function CRB_PublicEventStats:OnContextMenuReportPlayer(wndHandler, wndControl)
	local rptParticipant = wndHandler:GetData()
	Event_FireGenericEvent("GenericEvent_ReportPlayerPvP", rptParticipant)
	self:OnContextMenuPlayerClosed()
end

function CRB_PublicEventStats:OnContextMenuPlayerClosed(wndHandler, wndControl)
...
function CRB_PublicEventStats:BuildPublicEventGrid(wndGrid)
	-- Save scroll info
...
function CRB_PublicEventStats:OnPublicEventLeave(peEnding, eReason)
...
function CRB_PublicEventStats:OnPVPMatchFinished(eWinner, eReason, nDeltaTeam1, nDeltaTeam2)
...
function CRB_PublicEventStats:OnWarPartyMatchResults(tWarplotResults)
...
function CRB_PublicEventStats:OnGuildWarCoinsChanged(guildOwner, nAmountGained)
...
function CRB_PublicEventStats:OnResolutionChanged()
	self.bResolutionChanged = true -- Delay so we can get the new oValue
end

-- When the public event ends, show the appropriate screen
function CRB_PublicEventStats:OnPublicEventEnd(peEnding, eReason, tStats)
...
function CRB_PublicEventStats:OnChangeWorld()
	if self.wndScoreboard and self.wndScoreboard:IsValid() then
...
function CRB_PublicEventStats:OnClose(wndHandler, wndControl) -- Also AdventureCloseBtn
	if self.wndScoreboard then
...
-----------------------------------------------------------------------------------------------
function CRB_PublicEventStats:OnPvPLeaveMatchBtn(wndHandler, wndControl)
...
function CRB_PublicEventStats:OnPvPSurrenderMatchBtn( wndHandler, wndControl, eMouseButton )
...
function CRB_PublicEventStats:BuildAdventuresSummary() -- Also Dungeons
...
function CRB_PublicEventStats:HelperCompareAdventureScores(tPersonalStats, tComparisonStats, tScore)
...
function CRB_PublicEventStats:OnOpenDungeonMedalsBtn(wndHandler, wndControl)
	if self.wndDungeonMedalsForm and self.wndDungeonMedalsForm:IsValid() then
		self.wndDungeonMedalsForm:Destroy()
		self.wndDungeonMedalsForm = nil
	else
...
function CRB_PublicEventStats:OnDungeonMedalsClose(wndHandler, wndControl)
	if self.wndDungeonMedalsForm and self.wndDungeonMedalsForm:IsValid() then
		self.wndDungeonMedalsForm:Destroy()
		self.wndDungeonMedalsForm = nil
	end
...
function CRB_PublicEventStats:BuildDungeonMedalScreen(peDungeon)
...
-----    Helpers
-----------------------------------------------------------------------------------------------
function CRB_PublicEventStats:HelperSortTableForAdventuresSummary(tScore)
	local tNewTable = {}
...
function CRB_PublicEventStats:HelperConvertTimeToString(fTime)
...
function CRB_PublicEventStats:HelperGridFactoryProduce(wndGrid, tTargetComparison)
...
local CRB_PublicEventStatsInst = CRB_PublicEventStats:new()
