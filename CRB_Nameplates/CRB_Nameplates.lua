-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_Nameplates
...
function CRB_Nameplates:Init()
    Apollo.RegisterAddon(self, true, Apollo.GetString("CRB_Nameplates"), {"Tooltips", "RewardIcons"})
end

function CRB_Nameplates:OnDependencyError(strDependency, strError)
	return true
end

-----------------------------------------------------------------------------------------------
-- CRB_Nameplates OnLoad
-----------------------------------------------------------------------------------------------

function CRB_Nameplates:OnLoad()
	Apollo.RegisterEventHandler("UnitCreated", 					"OnPreloadUnitCreated", self)

	self.xmlDoc = XmlDoc.CreateFromFile("CRB_Nameplates.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_Nameplates:OnPreloadUnitCreated(unitNew)
	self.arPreloadUnits[#self.arPreloadUnits + 1] = unitNew
end

function CRB_Nameplates:OnDocumentReady()
	...
	self.wndMain = Apollo.LoadForm(self.xmlDoc, "CRB_NameplatesForm", nil, self)
...
function CRB_Nameplates:OnSave(eType)
...
function CRB_Nameplates:OnRestore(eType, tSavedData)
	if eType ~= GameLib.CodeEnumAddonSaveLevel.Character then
		return
	end
...
function CRB_Nameplates:OnPreloadUnitCreateTimer()
...
function CRB_Nameplates:OnVisibilityTimer()
	self:UpdateAllNameplateVisibility()
end

function CRB_Nameplates:RequestUpdateAllNameplateRewards()
	self.bRedrawRewardIcons = true
end

function CRB_Nameplates:UpdateNameplateRewardInfo(tNameplate)
	local tFlags =
	{
		bVert = false,
...
function CRB_Nameplates:UpdateAllNameplateVisibility()
...
function CRB_Nameplates:UpdateNameplateVisibility(tNameplate)
...
function CRB_Nameplates:OnUnitCreated(unitNew) -- build main options here
	if unitNew == nil
...
function CRB_Nameplates:OnUnitDestroyed(unitOwner)
...
function CRB_Nameplates:OnFrame()
	self.unitPlayer = GameLib.GetPlayerUnit()

	local fnHealth = CRB_Nameplates.DrawHealthShieldBar
...
function CRB_Nameplates:ColorNameplate(tNameplate) -- Every frame
...
function CRB_Nameplates:DrawName(tNameplate)
	local wndNameplate = tNameplate.wndNameplate
	local unitOwner = tNameplate.unitOwner
	...
	if bShow then
		...
		if tNameplate.strName ~= strNewName then
			...
			local nHalfNameWidth = math.ceil(math.max(Apollo.GetTextWidth("CRB_Nameplates", strNewName), Apollo.GetTextWidth("CRB_Interface9_BO", strNewGuild)) / 2)
...
function CRB_Nameplates:DrawGuild(tNameplate)
	local wndNameplate = tNameplate.wndNameplate
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:DrawLevel(tNameplate)
	local unitOwner = tNameplate.unitOwner

	tNameplate.wnd.level:SetText(unitOwner:GetLevel() or "-")
end

function CRB_Nameplates:DrawHealth(tNameplate)
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:DrawCastBar(tNameplate) -- Every frame
	local wndNameplate = tNameplate.wndNameplate
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:DrawVulnerable(tNameplate) -- Every frame
	local wndNameplate = tNameplate.wndNameplate
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:DrawRewards(tNameplate)
	local wndNameplate = tNameplate.wndNameplate
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:DrawTargeting(tNameplate)
	local wndNameplate = tNameplate.wndNameplate
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:CheckDrawDistance(tNameplate)
	local unitPlayer = self.unitPlayer
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:HelperVerifyVisibilityOptions(tNameplate)
	local unitPlayer = self.unitPlayer
	local unitOwner = tNameplate.unitOwner
...
function CRB_Nameplates:DrawHealthShieldBar(wndHealth, unitOwner, eDisposition, tNameplate) -- Every frame
...
function CRB_Nameplates:HelperFormatBigNumber(nArg)
	if nArg < 1000 then
...
function CRB_Nameplates:SetBarValue(wndBar, fMin, fValue, fMax)
	wndBar:SetMax(fMax)
	wndBar:SetFloor(fMin)
	wndBar:SetProgress(fValue)
end

function CRB_Nameplates:HelperCalculateConValue(unitTarget)
...
function CRB_Nameplates:OnNameplateNameClick(wndHandler, wndCtrl, eMouseButton)
...
function CRB_Nameplates:OnWorldLocationOnScreen(wndHandler, wndControl, bOnScreen)
	local tNameplate = self.arWnd2Nameplate[wndHandler:GetId()]
	if tNameplate ~= nil then
...
function CRB_Nameplates:OnUnitOcclusionChanged(wndHandler, wndControl, bOccluded)
	local tNameplate = self.arWnd2Nameplate[wndHandler:GetId()]
	if tNameplate ~= nil then
...
function CRB_Nameplates:OnUnitTextBubbleToggled(tUnitArg, strText, nRange)
...
function CRB_Nameplates:OnEnteredCombat(unitChecked, bInCombat)
	if unitChecked == self.unitPlayer then
		self.bPlayerInCombat = bInCombat
	end
end

function CRB_Nameplates:OnUnitGibbed(unitUpdated)
...
function CRB_Nameplates:OnUnitNameChanged(unitUpdated, strNewName)
	local tNameplate = self.arUnit2Nameplate[unitUpdated:GetId()]
	if tNameplate ~= nil then
		self:DrawName(tNameplate)
	end
...
function CRB_Nameplates:OnUnitTitleChanged(unitUpdated)
	local tNameplate = self.arUnit2Nameplate[unitUpdated:GetId()]
	if tNameplate ~= nil then
		self:DrawName(tNameplate)
	end
...
function CRB_Nameplates:OnPlayerTitleChanged()
...
function CRB_Nameplates:OnUnitLevelChanged(unitUpdating)
...
function CRB_Nameplates:OnGuildChange()
...
function CRB_Nameplates:OnUnitGuildNameplateChanged(unitUpdated)
	local tNameplate = self.arUnit2Nameplate[unitUpdated:GetId()]
	if tNameplate ~= nil then
		self:DrawGuild(tNameplate)
	end
...
function CRB_Nameplates:OnUnitMemberOfGuildChange(unitOwner)
...
function CRB_Nameplates:OnTargetUnitChanged(unitOwner) -- build targeted options here; we get this event when a creature attacks, too
...
-- Options
-----------------------------------------------------------------------------------------------
function CRB_Nameplates:OnConfigure()
	self:OnCRB_NameplatesOn()
end

function CRB_Nameplates:OnCRB_NameplatesOn()
...
function CRB_Nameplates:RefreshNameplatesConfigure()
...
function CRB_Nameplates:OnNormalViewCheck(wndHandler, wndCtrl)
	self.wndMain:FindChild("ContentMain"):Show(true)
	self.wndMain:FindChild("ContentTarget"):Show(false)
end

function CRB_Nameplates:OnTargetViewCheck(wndHandler, wndCtrl)
	self.wndMain:FindChild("ContentMain"):Show(false)
	self.wndMain:FindChild("ContentTarget"):Show(true)
end

-- when the OK button is clicked
function CRB_Nameplates:OnOK()
	self.wndMain:Show(false) -- hide the window
end

-- when the Cancel button is clicked
function CRB_Nameplates:OnCancel()
	self.wndMain:Show(false) -- hide the window
end

function CRB_Nameplates:OnDrawDistanceSlider(wndNameplate, wndHandler, nValue, nOldvalue)
...
function CRB_Nameplates:OnMainShowHealthBarAlways(wndHandler, wndCtrl)
	self:HelperOnMainShowHealthSettingChanged(true, false)
end

function CRB_Nameplates:OnMainShowHealthBarDamaged(wndHandler, wndCtrl)
	self:HelperOnMainShowHealthSettingChanged(false, true)
end

function CRB_Nameplates:OnMainShowHealthBarNever(wndHandler, wndCtrl)
	self:HelperOnMainShowHealthSettingChanged(false, false)
end

function CRB_Nameplates:HelperOnMainShowHealthSettingChanged(bShowHealthMain, bShowHealthMainDamaged)
	self.bShowHealthMain = bShowHealthMain
	self.bShowHealthMainDamaged = bShowHealthMainDamaged
end

function CRB_Nameplates:OnMainHideInCombat(wndHandler, wndCtrl)
	self.bHideInCombat = wndCtrl:IsChecked() -- onDraw
end

function CRB_Nameplates:OnMainHideInCombatOff(wndHandler, wndCtrl)
	self.bHideInCombat = not wndCtrl:IsChecked() -- onDraw
end

function CRB_Nameplates:OnGenericSingleCheck(wndHandler, wndControl, eMouseButton)
...
function CRB_Nameplates:OnFriendlyPlayersSingleCheck(wndHandler, wndControl, eMouseButton)
	local strSettingName = wndControl:GetData()
	if strSettingName ~= nil then
...
function CRB_Nameplates:OnSettingNameChanged()
	for idx, tNameplate in pairs(self.arUnit2Nameplate) do
		self:DrawName(tNameplate)
	end
end

function CRB_Nameplates:OnSettingTitleChanged()
	for idx, tNameplate in pairs(self.arUnit2Nameplate) do
		self:DrawGuild(tNameplate)
	end
end

function CRB_Nameplates:OnSettingHealthChanged()
	for idx, tNameplate in pairs(self.arUnit2Nameplate) do
		self:DrawLevel(tNameplate)
	end
end

function CRB_Nameplates:OnOcclusionCheck(wndHandler, wndControl, eMouseButton)
...
-----------------------------------------------------------------------------------------------
fnDrawHealth = CRB_Nameplates.DrawHealth
fnDrawRewards = CRB_Nameplates.DrawRewards
fnDrawCastBar = CRB_Nameplates.DrawCastBar
fnDrawVulnerable = CRB_Nameplates.DrawVulnerable
fnColorNameplate = CRB_Nameplates.ColorNameplate
fnDrawTargeting = CRB_Nameplates.DrawTargeting

-----------------------------------------------------------------------------------------------
-- CRB_Nameplates Instance
-----------------------------------------------------------------------------------------------
local CRB_NameplatesInst = CRB_Nameplates:new()
CRB_NameplatesInst:Init()
