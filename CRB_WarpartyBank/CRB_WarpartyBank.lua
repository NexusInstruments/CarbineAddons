-----------------------------------------------------------------------------------------------
-- Client Lua Script for CRB_WarpartyBank
...
function CRB_WarpartyBank:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function CRB_WarpartyBank:Init()
    Apollo.RegisterAddon(self)
end

function CRB_WarpartyBank:OnLoad()
    self.xmlDoc = XmlDoc.CreateFromFile("CRB_WarpartyBank.xml")
    self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function CRB_WarpartyBank:OnDocumentReady()
...
function CRB_WarpartyBank:WarPartyInitialize()
...
function CRB_WarpartyBank:Reinitialize(guildToInit)
...
function CRB_WarpartyBank:Initialize(guildOwner)
...
function CRB_WarpartyBank:CloseBank()
...
function CRB_WarpartyBank:OnBankTabBtnVault(wndHandler, wndControl)
...
function CRB_WarpartyBank:OnBankTabMouseEnter(wndHandler, wndControl)
...
function CRB_WarpartyBank:OnBankTabUncheck(wndHandler, wndControl)
    self.wndMain:FindChild("SharedBGMainFrame"):Show(true)
    self.wndMain:FindChild("MainBankNoVisibility"):Show(false)
    self:HelperEmptyMainBankScrollbar(false)
end

function CRB_WarpartyBank:OnGuildBankTab(guildOwner, nTab)
...
function CRB_WarpartyBank:HelperDrawBankItem(itemDrawing, nTab, nInventorySlot)
...
function CRB_WarpartyBank:OnGuildBankItem(guildOwner, nTab, nInventorySlot, itemUpdated, bRemoved)
...
function CRB_WarpartyBank:OnBankItemBeginDragDrop(wndHandler, wndControl) -- BankItemIcon
...
function CRB_WarpartyBank:OnBankItemQueryDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType)
...
function CRB_WarpartyBank:OnBankItemDragDropEnd() -- Any UI
    self:OnBankItemDragDropCancel()
end

function CRB_WarpartyBank:OnBankItemDragDropCancel() -- Also called from UI
    self.tCurrentDragData = nil
end

function CRB_WarpartyBank:OnBankItemEndDragDrop(wndHandler, wndControl, nX, nY, wndSource, strType, nBagSlot) -- Bank Icon
...
function CRB_WarpartyBank:OnPlayerCurrencyChanged()
...
function CRB_WarpartyBank:OnGuildInfluenceAndMoney(guildOwner, nInfluence, monCash)
...
function CRB_WarpartyBank:OnBankTabBtnPermissions(wndHandler, wndControl)
    self:DrawTabPermissions()
end

function CRB_WarpartyBank:OnPermissionsResetBtn(wndHandler, wndControl)
    self:DrawTabPermissions()
end

function CRB_WarpartyBank:OnGuildRankChange() -- C++ Event
...
function CRB_WarpartyBank:OnPermissionsCurrentBtnLeftRight(wndHandler, wndControl)
...
function CRB_WarpartyBank:DrawTabPermissions()
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:BuildPermissionIndividualTab(wndParent, guildOwner, bCanEditRanks, tCurrRankData, nBankTab)
...
function CRB_WarpartyBank:OnPermissionGridBtnVisibleDeposit(wndHandler, wndControl)
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:OnPermissionsSaveBtn(wndHandler, wndControl)
...
function CRB_WarpartyBank:OnBankTabBtnMgmt()
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:OnLeaderOptionsEditBoxChanged(wndHandler, wndControl)
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:OnLeaderOptionsTabRenameBtn(wndHandler, wndControl)
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:OnGuildBankTabRename(guildOwner)
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:OnLeaderOptionsTabBuyBtn(wndHandler, wndControl)
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
...
function CRB_WarpartyBank:OnGuildBankTabCount()
    if not self.wndMain or not self.wndMain:IsValid() or not self.wndMain:GetData() then
        return
    end
    local guildOwner = self.wndMain:GetData()
...
function CRB_WarpartyBank:HelperEmptyMainBankScrollbar(bShow)
...
function CRB_WarpartyBank:HelperUpdateHeaderText(strNewHeader)
...
function CRB_WarpartyBank:DoFlashAnimation()
    if not self.wndMain or not self.wndMain:IsValid() then
        return
    end
...
function CRB_WarpartyBank:HelperBuildItemTooltip(wndArg, itemCurrent)
...
function CRB_WarpartyBank:HelperLoadTabPerks(guildOwner)
    local tGuildPerks = guildOwner:GetPerks()
...
function CRB_WarpartyBank:FactoryProduce(wndParent, strFormName, tObject)
...
function CRB_WarpartyBank:HelperConvertToTime(nDays)
...
local CRB_WarpartyBankInst = CRB_WarpartyBank:new()
CRB_WarpartyBankInst:Init()
